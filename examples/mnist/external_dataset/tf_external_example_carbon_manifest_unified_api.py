...
def train_and_test(train_dataset_url, test_dataset_url, num_epochs, batch_size, evaluation_interval):
  """
  ...
  with make_batch_carbon_reader(os.path.join(train_dataset_url), num_epochs=num_epochs) as train_reader:
    with make_batch_carbon_reader(os.path.join(test_dataset_url), num_epochs=num_epochs) as test_reader:
      # Create the model
...
def main():
  ...
  parser.add_argument('--train_dataset-url', type=str,
                      default=default_dataset_url + "mnistcarbonTrain.manifest", metavar='S',
                      help='hdfs:// or file:/// URL to the MNIST pycarbon dataset'
                           '(default: %s)' % default_dataset_url)
  parser.add_argument('--test_dataset-url', type=str,
                      default=default_dataset_url + "mnistcarbonTest.manifest", metavar='S',
                      help='hdfs:// or file:/// URL to the MNIST pycarbon dataset'
                           '(default: %s)' % default_dataset_url)
  ...
  train_and_test(
    train_dataset_url=args.train_dataset_url,
