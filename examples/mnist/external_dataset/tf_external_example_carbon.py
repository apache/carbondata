...
def train_and_test(dataset_url, num_epochs, batch_size, evaluation_interval):
  ...
  with make_batch_carbon_reader(os.path.join(dataset_url, 'train'), num_epochs=num_epochs) as train_reader:
    with make_batch_carbon_reader(os.path.join(dataset_url, 'test'), num_epochs=num_epochs) as test_reader:
      # Create the model
