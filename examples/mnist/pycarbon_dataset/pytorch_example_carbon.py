...
def main():
  ...
  for epoch in range(1, loop_epochs + 1):
    with DataLoader(make_carbon_reader('{}/train'.format(args.dataset_url), num_epochs=reader_epochs,
                                       transform_spec=transform),
                    batch_size=args.batch_size) as train_loader:
      train(model, device, train_loader, args.log_interval, optimizer, epoch)

    with DataLoader(make_carbon_reader('{}/test'.format(args.dataset_url), num_epochs=reader_epochs,
                                       transform_spec=transform),
