...
from examples.mnist.pycarbon_dataset import mxnet_example_carbon_unified_api as mxnet_example_unified
...
def test_full_mxnet_example_unified(large_mock_mnist_data, tmpdir):
  # First, generate mock dataset
  dataset_url = 'file://{}'.format(tmpdir)
  mnist_data_to_pycarbon_dataset(tmpdir, dataset_url, mnist_data=large_mock_mnist_data,
                                 spark_master='local[1]', carbon_files_count=1)

  mxnet_example_unified.mnist_iter_test(
    dataset_path=dataset_url + "/train",
    num_epoch=1,
    batch_size=100
  )
