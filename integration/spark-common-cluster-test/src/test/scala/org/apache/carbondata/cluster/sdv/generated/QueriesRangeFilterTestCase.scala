...
class QueriesRangeFilterTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("Range_Filter_01_1", Include) {
    ...
    sql(s"""CREATE TABLE if not exists directDictionaryTable (empno int,doj Timestamp, salary int) STORED AS carbondata""").collect
