...
public final class StageManager {
  ...
  public static void writeStageInput(final String stageInputPath, final StageInput stageInput)
          throws IOException {
    AtomicFileOperations fileWrite = AtomicFileOperationFactory.getAtomicFileOperations(stageInputPath);
    BufferedWriter brWriter = null;
    DataOutputStream dataOutputStream = null;
    try {
      dataOutputStream = fileWrite.openForWrite(FileWriteOperation.OVERWRITE);
      brWriter = new BufferedWriter(new OutputStreamWriter(dataOutputStream, StandardCharsets.UTF_8));
      ...
      CarbonUtil.closeStreams(new Closeable[]{brWriter});
      fileWrite.close();
    }
  ...
  public static void writeSuccessFile(final String successFilePath) throws IOException {
    final DataOutputStream segmentStatusSuccessOutputStream =
            FileFactory.getDataOutputStream(
                    successFilePath,
                    FileFactory.getFileType(successFilePath),
