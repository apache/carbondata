...
class AllDataTypesTestCaseAggregate extends QueryTest with BeforeAndAfterAll {
  ...
  test("CARBONDATA-60-union-defect")({
    checkAnswer(
      sql("select c1,count(c1) from (select MAC as c1, deviceColor  as c2 from Carbon_automation_test union all select MAC as c1, deviceColor  as c2 from Carbon_automation_test)t where c1='MAC' group by c1"),
      sql("select c1,count(c1) from (select MAC as c1, deviceColor  as c2 from Carbon_automation_test_hive union all select MAC as c1, deviceColor  as c2 from Carbon_automation_test_hive)t where c1='MAC' group by c1"))
  })
