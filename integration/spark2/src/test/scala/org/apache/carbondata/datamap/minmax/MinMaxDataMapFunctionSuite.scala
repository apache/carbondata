...
class MinMaxDataMapFunctionSuite extends QueryTest with BeforeAndAfterAll {
  ...
  test("create minmax datamap on all datatypes with direct write and late rebuild") {
    val normalTableName = "normal_all_data_types"
    val tableName = "minmax_all_data_types"
    val dataMapName = "dm_minmax_with_all_data_types"
    sql(s"DROP TABLE IF EXISTS $normalTableName")
    sql(s"DROP TABLE IF EXISTS $tableName")
    createAllDataTypeTable(normalTableName)
    createAllDataTypeTable(tableName)
    loadAllDataTypeTable(normalTableName)
    ...
    loadAllDataTypeTable(normalTableName)
    loadAllDataTypeTable(tableName)
    checkExistence(sql(s"SHOW DATAMAP ON TABLE $tableName"),
      true, minmaxDataMapFactoryName, dataMapName)
    checkAnswer(sql(s"SELECT COUNT(*) FROM $tableName"), sql(s"SELECT COUNT(*) FROM $normalTableName"))
    checkAnswer(sql(s"SELECT COUNT(*) FROM (SELECT * FROM $tableName WHERE dateField='2015-05-23' AND timestampField='2015-05-23 12:01:03') b"),
      sql(s"SELECT COUNT(*) FROM (SELECT * FROM $normalTableName WHERE dateField='2015-05-23' AND timestampField='2015-05-23 12:01:03') b"))
    checkExistence(
      ...
      true, minmaxDataMapFactoryName, dataMapName)
    checkAnswer(
      sql(s"SELECT * FROM $tableName WHERE booleanField=false AND shortField=2 AND intField=12 AND bigintField=102 AND stringField='string2' AND timestampField='2015-05-23 12:01:03' AND decimalField=23.45 AND dateField='2015-05-23' AND charField='bbb' AND floatField=2.5 AND stringDictField='dict2' AND stringSortField='sort2' AND stringLocalDictField= 'local_dict2' AND longStringField='longstring2'"),
      sql(s"SELECT * FROM $normalTableName WHERE booleanField=false AND shortField=2 AND intField=12 AND bigintField=102 AND stringField='string2' AND timestampField='2015-05-23 12:01:03' AND decimalField=23.45 AND dateField='2015-05-23' AND charField='bbb' AND floatField=2.5 AND stringDictField='dict2' AND stringSortField='sort2' AND stringLocalDictField= 'local_dict2' AND longStringField='longstring2'"))
    checkAnswer(
      ...
      sql(s"SELECT COUNT(*) FROM (SELECT * FROM $normalTableName WHERE booleanField=false AND shortField=2 AND intField=12 AND bigintField=102 AND stringField='string2' AND timestampField='2015-05-23 12:01:03' AND decimalField=23.45 AND dateField='2015-05-23' AND charField='bbb' AND floatField=2.5 AND stringDictField='dict2' AND stringSortField='sort2' AND stringLocalDictField= 'local_dict2' AND longStringField='longstring2') b"))
    sql(s"DROP TABLE IF EXISTS $normalTableName")
    sql(s"DROP TABLE IF EXISTS $tableName")
  }

  test("create minmax datamap on all datatypes with sort columns with direct write and late rebuild") {
    val normalTableName = "normal_all_data_types_with_sort_column"
    val tableName = "minmax_all_data_types_with_sort_column"
    val dataMapName = "dm_minmax_with_all_data_types_with_sort_column"
    sql(s"DROP TABLE IF EXISTS $normalTableName")
    sql(s"DROP TABLE IF EXISTS $tableName")
    def createAllDataTypeWithSortColumnTable(name : String) = {
      sql(
        s"""
           | CREATE TABLE $name(
           |    booleanField boolean,
    ...
    createAllDataTypeWithSortColumnTable(normalTableName)
    createAllDataTypeWithSortColumnTable(tableName)
    loadAllDataTypeTable(normalTableName)
    loadAllDataTypeTable(tableName)
    // create datamap on all supported datatype
    sql(
  ...
  test("test minmax datamap with empty values on index column") {
    val normalTable = "minmax_normal_table"
    val bloomDMSampleTable = "minmax_bloom_table"
    val dataMapName = "minmax_dm_datamap"
  ...
  test("create multiple datamaps vs create on datamap on multiple columns") {
    val tableName1 = "minmax_all_data_types10"
    val tableName2 = "minmax_all_data_types20"
    val dataMapName1 = "dm_minmax_with_all_data_types10"
    val dataMapName2Prefix = "dm_minmax_with_all_data_types2"
    ...
    checkExistence(
      ...
      true, minmaxDataMapFactoryName, dataMapName1)
    val allDataMapsOnTable2 = (0 to 14).map(p => s"$dataMapName2Prefix$p")
    val existedString = (allDataMapsOnTable2 :+ minmaxDataMapFactoryName).toArray
    checkExistence(
    ...
    checkAnswer(
      sql(s"SELECT * FROM $tableName1 WHERE booleanField=false AND shortField=2 AND intField=12 AND bigintField=102 AND stringField='string2' AND timestampField='2015-05-23 12:01:03' AND decimalField=23.45 AND dateField='2015-05-23' AND charField='bbb' AND floatField=2.5 AND stringDictField='dict2' AND stringSortField='sort2' AND stringLocalDictField= 'local_dict2' AND longStringField='longstring2'"),
      sql(s"SELECT * FROM $tableName2 WHERE booleanField=false AND shortField=2 AND intField=12 AND bigintField=102 AND stringField='string2' AND timestampField='2015-05-23 12:01:03' AND decimalField=23.45 AND dateField='2015-05-23' AND charField='bbb' AND floatField=2.5 AND stringDictField='dict2' AND stringSortField='sort2' AND stringLocalDictField= 'local_dict2' AND longStringField='longstring2'"))
