...
case class BatchedDataSourceScanExec(
  ...
  override protected def doProduce(ctx: CodegenContext): String = {
    ...
    ctx.addMutableState("scala.collection.Iterator", input, v => s"$v = inputs[0];")

    // metrics
    val numOutputRows = metricTerm(ctx, "numOutputRows")
    val scanTimeMetric = metricTerm(ctx, "scanTime")
    val scanTimeTotalNs = ctx.freshName("scanTime")
    ctx.addMutableState("long", scanTimeTotalNs, v =>  s"$v = 0;")

    val columnarBatchClz = "org.apache.spark.sql.vectorized.ColumnarBatch"
    val batch = ctx.freshName("batch")
    ctx.addMutableState(columnarBatchClz, batch, v => s"$v = null;")

    val columnVectorClz = "org.apache.spark.sql.vectorized.ColumnVector"
    val idx = ctx.freshName("batchIdx")
    ctx.addMutableState("int", idx, v => s"$idx = 0;")
    val colVars = output.indices.map(i => ctx.freshName("colInstance" + i))
    val columnAssigns = colVars.zipWithIndex.map { case (name, i) =>
      ctx.addMutableState(columnVectorClz, name, v => s"$v = null;")
