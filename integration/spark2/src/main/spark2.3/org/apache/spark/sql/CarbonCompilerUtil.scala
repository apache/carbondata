...
object CarbonCompilerUtil {
  ...
  //May be not required to commit this change
  def addMutableState(ctx: CodegenContext, javaType: String, variableName: String,
                      initCode: String,initFunc: String => String = _ => ""): Unit = {
    ctx.addMutableState(javaType,variableName,initFunc)
  }
