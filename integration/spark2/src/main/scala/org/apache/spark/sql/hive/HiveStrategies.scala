...
private[hive] trait HiveStrategies {
   ...
   * applied.
   */
  object HiveTableHeaderCountScans extends Strategy {
    logError(s"===using HiveTableHeaderCountScans===")
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      case PhysicalOperation(projectList, predicates, relation: MetastoreRelation) =>
        ...
        pruneFilterProject(
          ...
          HiveTableHeaderCountScanExec(_, relation, pruningPredicates)(sparkSession)) :: Nil
      case _ =>
        Nil
    }
  }
