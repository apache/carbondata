 ...
 */
      ...
      case PhysicalOperation(projects, filters, l: LogicalRelation)
        ...
        try {
          pruneFilterProject(
            ...
            projects,
            filters,
            (a, f, p) => toCatalystRDD(l, a, relation.buildScan(
              a.map(_.name).toArray, filters, projects, f, p))) :: Nil
        } catch {
  ...
  RDD[InternalRow] = {
    ...
    scanRdd.setVectorReaderSupport(supportBatchedDataSource(relation.relation.sqlContext, output))
    rdd
  }
    ...
    if (projects.map(_.toAttribute) == projects &&
      ...
      var updateRequestedColumns =
        if (!vectorPushRowFilters && !implicitExisted) {
          (projectsAttr.to[mutable.LinkedHashSet] ++ filterSet)
            .map(relation.attributeMap).toSeq
      } else {
        requestedColumns
      }
      val supportBatch =
        supportBatchedDataSource(
          relation.relation.sqlContext,
