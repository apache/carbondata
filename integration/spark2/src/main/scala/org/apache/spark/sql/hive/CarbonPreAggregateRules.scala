  ...
  private def getExpressionsForStreaming(aggExp: Seq[Expression]): Seq[Expression] = {
    val updatedExp = aggExp map {
      ...
      case alias@Alias(aggExp: AggregateExpression, name) =>
        ...
        // sort, project, join
        CarbonCompilerUtil.createAliasRef(
          updatedAggExp.head,
          name,
          alias.exprId,
          alias.qualifier,
      ...
      if(!expressions.contains(normalizedExp)) {
        gExp match {
          case attr: AttributeReference =>
            newAggExp += attr
          case exp: Expression =>
            newAggExp += CarbonCompilerUtil.createAliasRef(
              exp,
              "dummy_" + counter,
    ...
    val updatedExp = addedExp.flatMap {
      ...
      case alias@Alias(aggExp: AggregateExpression, name) =>
        ...
        val updatedExp = newAggExp.map { exp =>
          CarbonCompilerUtil.createAliasRef(exp,
            name,
            NamedExpression.newExprId,
            alias.qualifier,
            Some(alias.metadata),
      ...
      case alias@Alias(exp: Expression, name) =>
        ...
        val attr = CarbonCompilerUtil.createAttributeReference(name,
          alias.dataType,
          alias.nullable,
          alias.metadata,
          alias.exprId,
  ...
  def getUpdatedExpressions(groupingExpressions: Seq[Expression],
    ...
    val updatedAggExp = aggregateExpressions.flatMap {
      ...
      case attr: AttributeReference =>
        ...
        val childTableAttr = CarbonCompilerUtil.createAttributeReference(attr.name,
          childAttr.dataType,
          childAttr.nullable,
          childAttr.metadata,
          newExpressionId,
      ...
      case alias@Alias(attr: AttributeReference, name) =>
        val childAttr = getChildAttributeReference(aggDataMapSchema,
          ...
          attributes)
        val newExpressionId = NamedExpression.newExprId
        val parentTableAttr = CarbonCompilerUtil.createAttributeReference(name,
          alias.dataType,
          alias.nullable,
          Metadata.empty,
          alias.exprId,
          alias.qualifier,
          alias)
        val childTableAttr = CarbonCompilerUtil.createAttributeReference(name,
          alias.dataType,
          alias.nullable,
          Metadata.empty,
          newExpressionId,
