        ...
        if (FileFactory.isFileExist(tablePath, fileType)) {
          ...
          segments.foreach {validSeg =>
            if (validSeg.getSegmentFileName != null) {
              val fileStore = new SegmentFileStore
              fileStore.readSegment(tablePath, validSeg.getSegmentFileName)
              size = size + CarbonUtil.getSizeOfSegment(
                carbonTablePath, new Segment(validSeg.getSegmentId, validSeg.getSegmentFileName))
            } else {
              size = size + FileFactory.getDirectorySize(
                CarbonTablePath.getSegmentPath(tablePath, validSeg.getSegmentId))
            }
          }
