...
class CarbonFileMetastore extends CarbonMetaStore {
    ...
    tables match {
      ...
      case None =>
        readCarbonSchema(absIdentifier,
          parameters.getOrElse("isNonTransactional", "false").toBoolean) match {
          case Some(meta) =>
  ...
  private def readCarbonSchema(identifier: AbsoluteTableIdentifier,
    ...
    if (inferSchema) {
      ...
      wrapperTableInfo.setNonTransactionalTable(true)
  ...
  def dropTable(absoluteTableIdentifier: AbsoluteTableIdentifier)
    ...
    if (FileFactory.isFileExist(metadataFilePath, fileType)) {
      ...
      if (isNonTransactionalCarbonTable(absoluteTableIdentifier)) {
        removeTableFromMetadata(dbName, tableName)
        CarbonHiveMetadataUtil.invalidateAndDropTable(dbName, tableName, sparkSession)
        // discard cached table info in cachedDataSourceTables
        val tableIdentifier = TableIdentifier(tableName, Option(dbName))
  ...
  def isNonTransactionalCarbonTable(identifier: AbsoluteTableIdentifier): Boolean = {
    val table = getTableFromMetadataCache(identifier.getDatabaseName, identifier.getTableName)
    table.map(_.getTableInfo.isNonTransactionalTable).getOrElse(false)
  }
