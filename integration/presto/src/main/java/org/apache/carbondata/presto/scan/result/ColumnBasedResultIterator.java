...
public class ColumnBasedResultIterator extends AbstractDetailQueryResultIterator<Object> {
  ...
  private BatchResult getBatchResult() {
    ...
    synchronized (lock) {
      ...
      if (dataBlockIterator != null) {
        batchResult
            .setRows(((ColumnDataBlockIteratorImpl) dataBlockIterator).processNextColumnBatch());
      }
    }
    return batchResult;
  }

  @Override protected void updateDataBlockIterator() {
  ...
  private ColumnDataBlockIteratorImpl getDataBlockIterator() {
    if (blockExecutionInfos.size() > 0) {
      ...
      return new ColumnDataBlockIteratorImpl(executionInfo, fileReader, batchSize,
          queryStatisticsModel, execService);
    }
    return null;
  }
