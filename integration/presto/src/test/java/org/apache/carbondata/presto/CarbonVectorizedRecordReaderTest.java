...
public class CarbonVectorizedRecordReaderTest {
    ...
    private static QueryDimension queryDimension;
    private static QueryMeasure queryMeasure;
    private static List<QueryMeasure> queryMeasures;
    private static List<QueryDimension> queryDimensions;
    ...
    public static void setUp() {
        ...
        queryDimension = new QueryDimension("name");
        queryDimension.setQueryOrder(0);
        ColumnSchema columnSchema = new ColumnSchema();
        List<Encoding> encodings = new ArrayList();
        encodings.add(Encoding.DICTIONARY);
        columnSchema.setEncodingList(encodings);
        queryDimension.setDimension(new CarbonDimension(columnSchema, 0, 0, 0, 0));
        queryMeasure = new QueryMeasure("id");
        queryMeasure.setQueryOrder(1);
        queryMeasures = new ArrayList<>();
        ColumnSchema measureColSchema = new ColumnSchema();
        measureColSchema.setDataType(DataType.INT);
        queryMeasure.setMeasure(new CarbonMeasure(measureColSchema, 1));
        queryMeasures.add(queryMeasure);
        queryDimensions = new ArrayList<>();
        queryDimensions.add(queryDimension);
        queryModel.setQueryDimension(queryDimensions);
        queryModel.setQueryMeasures(queryMeasures);
    ...
    @Test
    public void testInitializeForEmptySplit()
    ...
    @Test
    public void testInitializeForValidSplit()
            throws IOException, InterruptedException, NoSuchFieldException, IllegalAccessException {
        carbonInputSplit = new CarbonInputSplit();
    ...
    @Test
    public void testNextKeyValueFalseCase() throws IOException, InterruptedException {
        new MockUp<AbstractDetailQueryResultIterator>() {
            @Mock
    ...
    @Test
    public void testNextKeyValueInitBatchCase() throws IOException, InterruptedException, NoSuchFieldException, IllegalAccessException {
        Field columnBatch = CarbonVectorizedRecordReader.class.getDeclaredField("columnarBatch");
        columnBatch.setAccessible(true);
        columnBatch.set(carbonVectorizedRecordReader, null);
	    assertTrue(carbonVectorizedRecordReader.nextKeyValue());
    }
}
