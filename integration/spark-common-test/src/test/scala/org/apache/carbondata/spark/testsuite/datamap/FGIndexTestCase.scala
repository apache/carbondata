...
import org.apache.carbondata.core.datamap.dev.fgindex.{FineGrainBlocklet, FineGrainIndex, FineGrainIndexFactory}
...
class FGIndexFactory(carbonTable: CarbonTable,
   ...
   */
  override def getIndexes(distributable: IndexDistributable): java.util.List[FineGrainIndex]= {
...
class FGIndexTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("test fg index") {
    ...
    sql(
      s"""
         | CREATE INDEX ggdatamap ON TABLE datamap_test
  ...
  test("test fg index with 2 indexes ") {
    sql("DROP TABLE IF EXISTS datamap_test")
    sql(
      """
        | CREATE TABLE datamap_test(id INT, name STRING, city STRING, age INT)
    ...
    sql(
      s"""
         | CREATE INDEX ggdatamap1 ON TABLE datamap_test
         | USING '${classOf[FGIndexFactory].getName}'
         | PROPERTIES('index_columns'='name')
       """.stripMargin)
    sql(
      s"""
         | CREATE INDEX ggdatamap2 ON TABLE datamap_test
         | USING '${classOf[FGIndexFactory].getName}'
