...
class TestIndexCommand extends QueryTest with BeforeAndAfterAll {
  ...
  test("test index create: don't support using non-exist class") {
    intercept[MetadataProcessException] {
      sql(s"CREATE INDEX datamap1 ON TABLE datamaptest USING '$newClass'")
    }
  }

  test("test index create with dmproperties: don't support using non-exist class") {
    intercept[MetadataProcessException] {
      sql(s"CREATE INDEX datamap2 ON TABLE datamaptest USING '$newClass' PROPERTIES('key'='value')")
    }
  }
  ...
  test("test index create with existing name: don't support using non-exist class") {
    intercept[MetadataProcessException] {
      sql(
        s"CREATE INDEX datamap2 ON TABLE datamaptest USING '$newClass' PROPERTIES('key'='value')")
    }
  }
  ...
  test("test show indexes with no index") {
    ...
    assert(sql("show indexes on table datamapshowtest").collect().length == 0)
  }
  ...
  test("test show indexes: show index property related information") {
    ...
    sql(
      s"""
         | create index $datamapName on table $tableName using 'bloomfilter'
         | PROPERTIES ('index_columns'='a', 'bloom_size'='32000', 'bloom_fpp'='0.001')
       """.stripMargin)
    sql(
      s"""
         | CREATE INDEX $datamapName2 on table $tableName using 'bloomfilter'
         | PROPERTIES ('index_columns'='b')
       """.stripMargin)
    sql(
      s"""
         | CREATE INDEX $datamapName3 on table $tableName using 'bloomfilter'
         | PROPERTIES ('index_columns'='c')
       """.stripMargin)
    var result = sql(s"show indexes on table $tableName").cache()
