...
import org.apache.carbondata.core.datamap.dev.cgindex.{CoarseGrainIndex, CoarseGrainIndexFactory}
import org.apache.carbondata.core.index.{IndexDistributable, IndexMeta}
...
class CGIndexFactory(
   ...
   */
  override def getIndexes(distributable: IndexDistributable): java.util.List[CoarseGrainIndex] = {
    val mapDistributable = distributable.asInstanceOf[BlockletIndexDistributable]
    val dataMap: CoarseGrainIndex = new CGIndex()
   ...
   * @return
   */
  override def toDistributable(segment: Segment): java.util.List[IndexDistributable] = {
    val path = identifier.getTablePath
    val file = FileFactory.getCarbonFile(
      path+ "/" +dataMapSchema.getDataMapName + "/" + segment.getSegmentNo)
    ...
    val files = file.listFiles()
    files.map { f =>
      val d:IndexDistributable = new BlockletIndexDistributable(f.getCanonicalPath)
      d
    }.toList.asJava
  }
...
class CGIndex extends CoarseGrainIndex {
  ...
  /**
   * clears all the resources for indexes
   */
  override def finish() = {
    ???
  }
...
class CGIndexTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("test cg index") {
    ...
    sql(
      """
        ...
        | TBLPROPERTIES('SORT_COLUMNS'='city,name', 'SORT_SCOPE'='LOCAL_SORT')
      """.stripMargin)
    sql(s"create index cgindex on table datamap_test_cg " +
        s"using '${classOf[CGIndexFactory].getName}' " +
        s"PROPERTIES('index_columns'='name')")
  ...
  test("test index storage in system folder") {
    ...
    sql(
      s"""
         |create index test_cg_datamap on table datamap_store_test
  ...
  test("test index storage and drop in system folder") {
    ...
    sql(
      s"""
         |create index test_cg_datamap1 on table datamap_store_test1
         |using '$dataMapProvider'
         |properties('index_columns'='name')
       """.stripMargin)
  ...
  test("test show indexes storage") {
    ...
    sql(
      s"""
         |create index test_cg_datamap2 on table datamap_store_test2
         |using '$dataMapProvider'
         |properties('index_columns'='name')
       """.stripMargin)
    ...
    checkExistence(sql("show indexes"), true, "test_cg_datamap2")
