...
class GeoTest extends QueryTest with BeforeAndAfterAll {
  ...
  test("Invalid geo index handler property") {
    ...
    var exception = intercept[MalformedCarbonCommandException](sql(
      s"""
         | CREATE TABLE malformed(regionId LONG, cellkeyid LONG, longitude LONG, latitude LONG)
         | COMMENT "This is a malformed table"
         | STORED AS carbondata
         | TBLPROPERTIES ('INDEX_HANDLER'='cellkeyid')
      """.stripMargin))

    assert(exception.getMessage.contains(
      "handler: cellkeyid must not match with any other column name in the table"))
