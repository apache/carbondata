...
class TestIndexStatus extends QueryTest with BeforeAndAfterAll {
  ...
  test("index status enable for new index") {
    ...
    sql(
      ...
      """.stripMargin)
    sql(
      s"""create index statusdatamap on table datamapstatustest
  ...
  test("index status disable for new index WITH DEFERRED REFRESH") {
    sql("DROP TABLE IF EXISTS datamapstatustest")
    sql(
      """
        | CREATE TABLE datamapstatustest(id int, name string, city string, age int)
  ...
  test("index status disable after new load  WITH DEFERRED REFRESH") {
    ...
    sql(
      ...
      """.stripMargin)
    sql(
      s"""create index statusdatamap1 on table datamapstatustest1
         |using '${classOf[TestIndexFactory].getName}'
         |WITH DEFERRED REFRESH
         |properties('index_columns'='name')
         | """.stripMargin)
  ...
  test("index status with REFRESH INDEX") {
    ...
    sql(
      ...
      """.stripMargin)
    sql(
      s"""create index statusdatamap2 on table datamapstatustest2
         |using '${classOf[TestIndexFactory].getName}'
         |WITH DEFERRED REFRESH
         |properties('index_columns'='name')
         | """.stripMargin)
  ...
  test("refresh index status") {
    ...
    sql(
      ...
      """.stripMargin)
    sql(
      s"""create index statusdatamap3 on table datamapstatustest3
         |using '${classOf[TestIndexFactory].getName}'
         |WITH DEFERRED REFRESH
         |properties('index_columns'='name')
         | """.stripMargin)
    ...
    checkExistence(sql(s"show indexes"), true, "statusdatamap3")
...
class TestIndexFactory(
  ...
  override def getIndexes(distributable: IndexDistributable): util.List[CoarseGrainIndex] = {
    ???
  }
  ...
  override def toDistributable(segmentId: Segment): util.List[IndexDistributable] = {
