...
class LuceneFineGrainIndexSuite extends QueryTest with BeforeAndAfterAll {
  ...
  test("validate INDEX_COLUMNS property") {
    ...
    var exception = intercept[MalformedIndexCommandException](sql(
      s"""
         | CREATE INDEX dm1
         | ON TABLE datamap_test (city, school)
         | AS 'lucene'
    """.stripMargin))
    ...
    exception = intercept[MalformedIndexCommandException](sql(
      s"""
         | CREATE INDEX dm1
         | ON TABLE datamap_test (name, city, name)
         | AS 'lucene'
      """.stripMargin))
  ...
  ignore("test lucene rebuild data map") {
    ...
    sql(
      s"""
         | CREATE INDEX dm4
         | ON TABLE datamap_test4 (name, city)
         | AS 'lucene'
         | WITH DEFERRED REFRESH
      """.stripMargin)
  ...
  test("test lucene fine grain data map drop") {
    ...
    sql(
      s"""
         | CREATE INDEX dm12
         | ON TABLE datamap_test1 (name, city)
  ...
  test("test lucene fine grain data map show") {
    ...
    sql(
      s"""
         | CREATE INDEX dm122
         | ON TABLE datamap_test2 (name, city)
         | AS 'org.apache.carbondata.index.lucene.LuceneFineGrainIndexFactory'
      """.stripMargin)
    ...
    sql(
      s"""
         | CREATE INDEX dm123
         | ON TABLE datamap_test3 (name, city)
         | AS 'org.apache.carbondata.index.lucene.LuceneFineGrainIndexFactory'
      """.stripMargin)
  ...
  test("test lucene fine grain data map with wildcard matching ") {
    ...
    sql(
      s"""
         | CREATE INDEX dm
         | ON TABLE datamap_test_table (name, city)
         | AS 'lucene'
      """.stripMargin)
  ...
  test("test lucene fine grain data map with CTAS") {
    ...
    sql(
      s"""
         | CREATE INDEX dm
         | ON TABLE source_table (name,city)
         | AS 'lucene'
          """.stripMargin)
  ...
  test("test lucene fine grain data map with text-match limit") {
    ...
    sql(
      s"""
         | CREATE INDEX dm
         | ON TABLE datamap_test_limit (name,city)
         | AS 'lucene'
      """.stripMargin)
  ...
  test("test lucene fine grain data map with InsertOverwrite") {
    ...
    sql(
      s"""
         | CREATE INDEX dm
         | ON TABLE datamap_test_overwrite (name,city)
         | AS 'lucene'
      """.stripMargin)
  ...
  test("test lucene index creation for blocked features") {
    ...
    sql(
      s"""
         | CREATE INDEX dm124
         | ON TABLE datamap_test7 (name,city)
         | AS 'org.apache.carbondata.index.lucene.LuceneFineGrainIndexFactory'
      """.stripMargin)
  ...
  ignore("test lucene fine grain multiple data map on table") {
    ...
    sql(
      s"""
         ...
         | ON TABLE datamap_test5 (city)
         | AS 'lucene'
      """.stripMargin)
    sql(
      s"""
         | CREATE INDEX dm_name
         | ON TABLE datamap_test5 (name)
         | AS 'lucene'
      """.stripMargin)
  ...
  test("test create index: unable to create same index for one column") {
    ...
    val exception_duplicate_column: Exception = intercept[MalformedIndexCommandException] {
      sql(
        s"""
           | CREATE INDEX dm
           | ON TABLE datamap_test_table (name)
           | AS 'lucene'
      """.stripMargin)
      sql(
        s"""
