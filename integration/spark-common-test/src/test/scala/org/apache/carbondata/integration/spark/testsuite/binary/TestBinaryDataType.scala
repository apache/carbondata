...
class TestBinaryDataType extends QueryTest with BeforeAndAfterAll {
  ...
  test("create table and load data with binary column") {
    ...
    sql("CREATE TABLE IF NOT EXISTS binaryTable (id Int, name String, image Binary)" +
        " STORED BY 'org.apache.carbondata.format'")
    sql(s"LOAD DATA LOCAL INPATH '$resourcesPath/binarydata.csv' into table binaryTable options" +
        s"('header'='false')")
