...
package org.apache.carbondata.trino.impl;
...
import org.apache.carbondata.trino.split.CarbonLocalInputSplit;
...
public class CarbonTableReader {
   ...
   */
  private final AtomicReference<Map<SchemaTableName, CarbonTableCacheModel>> carbonCache;
   ...
   * and cache all the table names in this.tableList. Notice that whenever
   * this method
   * is called, it clears this.tableList and populate the list by reading the
   * files.
   */
  private void updateSchemaTables(SchemaTableName schemaTableName, Configuration config) {
    ...
    if (carbonTableCacheModel != null && carbonTableCacheModel.getCarbonTable()
        .isTransactionalTable()) {
      CarbonTable carbonTable = carbonTableCacheModel.getCarbonTable();
      long latestTime =
          FileFactory.getCarbonFile(CarbonTablePath.getSchemaFilePath(carbonTable.getTablePath()),
   ...
   * @return the CarbonTableCacheModel instance which contains all the needed
  ...
  private CarbonTableCacheModel parseCarbonMetadata(SchemaTableName table, String tablePath,
    ...
    try {
      ...
      // multiple tasks can be launched in a worker concurrently. Hence need
      // to synchronize this.
      synchronized (this) {
        ...
        // wrapperTableInfo is the code level information of a table in
        // carbondata core,
        // different from the Thrift TableInfo.
        TableInfo wrapperTableInfo =
            schemaConverter.fromExternalToWrapperTableInfo(tableInfo, table.getSchemaName(),
                table.getTableName(), tablePath);
  ...
  private void refreshIndexInfo(CarbonTable carbonTable, Configuration config) {
    ...
    for (IndexTableInfo indexTableInfo : indexTableInfos) {
      ...
      if (indexProperties != null) {
        ...
        // in case if SI table has been created before the change
        // CARBONDATA-3765,
        // indexProperties variable will not be present. On direct upgrade of
        // SI store,
        // indexProperties will be null, in that case, create indexProperties
        // from indexCols
        // For details, refer
        // {@link org.apache.spark.sql.secondaryindex.hive
  ...
  public List<CarbonLocalMultiBlockSplit> getInputSplits(CarbonTableCacheModel tableCacheModel,
    ...
    if (CarbonProperties.getInstance().isCoarseGrainSecondaryIndex(tableInfo.getDatabaseName(),
      ...
      CarbonInputFormat.checkAndSetSecondaryIndexPruning(carbonTable.getTableInfo(), filters,
          config);
    }
  ...
  private void populateCarbonProperties() {
    ...
    addProperty(CarbonCommonConstants.CARBON_WRITTEN_BY_APPNAME, "Trino_Server");
  }
  ...
  public Configuration updateS3Properties(Configuration configuration) {
    ...
    configuration.set(CarbonCommonConstants.S3_ACCESS_KEY,
        Objects.toString(config.getS3_AcesssKey(), ""));
    configuration.set(CarbonCommonConstants.S3_SECRET_KEY,
        Objects.toString(config.getS3_SecretKey()));
    configuration.set(CarbonCommonConstants.S3N_ACCESS_KEY,
        Objects.toString(config.getS3N_AcesssKey(), ""));
    configuration.set(CarbonCommonConstants.S3N_SECRET_KEY,
