...
import org.apache.carbondata.trino.CarbonTrinoDecodeReadSupport;
import org.apache.carbondata.trino.CarbonVectorBatch;
import org.apache.carbondata.trino.TrinoCarbonVectorizedRecordReader;
import org.apache.carbondata.trino.TrinoFilterUtil;
import org.apache.carbondata.trino.Types;
import org.apache.carbondata.trino.readers.TrinoVectorBlockBuilder;
import org.apache.carbondata.trino.split.CarbonLocalMultiBlockSplit;
...
class CarbonDataPageSource implements ConnectorPageSource {
  ...
  private final boolean isDirectVectorFill;
  ...
  private CarbonTrinoDecodeReadSupport readSupport;
  // columnar format split
  private TrinoCarbonVectorizedRecordReader vectorReader;
  // row format split
  ...
  private void initialize() {
    CarbonMultiBlockSplit carbonInputSplit =
        CarbonLocalMultiBlockSplit.convertSplit(split.getSchema().getProperty("carbonSplit"));
  ...
  private void initializeForColumnar() {
    readSupport = new CarbonTrinoDecodeReadSupport();
    vectorReader =
  ...
  private Page getNextPageForRow() {
    ...
    try {
      ...
      if (count == 0) {
        ...
        for (int index = 0; index < columnCount; index++) {
          blocks[index] = ((TrinoVectorBlockBuilder) columns[index]).buildBlock();
  ...
  @Override
  public void close() {
    // some hive input formats are broken and bad things can happen if you
   ...
   */
  private TrinoCarbonVectorizedRecordReader createReaderForColumnar(HiveSplit carbonSplit,
      List<? extends ColumnHandle> columns, ConnectorTableHandle tableHandle,
      CarbonTrinoDecodeReadSupport readSupport, Configuration conf) {
    ...
    try {
      ...
      readSupport.initialize(queryModel.getProjectionColumns(), queryModel.getTable());
      TrinoCarbonVectorizedRecordReader reader =
          new TrinoCarbonVectorizedRecordReader(queryExecutor, queryModel,
  ...
  private QueryModel createQueryModel(HiveSplit carbondataSplit, ConnectorTableHandle tableHandle,
    ...
    try {
      ...
      CarbonTableInputFormat.setTransactionalTable(conf,
      ...
      CarbonTableInputFormat carbonTableInputFormat = createInputFormat(jobConf, carbonTable,
          new IndexFilter(carbonTable,
              TrinoFilterUtil.parseFilterExpression(hiveTable.getCompactEffectivePredicate())),
          carbonProjection);
      TaskAttemptContextImpl hadoopAttemptContext =
          new TaskAttemptContextImpl(jobConf, new TaskAttemptID("", 1, TaskType.MAP, 0, 0));
      CarbonMultiBlockSplit carbonInputSplit = CarbonLocalMultiBlockSplit.convertSplit(
          carbondataSplit.getSchema().getProperty("carbonSplit"));
  ...
  private CarbonTableInputFormat<Object> createInputFormat(Configuration conf,
    ...
    try {
      ...
      CarbonTableInputFormat.setDatabaseName(conf,
          identifier.getCarbonTableIdentifier().getDatabaseName());
      CarbonTableInputFormat.setTableName(conf,
  ...
  private final class CarbondataBlockLoader implements LazyBlockLoader {
    ...
    CarbondataBlockLoader(int columnIndex, int batchSize) {
    ...
    public Block load() {
      ...
      try {
        ...
        TrinoVectorBlockBuilder blockBuilder =
            (TrinoVectorBlockBuilder) vectorReader.getColumnarBatch().column(columnIndex);
        blockBuilder.setBatchSize(batchSize);
