  ...
  protected lazy val deleteRecords: Parser[LogicalPlan] =
    (DELETE ~> FROM ~> table) ~ restInput.? <~ opt(";") ^^ {
      case table ~ rest =>
        ...
        DeleteRecords("select tupleId from " + tableName + " " +
                      SubqueryAlias(table.tableName, UnresolvedRelation(table.tableIdentifier)) +
          ...
          if (!sel.toLowerCase.startsWith("select ")) {
            ...
            val relation = tab match {
              case r@UnresolvedRelation(tableIdentifier) =>
                updateRelation(r,
                  tableIdentifier,
                  Some(SubqueryAlias(tab.tableName, r).alias))
              case _ => tab
            }
            ("select " + sel + " from " + getTableName(relation.tableIdentifier) + " " +
             Some(SubqueryAlias(relation.tableName, UnresolvedRelation(relation.tableIdentifier))
               .alias), relation)
          } else {
            (sel, updateRelation(tab,
              tab.tableIdentifier,
              Some(SubqueryAlias(tab.tableName, UnresolvedRelation(tab.tableIdentifier)).alias)))
          }
