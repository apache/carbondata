...
public class FileFooterValidator {
  ...
  public void setUp() throws Exception {
    ...
    CarbonHiveContext
        .sql("CREATE CUBE validatefooter DIMENSIONS (empno Integer, empname String, designation String, doj Timestamp, workgroupcategory Integer, workgroupcategoryname String, deptno Integer, deptname String, projectcode Integer, projectjoindate Timestamp, projectenddate Timestamp) MEASURES (attendance Integer,utilization Integer,salary Integer) OPTIONS (PARTITIONER [PARTITION_COUNT=1])");
    CarbonHiveContext
        .sql("LOAD DATA fact from './src/test/resources/data.csv' INTO CUBE validatefooter PARTITIONDATA(DELIMITER ',', QUOTECHAR '\"')");
    String storePath =
        CarbonProperties.getInstance().getProperty(CarbonCommonConstants.STORE_LOCATION);
    CarbonTableIdentifier tableIdentifier = new CarbonTableIdentifier("default", "validatefooter");
    String segmentPath =
        CarbonStorePath.getCarbonTablePath(storePath, tableIdentifier).getCarbonDataDirectoryPath(
          "0", 0);
    ...
    for (CarbonFile file : list) {
      ...
      testDataFileFooter(fileLocation, offset);
    }
    setUpIsDone = true;
  }

  private static void testDataFileFooter(String fileLocation, long offset) throws IOException {
    DataFileFooterConverter converter = new DataFileFooterConverter();
    converter.readDataFileFooter(fileLocation, offset);
  }
  ...
  @Test
  public void testFileFooterExist() {
    assertTrue(fileFooter != null);
  }

  @Test
  public void testFileFooterVersion() {
    assertTrue(fileFooter.getVersion() >= 0);
  }

  @Test
  public void testFileFooterNumRows() {
    assertTrue(fileFooter.getNum_rows() > 0);
  }

  @Test
  public void testFileFooterTableColumns() {
    assertTrue(fileFooter.getTable_columns() != null && fileFooter.getTable_columns().size() > 0);
  }

  @Test
  public void testFileFooterSegmentInfo() {
    assertTrue(fileFooter.getSegment_info() != null
        && fileFooter.getSegment_info().getNum_cols() > 0
        && fileFooter.getSegment_info().getColumn_cardinalities().size() > 0);
  }

  @Test
  public void testFileFooterBlockletIndex() {
  ...
  @Test
  public void testFileFooterBlockletInfo() {
  ...
  private void testColumnDataChunk(DataChunk columnDatachunk) {
    ...
    if (columnDatachunk.getEncoders().contains(Encoding.DELTA)) {
      assertTrue(columnDatachunk.getPresence() != null && columnDatachunk.getEncoder_meta() != null);
    } else {
