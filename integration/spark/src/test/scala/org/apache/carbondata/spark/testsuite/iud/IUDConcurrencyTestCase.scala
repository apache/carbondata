...
class IUDConcurrencyTestCase extends QueryTest with BeforeAndAfterAll {
  ...
  test("Second update should success when segments are not updated by first update") {
    ...
    val mockUpdate = new MockUp[CarbonProjectForUpdateCommand]() {
      ...
      def mockForConcurrentUpdateTest(): Unit = {
        this.tearDown()
        val secondUpdateFuture = runSqlAsync(secondUpdateSql)
        assert(secondUpdateFuture.get().contains("PASS"))
      }
    }
  ...
  // 1st update -> 2nd update -> 1st update
  // segments updated by two update operations have intersection
  test("First update should fail when segments are updated by second update") {
    val firstUpdateSql = "update orders set (o_country)=('fifthCountry')" +
      " where o_country='thirdCountry'"
    val secondUpdateSql =
      "update orders set (o_country)=('fifthCountry') where o_country='thirdCountry'"
    ...
    val firstUpdateException = intercept[ConcurrentOperationException] {
      sql(firstUpdateSql)
    }
    assert(firstUpdateException.getMessage.contains(
      "Update operation failed because there is a concurrent " +
        "operation conflict. please try later."))

    checkAnswer(
      sql("select count(1) from orders o_country='thirdCountry'"), Seq(Row(0)))
    checkAnswer(
      sql("select count(1) from orders o_country='fourthCountry'"), Seq(Row(ONE_LOAD_SIZE * 2)))
    checkAnswer(
      sql("select count(1) from orders where o_country='fifthCountry'"), Seq(Row(ONE_LOAD_SIZE)))
  }
  ...
  test("Second delete should success when segments are not deleted by first delete") {
    ...
    sql("insert into temp_table select 3, 'ccc', 'xxxxx'")
    // segment 0:  1,2,3
    sql("insert into table_1 select * from temp_table")

    sql("delete from temp_table where c1 in (1,2,3)")
    sql("insert into temp_table select 4, 'aaa', 'xxxxx'")
    sql("insert into temp_table select 5, 'aaa', 'xxxxx'")
    sql("insert into temp_table select 6, 'aaa', 'xxxxx'")
    // segment 1:  4,5,6
    sql("insert into table_1 select * from temp_table")

    val firstDeleteSql = "delete from table_1 where c1=1"
    val secondDeleteSql = "delete from table_1 where c1=4"

    val mockDelete = new MockUp[CarbonProjectForDeleteCommand]() {
      @Mock
      def mockForConcurrentDeleteTest(): Unit = {
        this.tearDown()
        val secondDeleteFuture = runSqlAsync(secondDeleteSql)
        assert(secondDeleteFuture.get().contains("PASS"))
      }
    }
    ...
    checkAnswer(sql("select count(1) from table_1 where c1=2"), Seq(Row(1)))
    checkAnswer(sql("select count(1) from table_1 where c1=3"), Seq(Row(1)))
    checkAnswer(sql("select count(1) from table_1 where c1=4"), Seq(Row(0)))
    checkAnswer(sql("select count(1) from table_1 where c1=5"), Seq(Row(1)))
    checkAnswer(sql("select count(1) from table_1 where c1=6"), Seq(Row(1)))
  }
  ...
  test("Second delete should fail when segments are deleted by first delete") {
    ...
    sql("create table temp_table (c1 int, c2 String, c3 String) stored as carbondata")
    sql("create table table_2 (c1 int, c2 String, c3 String) stored as carbondata")
    sql("insert into temp_table select 1, 'aaa', 'xxxx'")
    sql("insert into temp_table select 2, 'bbb', 'xxxx'")
    sql("insert into temp_table select 20, 'bbb', 'xxxx'")
    // segment 0:  1,2,20
    sql("insert into table_2 select * from temp_table")
    sql("insert into temp_1 select 3, 'ccc', 'xxxx'")
    // segment 1:  1,2,3,20
    sql("insert into table_2 select * from temp_table")

    val firstDeleteSql = "delete from table_2 where c1=1"
    val secondDeleteSql = "delete from table_2 where c1=2"
    ...
    val firstDeleteException = intercept[ConcurrentOperationException] {
      sql(firstDeleteSql)
    }
    assert(firstDeleteException.getMessage.contains(
      "Delete operation failed because there is a concurrent " +
        "operation conflict. please try later."))

    checkAnswer(sql("select count(1) from table_1 where c1=1"), Seq(Row(2)))
