...
/**
 * test functionality for alter table with index
 */
class TestRenameTableWithIndex extends QueryTest with BeforeAndAfterAll {
  ...
  test("Creating a bloomfilter index, then table rename") {
    sql(
      s"""
  ...
  /*
   * mv does not support running here, now must run in mv project.
  test("Creating a mv,then table rename") {
    sql(
      """
    ...
    assert(exception_tb_rename.getMessage
      .contains("alter rename is not supported for mv"))
  } */
