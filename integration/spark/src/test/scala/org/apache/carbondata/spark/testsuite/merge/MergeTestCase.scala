...
//      whenMatched((col("A.address") =!= col("B.address")).and(col("A.current")
//        .equalTo(lit(true)))).
//      updateExpr(updateMap).
...
//    cdc.createOrReplaceTempView("changes")
//
//    cdc = sql("SELECT key, latest.newValue as newValue, latest.deleted as deleted
  //    FROM ( SELECT " +
//              "key, max(struct(time, newValue, deleted)) as latest FROM changes GROUP BY key)")
//
//    target.as("A").merge(cdc.as("B"), "A.key=B.key").
...
//      insertExpr(insertMap).execute()
//    val sdf = new SimpleDateFormat("yyyy-MM-dd")
//    // in case of cdc, the insert into flow goes to no converter step to save time as
//    //    incoming data
...
//      Seq(
//        Row(new Date(sdf.parse("2020-07-01").getTime), Timestamp
//          .valueOf("2020-04-04 09:40:05.205"))
//      ))
//  }
