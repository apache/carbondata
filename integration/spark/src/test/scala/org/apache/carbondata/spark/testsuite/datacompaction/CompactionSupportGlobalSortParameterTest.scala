...
class CompactionSupportGlobalSortParameterTest
  ...
  test("MINOR, ENABLE_AUTO_LOAD_MERGE: true") {
    ...
    // auto merge will compact and produce 1 segment
    // because 6 is bigger than 4 (default value of minor),
  ...
  test("MINOR, PRESERVE_LATEST_SEGMENTS_NUMBER: 4") {
    CarbonProperties.getInstance()
      .addProperty(CarbonCommonConstants.PRESERVE_LATEST_SEGMENTS_NUMBER, "4")
    for (i <- 0 until 2) {
      sql(s"LOAD DATA LOCAL INPATH '$file1' INTO TABLE carbon_localsort")
      sql(s"LOAD DATA LOCAL INPATH '$file2' INTO TABLE carbon_localsort")
      sql(s"LOAD DATA LOCAL INPATH '$file3' INTO TABLE carbon_localsort")
  ...
  test("MAJOR, ENABLE_PREFETCH_DURING_COMPACTION: true") {
    CarbonProperties.getInstance()
      .addProperty(CarbonCommonConstants.CARBON_COMPACTION_PREFETCH_ENABLE, "true")
      .addProperty(CarbonCommonConstants.ENABLE_AUTO_LOAD_MERGE, "false")
    for (i <- 0 until 2) {
      sql(s"LOAD DATA LOCAL INPATH '$file1' INTO TABLE carbon_localsort")
      sql(s"LOAD DATA LOCAL INPATH '$file2' INTO TABLE carbon_localsort")
    ...
    CarbonProperties.getInstance()
      .addProperty(CarbonCommonConstants.ENABLE_AUTO_LOAD_MERGE,
        CarbonCommonConstants.DEFAULT_ENABLE_AUTO_LOAD_MERGE)
      .addProperty(CarbonCommonConstants.CARBON_COMPACTION_PREFETCH_ENABLE,
