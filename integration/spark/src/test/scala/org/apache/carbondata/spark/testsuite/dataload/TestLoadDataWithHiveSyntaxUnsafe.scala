...
class TestLoadDataWithHiveSyntaxUnsafe extends QueryTest with BeforeAndAfterAll {
    ...
    try {
      ...
      case e: Throwable =>
        assert(e.getMessage.equals(
          "Overwrite is not supported for carbon table with default.carbontable"))
    }
  }
  ...
  test("array<string> and string datatype for same column is not working properly") {
    sql("drop table if exists complexcarbontable")
    sql(
      "create table complexcarbontable(deviceInformationId int, MAC array<string>, channelsId " +
      "string, " +
      "ROMSize string, purchasedate string, gamePointId double,contractNumber double) STORED AS " +
      "carbondata ")
    sql(
      s"LOAD DATA local inpath '$resourcesPath/complexdatareordered.csv' INTO table " +
      s"complexcarbontable " +
      "OPTIONS('DELIMITER'=',', 'QUOTECHAR'='\"', 'FILEHEADER'='deviceInformationId,MAC," +
      "channelsId,ROMSize,purchasedate,gamePointId,contractNumber'," +
      "'COMPLEX_DELIMITER_LEVEL_1'='$', 'COMPLEX_DELIMITER_LEVEL_2'=':')")
    sql("drop table if exists complexcarbontable")
    sql(
      "create table primitivecarbontable(deviceInformationId int, MAC string, channelsId string, " +
      "ROMSize string, purchasedate string, gamePointId double,contractNumber double) STORED AS " +
      "carbondata ")
    sql(
      s"LOAD DATA local inpath '$resourcesPath/complexdatareordered.csv' INTO table " +
      s"primitivecarbontable " +
      "OPTIONS('DELIMITER'=',', 'QUOTECHAR'='\"', 'FILEHEADER'='deviceInformationId,MAC," +
      "channelsId,ROMSize,purchasedate,gamePointId,contractNumber'," +
      "'COMPLEX_DELIMITER_LEVEL_1'='$', 'COMPLEX_DELIMITER_LEVEL_2'=':')")
    sql("drop table if exists primitivecarbontable")
  ...
  test("test carbon table data loading with special character 2") {
    ...
    checkAnswer(sql("select 124_string_level_province from specialcharacter2 where " +
                    "customer_id=103"),
  ...
  test("test data loading with comment option") {
    ...
    sql(
      ...
      "options('DELIMITER' = ',', 'QUOTECHAR' = '.', 'COMMENTCHAR' = '?','FILEHEADER'='imei,age," +
      "task,num,level,productdate,mark,name', 'maxcolumns'='180')"
    )
    checkAnswer(sql("select imei from comment_test"),
      Seq(Row("\".carbon"), Row("#?carbon"), Row(""),
        Row("~carbon,")))
  }

  test("test decimal var lenght comlumn page") {
    sql(
      s"LOAD DATA LOCAL INPATH '$resourcesPath/big_decimal_without_header.csv' INTO TABLE " +
      s"decimal_varlength" +
      s" OPTIONS('FILEHEADER'='id,value')")
    sql(s"LOAD DATA LOCAL INPATH '$resourcesPath/big_decimal_without_header.csv' INTO TABLE " +
        s"decimal_varlength_hive")
    checkAnswer(sql("select value from decimal_varlength"),
      sql("select value from decimal_varlength_hive"))
    checkAnswer(sql("select sum(value) from decimal_varlength"),
      sql("select sum(value) from decimal_varlength_hive"))
  }
