...
class SparkCarbonDataSourceTest extends QueryTest with BeforeAndAfterAll {
  ...
  test("test write using ddl") {
    ...
    if (!sqlContext.sparkContext.version.startsWith("2.1")) {
      ...
      DataMapStoreManager.getInstance()
        .clearIndexes(AbsoluteTableIdentifier.from(warehouse + "/carbon_table"))
  ...
  test("test write using subfolder") {
    if (!sqlContext.sparkContext.version.startsWith("2.1")) {
      ...
      val mapSize = DataMapStoreManager.getInstance().getIndexes.size()
      DataMapStoreManager.getInstance()
        .clearIndexes(AbsoluteTableIdentifier.from(warehouse + "/test_folder"))
      assert(mapSize > DataMapStoreManager.getInstance().getIndexes.size())
      FileFactory.deleteAllCarbonFilesOfDir(FileFactory.getCarbonFile(warehouse + "/test_folder"))
    }
  }
  ...
  test("test clearing datamaps") {
    ...
    if (!sqlContext.sparkContext.version.startsWith("2.1")) {
      ...
      assert(DataMapStoreManager.getInstance().getIndexes.size() == 1)
      DataMapStoreManager.getInstance()
        .clearIndexes(AbsoluteTableIdentifier.from(warehouse + "/carbon_table1"))
