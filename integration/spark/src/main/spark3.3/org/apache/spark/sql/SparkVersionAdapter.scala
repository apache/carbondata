...
import org.apache.spark.sql.catalyst.parser.SqlBaseParser.{BucketSpecContext, ColTypeListContext, CreateTableHeaderContext, LocationSpecContext, PartitionFieldListContext, PropertyListContext, QueryContext, SkewSpecContext}
...
trait SparkVersionAdapter {
  ...
  def getUpdatedPlan(plan: LogicalPlan, sqlText: String): LogicalPlan = {
    plan match {
      case create@CreateTable(_, _, _, _, _) =>
        if ( create.tableSpec.location.isDefined &&
          ...
          create.tableSpec.properties. +("hasexternalkeyword" -> "false")
          CreateTable(create.name, create.tableSchema, create.partitioning, create.tableSpec,
            create.ignoreIfExists)
        } else if (create.tableSpec.options.contains("latestversion")) {
          // remove latestversion property in options if present
          create.tableSpec.options.-("latestversion")
          CreateTable(create.name, create.tableSchema, create.partitioning, create.tableSpec,
            create.ignoreIfExists)
        } else {
          create
  ...
  def showPartitionsCommand(spec: Option[TablePartitionSpec],
    ...
    ShowPartitionsCommand(showPartitionsCommand.tableName, showPartitionsCommand.output, spec)
  }
...
abstract class CarbonTakeOrderedAndProjectExecHelper(sortOrder: Seq[SortOrder],
  ...
  override protected def withNewChildInternal(newChild: SparkPlan): SparkPlan = newChild
}

trait CarbonMergeIntoSQLCommandCarbon extends AtomicRunnableCommand {
  override protected def withNewChildrenInternal(
    newChildren: IndexedSeq[LogicalPlan]): LogicalPlan = newChildren.head
}
...
trait CarbonCustomDeterministicExpression extends Expression {
  override protected def withNewChildrenInternal(newChildren:
    IndexedSeq[Expression]): Expression = newChildren.head
}

abstract class OutputWriterCarbon(paths: String) extends OutputWriter {
  override def path(): String = paths
}

trait CarbonCommands extends MetadataCommand {
  override protected def withNewChildrenInternal(newChildren:
    IndexedSeq[LogicalPlan]): LogicalPlan = newChildren.head
}
...
trait CarbonRunnableCommand extends RunnableCommand {
  override protected def withNewChildrenInternal(newChildren:
    IndexedSeq[LogicalPlan]): LogicalPlan = newChildren.head
}
...
trait CarbonBinaryExecNode extends BinaryExecNode {
  override protected def withNewChildrenInternal(newLeft: SparkPlan,
    newRight: SparkPlan): SparkPlan = newLeft
}
