...
object CarbonToSparkAdapter {
  ...
  def transformDown(plan: ModularPlan,
    ...
    val func: ModularPlan => ModularPlan = {
      a => a
    }
    val afterRule = CurrentOrigin.withOrigin(CurrentOrigin.get) {
      rule.applyOrElse(plan, func)
    }

    // Check if unchanged and then possibly return old copy to avoid gc churn.
    if (plan fastEquals afterRule) {
      mapChildren(plan, transformDown(_, rule))
    } else {
      // If the transform function replaces this node with a new one, carry over the tags.
      afterRule.copyTagsFrom(plan)
      afterRule.mapChildren(_.transformDown(rule))
    }
  }

  def mapChildren(plan: ModularPlan, f: ModularPlan => ModularPlan): ModularPlan = {
    val containsChild: Set[TreeNode[_]] = plan.children.toSet
    if (containsChild.nonEmpty) {
      mapChildren(f, forceCopy = false, containsChild, plan)
    } else {
      plan
    }
  }
  ...
  private def mapChildren(
      f: ModularPlan => ModularPlan,
      forceCopy: Boolean,
      containsChild: Set[TreeNode[_]],
      plan: ModularPlan): ModularPlan = {
    var changed = false

    def mapChild(child: Any): Any = {
      child match {
        case arg: TreeNode[_] if containsChild(arg) =>
          val newChild = f(arg.asInstanceOf[ModularPlan])
        ...
        case tuple@(arg1: TreeNode[_], arg2: TreeNode[_]) =>
          val newChild1 = if (containsChild(arg1)) {
            f(arg1.asInstanceOf[ModularPlan])
          } else {
            arg1.asInstanceOf[ModularPlan]
          }

          val newChild2 = if (containsChild(arg2)) {
            f(arg2.asInstanceOf[ModularPlan])
          } else {
            arg2.asInstanceOf[ModularPlan]
          }
    ...
    val newArgs = plan match {
      case select: Select =>
        select.mapProductIterator {
          case arg: TreeNode[_] if containsChild(arg) =>
            val newChild = f(arg.asInstanceOf[ModularPlan])
            if (forceCopy || !(newChild fastEquals arg)) {
              changed = true
          ...
          // `map.mapValues().view.force` return `Map` in Scala 2.12 but return `IndexedSeq` in
          // Scala
    ...
    if (forceCopy || changed) plan.makeCopy(newArgs) else plan
  }
