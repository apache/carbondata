...
public final class CarbonQueryUtil {
   ...
   */
  public static synchronized TableSplit[] getTableSplits(String schemaName, String tableName,
    ...
    if (queryPlan == null) {
      allPartitions =
          QueryPartitionHelper.getInstance().getAllPartitions(schemaName, tableName, partitioner);
    } else {
      allPartitions =
    ...
    for (int i = 0; i < splits.length; i++) {
      ...
      String location = QueryPartitionHelper.getInstance()
          .getLocation(partition, schemaName, tableName, partitioner);
