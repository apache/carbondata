...
public final class MolapLoaderUtil {
    ...
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(MolapLoaderUtil.class.getName());
    ...
    public static void executeGraph(MolapLoadModel loadModel, String storeLocation,
        ...
        MolapProperties.getInstance()
                .addProperty(MolapCommonConstants.STORE_LOCATION_HDFS, hdfsStoreLocation);
        ...
        graphExecuter.executeGraph(graphPath,
                new ArrayList<String>(MolapCommonConstants.CONSTANT_SIZE_TEN), info,
    ...
    public static String[] getStorelocs(String schemaName, String cubeName, String factTableName,
        ...
        MolapFile[] listFiles = file.listFiles(new MolapFileFilter() {
            ...
            public boolean accept(MolapFile path) {
                return path.getName().startsWith(MolapCommonConstants.LOAD_FOLDER) && !path
    ...
    public static void deletePartialLoadDataIfExist(int partitionCount, String schemaName,
        ...
        for (int i = 0; i < partitionCount; i++) {
            ...
            for (int j = 0; j < loadFolder; j++) {
                loadFolders.add((tableLoc + File.separator + MolapCommonConstants.LOAD_FOLDER + j)
                        .replace("\\", "/"));
            }
            if (loadFolder != 0) {
                ...
                MolapFile[] listFiles = molapFile.listFiles(new MolapFileFilter() {
                    ...
                    public boolean accept(MolapFile path) {
                        return !loadFolders.contains(path.getAbsolutePath().replace("\\", "/"))
                                && !path.getName().contains(MolapCommonConstants.MERGERD_EXTENSION);
                    }
                });
    ...
    public static boolean isSliceValid(String loc, List<String> activeSlices,
        ...
        String loadFolderName = loc.substring(loc.indexOf(MolapCommonConstants.LOAD_FOLDER));
        String sliceNum = loadFolderName.substring(MolapCommonConstants.LOAD_FOLDER.length());
    ...
    public static List<String> getListOfValidSlices(LoadMetadataDetails[] details) {
        List<String> activeSlices = new ArrayList<>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        for (LoadMetadataDetails oneLoad : details) {
            if (MolapCommonConstants.STORE_LOADSTATUS_SUCCESS.equals(oneLoad.getLoadStatus())
                    || MolapCommonConstants.STORE_LOADSTATUS_PARTIAL_SUCCESS
                    .equals(oneLoad.getLoadStatus()) || MolapCommonConstants.MARKED_FOR_UPDATE
                    .equals(oneLoad.getLoadStatus())) {
                if (null != oneLoad.getMergedLoadName()) {
                    String loadName =
                            MolapCommonConstants.LOAD_FOLDER + oneLoad.getMergedLoadName();
    ...
    public static List<String> getListOfUpdatedSlices(LoadMetadataDetails[] details) {
        List<String> updatedSlices = new ArrayList<>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        for (LoadMetadataDetails oneLoad : details) {
            if (MolapCommonConstants.MARKED_FOR_UPDATE.equals(oneLoad.getLoadStatus())) {
                if (null != oneLoad.getMergedLoadName()) {
    ...
    public static void removeSliceFromMemory(String schemaName, String cubeName, String loadName) {
        List<InMemoryCube> activeSlices =
                InMemoryCubeStore.getInstance().getActiveSlices(schemaName + '_' + cubeName);
    ...
    public static void createEmptyLoadFolder(MolapLoadModel model, String factLoadFolderLocation,
            String hdfsStoreLocation, int currentRestructNumber) {
        String loadFolderName = factLoadFolderLocation
                .substring(factLoadFolderLocation.indexOf(MolapCommonConstants.LOAD_FOLDER));
    ...
    public static void copyCurrentLoadToHDFS(MolapLoadModel loadModel, int currentRestructNumber,
            String loadName, List<String> updatedSlices, int sliceRestructNumber)
            throws IOException, MolapUtilException {
        //Copy the current load folder to HDFS
        boolean copyStore = Boolean.valueOf(
                MolapProperties.getInstance().getProperty("dataload.hdfs.copy", "true"));
        ...
        if (copyStore) {
            String hdfsLocation = MolapProperties.getInstance()
            ...
            MolapUtil.deleteFoldersAndFiles(new File[] { new File(
                    localStore + File.separator + schemaName + File.separator + cubeName) });
        }
    }

    public static void generateGlobalSurrogates(MolapLoadModel loadModel, String storeLocation,
    ...
    public static void copyToHDFS(String loadName, String schemaName, String cubeName,
        ...
        if (hdfsLocation != null && !hdfsLocation.equals(localStore)) {
            ...
            String localLoadedTable =
                    currentloadedStore + File.separator + MolapCommonConstants.RESTRUCTRE_FOLDER
            ...
            String localLoadFolder =
                    localLoadedTable + File.separator + MolapCommonConstants.LOAD_FOLDER
                            + loadCounter;
        ...
        try {
            if (FileFactory.isFileExist(location, fileType)) {
                ...
                MolapFile[] listFiles = molapFile.listFiles(new MolapFileFilter() {
                    @Override
                    public boolean accept(MolapFile path) {
                        return path.getName().endsWith(MolapCommonConstants.FACT_FILE_EXT) && !path
                                .getName().endsWith(MolapCommonConstants.FILE_INPROGRESS_STATUS);
                    }
                });
                for (int i = 0; i < listFiles.length; i++) {
                    ...
                    String changedFileName = factFilePath
                            .replace(MolapCommonConstants.FACT_FILE_EXT,
    ...
    public static int getLoadCount(MolapLoadModel loadModel, int currentRestructNumber)
        ...
        int loadCounter = MolapUtil.checkAndReturnCurrentLoadFolderNumber(hdfsLoadedTable);

        String hdfsStoreLoadFolder =
                hdfsLoadedTable + File.separator + MolapCommonConstants.LOAD_FOLDER + loadCounter;
        hdfsStoreLoadFolder = hdfsStoreLoadFolder.replace("\\", "/");
     ...
     * in the metadata.
     */
    public static String getLoadFolderPath(MolapLoadModel loadModel, String cubeName,
    ...
    public static void recordLoadMetadata(int loadCount, LoadMetadataDetails loadMetadataDetails,
            MolapLoadModel loadModel, String loadStatus, String startLoadTime) throws IOException {
    ...
    public static String extractLoadMetadataFileLocation(Schema schema, String schemaName,
        ...
        if (null == cube) {
            ...
            MolapDef.Cube mondrianCube = MolapSchemaParser.getMondrianCube(schema, cubeName);
            MolapMetadata.getInstance()
    ...
    public static CubeDimension[][] getDimensionSplit(Schema schema, String cubeName,
        ...
        List<CubeDimension> selectedDims =
                new ArrayList<CubeDimension>(MolapCommonConstants.CONSTANT_SIZE_TEN);
        for (int i = 0; i < allDims.length; i++) {
            Level extractHierarchies =
                    MolapSchemaParser.extractHierarchies(schema, allDims[i])[0].levels[0];
    ...
    public static String extractLoadMetadataFileLocation(MolapLoadModel loadModel) {
        Cube cube = MolapMetadata.getInstance()
                .getCube(loadModel.getSchemaName() + '_' + loadModel.getCubeName());
        if (null == cube) {
            Schema schema = loadModel.getSchema();
            MolapDef.Cube mondrianCube =
                    MolapSchemaParser.getMondrianCube(schema, loadModel.getCubeName());
            MolapMetadata.getInstance()
                    .loadCube(schema, schema.name, mondrianCube.name, mondrianCube);
            cube = MolapMetadata.getInstance()
                    .getCube(loadModel.getSchemaName() + '_' + loadModel.getCubeName());
        }
    ...
    public static int getCurrentRestructFolder(String schemaName, String cubeName, Schema schema) {
        ...
        int currentRestructNumber =
                MolapUtil.checkAndReturnCurrentRestructFolderNumber(metaDataPath, "RS_", false);
    ...
    public static Set<String> getColumnListFromAggTable(MolapLoadModel model) {
        Set<String> columnList = new HashSet<String>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        Cube metadataCube = MolapMetadata.getInstance()
        ...
        for (int i = 0; i < aggTables.length; i++) {
            ...
            if (model.getAggTableName().equals(aggTableName)) {
                ...
                for (AggLevel aggDim : aggLevels) {
                    Dimension dim = MolapQueryParseUtil.findDimension(dimensions, aggDim.column);
                    if (null != dim) {
                        columnList.add(dim.getColName());
                    }
                }
                for (AggMeasure aggMsr : aggMeasures) {
                    Dimension dim = MolapQueryParseUtil.findDimension(dimensions, aggMsr.column);
                    if (null != dim) {
                        columnList.add(dim.getColName());
                    }
                }
    ...
    public static void copyMergedLoadToHDFS(MolapLoadModel loadModel, int currentRestructNumber,
    ...
    public static void copyMergeToHDFS(String schemaName, String cubeName, String factTable,
        ...
        try {
            ...
            if (hdfsLocation != null && !hdfsLocation.equals(localStore)) {
                ...
                String localLoadName = MolapCommonConstants.LOAD_FOLDER + mergedLoadName;
                String localLoadFolder =
                        localLoadedTable + File.separator + MolapCommonConstants.LOAD_FOLDER
                                + mergedLoadName;
