...
import org.carbondata.processing.merger.MolapColumnarSliceMerger;
import org.carbondata.processing.merger.MolapSliceMergerInfo;
...
public final class MolapDataMergerUtil {
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(MolapDataMergerUtil.class.getName());

    private MolapDataMergerUtil() {

    }

    public static boolean executeMerging(MolapLoadModel molapLoadModel, String storeLocation,
        ...
        MolapProperties.getInstance()
                .addProperty(MolapCommonConstants.STORE_LOCATION, storeLocation);
        MolapProperties.getInstance()
    ...
    public static List<String> getLoadsToMergeFromHDFS(String storeLocation, FileType fileType,
            String metadataPath, MolapLoadModel molapLoadModel, int currentRestructNumber,
            int partitionCount) {
        ...
        try {
            toLoadMergeMaxSize = Integer.parseInt(MolapProperties.getInstance()
                    .getProperty(MolapCommonConstants.TO_LOAD_MERGE_MAX_SIZE,
                            MolapCommonConstants.TO_LOAD_MERGE_MAX_SIZE_DEFAULT));
        } catch (NumberFormatException e) {
            toLoadMergeMaxSize =
                    Integer.parseInt(MolapCommonConstants.TO_LOAD_MERGE_MAX_SIZE_DEFAULT);
        }

        LoadMetadataDetails[] loadDetails = MolapUtil.readLoadMetadata(metadataPath);
            ...
            if (loadNames.size() < 2) {
                ...
                if (checkIfLoadIsNotDeleted(loadDetail)) {
                    ...
                    if (checkIfLoadIsMergedAlready(loadDetail)) {
                        if (checkSizeOfloadToMerge(loadDetail, toLoadMergeMaxSize, molapLoadModel,
    ...
    private static boolean checkSizeOfloadToMerge(final LoadMetadataDetails loadDetail,
            int toLoadMergeMaxSize, MolapLoadModel molapLoadModel, int partitionCount,
        ...
        for (int partition = 0; partition < partitionCount; partition++) {
            String loadPath = LoadMetadataUtil
                    .createLoadFolderPath(molapLoadModel, storeLocation, partition,
                            currentRestructNumber);

            MolapFile parentLoadFolder =
                    FileFactory.getMolapFile(loadPath, FileFactory.getFileType(loadPath));
            MolapFile[] loadFiles = parentLoadFolder.listFiles(new MolapFileFilter() {
                @Override
                public boolean accept(MolapFile file) {
                    if (file.getName()
                            .substring(file.getName().indexOf('_') + 1, file.getName().length())
    ...
    public static boolean checkIfLoadMergingRequired(String metadataFilePath,
            MolapLoadModel molapLoadModel, String storeLocation, int partition,
            int currentRestructNumber) {

        String loadPath = LoadMetadataUtil
                .createLoadFolderPath(molapLoadModel, storeLocation, 0, currentRestructNumber);
        ...
        MolapFile[] loadFiles = parentLoadFolder.listFiles( new MolapFileFilter() {
            @Override
            public boolean accept(MolapFile file) {
                if (file.getName().startsWith(MolapCommonConstants.LOAD_FOLDER)) {
                    return true;
                }
                return false;
            }
        ...
        String isLoadMergeEnabled = MolapProperties.getInstance()
                .getProperty(MolapCommonConstants.ENABLE_LOAD_MERGE,
        ...
        int mergeThreshold;
        try {
            mergeThreshold = Integer.parseInt(MolapProperties.getInstance()
                    .getProperty(MolapCommonConstants.MERGE_THRESHOLD_VALUE,
    ...
    private static int getNumberOfValidLoads(LoadMetadataDetails[] details, MolapFile[] loadFiles) {
        ...
        for (LoadMetadataDetails load : details) {
            if (load.getLoadStatus().equalsIgnoreCase(MolapCommonConstants.STORE_LOADSTATUS_SUCCESS)
                    || load.getLoadStatus()
                    .equalsIgnoreCase(MolapCommonConstants.STORE_LOADSTATUS_PARTIAL_SUCCESS) || load
                    .getLoadStatus().equalsIgnoreCase(MolapCommonConstants.MARKED_FOR_UPDATE)) {
    ...
    private static boolean isLoadMetadataPresentInRsFolders(MolapFile[] loadFiles,
            String loadName) {
        for (MolapFile load : loadFiles) {
            String nameOfLoad = load.getName()
                    .substring(load.getName().indexOf(MolapCommonConstants.UNDERSCORE) + 1,
                            load.getName().length());
            if (nameOfLoad.equalsIgnoreCase(loadName)) {
                ...
                MolapFile[] factFiles = load.listFiles( new MolapFileFilter() {
                    @Override
                    public boolean accept(MolapFile file) {
                        if (file.getName().endsWith(MolapCommonConstants.FACT_FILE_EXT) || file
                                .getName().endsWith(MolapCommonConstants.FACT_UPDATE_EXTENSION)) {
                            return true;
                        }
                        return false;
                    }
    ...
    public static String getMergedLoadName(List<String> loadName) {
        ...
        if (mergeLoadName.contains(MolapCommonConstants.MERGERD_EXTENSION)) {
            String loadNum = mergeLoadName
                    .substring(0, mergeLoadName.indexOf(MolapCommonConstants.MERGERD_EXTENSION));
            return loadNum + MolapCommonConstants.MERGERD_EXTENSION
                    + MolapCommonConstants.UNDERSCORE + timeStamp;
        } else {
            return mergeLoadName + MolapCommonConstants.MERGERD_EXTENSION
                    + MolapCommonConstants.UNDERSCORE + timeStamp;
        }
    ...
    public static void updateLoadMetadataWithMergeStatus(List<String> loadsToMerge,
            String metaDataFilepath, String MergedLoadName, MolapLoadModel molapLoadModel) {
        ...
        for (LoadMetadataDetails loadDetail : loadDetails) {
            ...
            if (null != loadDetail.getMergedLoadName()) {
                ...
                if (first) {
                    ...
                    loadDetail.setDeletionTimestamp(MolapLoaderUtil.readCurrentTime());
                }
        ...
        try {
            MolapLoaderUtil
                    .writeLoadMetadata(molapLoadModel.getSchema(), molapLoadModel.getSchemaName(),
                            molapLoadModel.getCubeName(), Arrays.asList(loadDetails));
        } catch (IOException e) {
            LOGGER.error(MolapCoreLogEvent.UNIBI_MOLAPCORE_MSG,
    ...
    public static void cleanUnwantedMergeLoadFolder(MolapLoadModel loadModel, int partitionCount,
            String storeLocation, boolean isForceDelete, int currentRestructNumber) {

        String loadMetadataFilePath = MolapLoaderUtil.extractLoadMetadataFileLocation(loadModel);

        LoadMetadataDetails[] details = MolapUtil.readLoadMetadata(loadMetadataFilePath);
        ...
        for (int partitionId = 0; partitionId < partitionCount; partitionId++) {
            ...
            MolapFile loadFolder = FileFactory.getMolapFile(path, FileFactory.getFileType(path));

            MolapFile[] loads = loadFolder.listFiles(new MolapFileFilter() {
                @Override
                public boolean accept(MolapFile file) {
                    if (file.getName().startsWith(MolapCommonConstants.LOAD_FOLDER) && file
                            .getName().contains(MolapCommonConstants.MERGER_FOLDER_EXT)) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    private static boolean checkIfOldMergeLoadCanBeDeleted(MolapFile eachMergeLoadFolder,
            LoadMetadataDetails[] details) {
        boolean found = false;
        for (LoadMetadataDetails loadDetail : details) {
            if (null != loadDetail.getMergedLoadName() && (MolapCommonConstants.LOAD_FOLDER
                    + loadDetail.getMergedLoadName())
                    .equalsIgnoreCase(eachMergeLoadFolder.getName())) {
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            ...
            long loadTime = Long.parseLong(
                    loadName.substring(loadName.lastIndexOf(MolapCommonConstants.UNDERSCORE) + 1,
                            loadName.length()));
