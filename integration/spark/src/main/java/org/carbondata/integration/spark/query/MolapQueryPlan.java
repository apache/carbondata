...
public class MolapQueryPlan implements Serializable {
     ...
     */
    private List<MolapDimension> dimensions =
            new ArrayList<MolapDimension>(MolapCommonConstants.CONSTANT_SIZE_TEN);
     ...
     * If there is no dimensions asked in query then it would be remained as empty.
     */
    private List<MolapMeasure> measures =
            new ArrayList<MolapMeasure>(MolapCommonConstants.CONSTANT_SIZE_TEN);

    /**
     * List of expressions. Sum(m1+10), Sum(m1)
     */
    private List<MolapQueryExpression> expressions =
            new ArrayList<MolapQueryExpression>(MolapCommonConstants.CONSTANT_SIZE_TEN);

    /**
     * Map of dimension and corresponding dimension filter.
     */
    private Map<MolapDimension, MolapDimensionFilter> dimensionFilters =
            new HashMap<MolapDimension, MolapDimensionFilter>(
                    MolapCommonConstants.DEFAULT_COLLECTION_SIZE);

    private Map<MolapDimension, List<MolapLikeFilter>> dimensionLikeFilters =
            new HashMap<MolapDimension, List<MolapLikeFilter>>(
                    MolapCommonConstants.DEFAULT_COLLECTION_SIZE);

    /**
     * Map of measures and corresponding measure filter.
     */
    private Map<MolapMeasure, List<MolapMeasureFilter>> measureFilters =
            new HashMap<MolapMeasure, List<MolapMeasureFilter>>(
                    MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
