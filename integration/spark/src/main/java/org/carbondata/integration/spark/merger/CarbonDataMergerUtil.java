...
public final class CarbonDataMergerUtil {
    ...
    public static List<String> getLoadsToMergeFromHDFS(String storeLocation, FileType fileType,
        ...
        try {
            ...
            toLoadMergeMaxSize =
                    Integer.parseInt(CarbonCommonConstants.TO_LOAD_MERGE_MAX_SIZE_DEFAULT);
        }
    ...
    private static boolean checkSizeOfloadToMerge(final LoadMetadataDetails loadDetail,
        ...
        for (int partition = 0; partition < partitionCount; partition++) {
            String loadPath = LoadMetadataUtil
                    .createLoadFolderPath(carbonLoadModel, storeLocation, partition,
    ...
    private static boolean isLoadMetadataPresentInRsFolders(CarbonFile[] loadFiles,
            String loadName) {
        for (CarbonFile load : loadFiles) {
            ...
            if (nameOfLoad.equalsIgnoreCase(loadName)) {
                ...
                CarbonFile[] factFiles = load.listFiles( new CarbonFileFilter() {
                    @Override
                    public boolean accept(CarbonFile file) {
                        if (file.getName().endsWith(CarbonCommonConstants.FACT_FILE_EXT) || file
    ...
    public static String getMergedLoadName(List<String> loadName) {
        ...
        if (mergeLoadName.contains(CarbonCommonConstants.MERGERD_EXTENSION)) {
            ...
            return loadNum + CarbonCommonConstants.MERGERD_EXTENSION
       ...
       CarbonTable cube = org.carbondata.core.carbon.metadata.CarbonMetadata.getInstance().getCarbonTable(loadModel.getSchemaName() + '_' + loadModel.getCubeName());
        ...
        for (int partitionId = 0; partitionId < partitionCount; partitionId++) {
            ...
            CarbonFile[] loads = loadFolder.listFiles(new CarbonFileFilter() {
                @Override
                public boolean accept(CarbonFile file) {
                    if (file.getName().startsWith(CarbonCommonConstants.LOAD_FOLDER) && file
                            .getName().contains(CarbonCommonConstants.MERGER_FOLDER_EXT)) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    private static boolean checkIfOldMergeLoadCanBeDeleted(CarbonFile eachMergeLoadFolder,
        ...
        for (LoadMetadataDetails loadDetail : details) {
            if (null != loadDetail.getMergedLoadName() && (CarbonCommonConstants.LOAD_FOLDER
                    + loadDetail.getMergedLoadName())
        ...
        if (!found) {
            ...
            long loadTime = Long.parseLong(
                    loadName.substring(loadName.lastIndexOf(CarbonCommonConstants.UNDERSCORE) + 1,
                            loadName.length()));
