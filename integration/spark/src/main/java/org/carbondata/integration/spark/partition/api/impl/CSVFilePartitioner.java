...
import org.carbondata.integration.spark.partition.reader.CSVReader;
...
public class CSVFilePartitioner {
     ...
     */
    @Deprecated
    public void splitFile(String schemaName, String cubeName, List<String> sourceFilePath,
        ...
        badRecordslogger =
                new BadRecordslogger(key, "Partition_" + System.currentTimeMillis() + ".log",
        ...
        for (int i = 0; i < sourceFilePath.size(); i++) {
            try {
                ...
                if (fileName.endsWith(".gz")) {
                    ...
                    dataInputStream = new CSVReader(
                            new InputStreamReader(gzipInputStream, Charset.defaultCharset()),
                    ...
                    dataInputStream =
                            new CSVReader(new InputStreamReader(stream, Charset.defaultCharset()),
                                    CSVReader.DEFAULT_SKIP_LINES, customParser);
    ...
    private void partitionData(String targetFolder, List<String> nodes, int partitionCount,
            ...
            HashMap<Partition, CSVWriter> outputStreamsMap, CSVReader dataInputStream,
            long recordCounter, String fileName, int[] indexes, String fileAbsolutePath)
            throws InstantiationException, IllegalAccessException, ClassNotFoundException,
            IOException {
        DataPartitioner dataPartitioner =
                getDataPartitioner(targetFolder, nodes, partitionCount, partitionColumn,
                        headerColumns);
    ...
    private long writeTargetStream(HashMap<Partition, CSVWriter> outputStreamsMap,
            ...
            DataPartitioner dataPartitioner, String[] headerColumns, String fileAbsolutePath)
            throws IOException {
        String[] record = null;
    ...
    private void loopPartitionsAndPopulateOutStreamMap(
        ...
        for (Partition partition : allPartitions) {
            ...
            outputStreamsMap.put(partition, new CSVWriter(new OutputStreamWriter(FileFactory
                    .getDataOutputStream(
                            targetFolderPath + '/' + fileName + '_' + partition.getUniqueID()
                                    + ".csv", fileType, (short) 1), Charset.defaultCharset())));
        }
    }
