...
/**
 * Index type Index, all index should implement this interface.
 */
...
public class IndexProvider extends DataMapProvider {
  ...
  public void cleanMeta() throws IOException {
    if (getMainTable() == null) {
      throw new UnsupportedOperationException("Table need to be specified in index");
    }
  ...
  private IndexFactory<? extends Index> createDataMapFactory()
    ...
    try {
      ...
      indexFactory =
          IndexRegistry.getIndexFactoryByShortName(mainTable, dataMapSchema);
    } catch (Throwable e) {
