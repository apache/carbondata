...
class CarbonDataCacheRDD[K, V](
  ...
  override def compute(theSplit: Partition, context: TaskContext): Iterator[(K, V)] = {
    val iter = new Iterator[(K, V)] {
      val currentRestructNumber =
        CarbonLoaderUtil.getCurrentRestructFolder(schemaName, cubeName, schema)
      ...
      CarbonQueryUtil.createDataSource(currentRestructNumber, schema, null,
        split.serializableHadoopSplit.value.getPartition().getUniqueID(), null, null, null,
        cubeCreationTime, null);
