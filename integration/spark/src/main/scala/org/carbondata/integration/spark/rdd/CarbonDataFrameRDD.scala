...
import org.apache.spark.sql.{DataFrame, CarbonContext, Row}
import org.carbondata.query.querystats.{QueryDetail, QueryStatsCollector}

class CarbonDataFrameRDD(val sql: String, val sc: CarbonContext, logicalPlan: LogicalPlan) extends DataFrame(sc, logicalPlan) {
