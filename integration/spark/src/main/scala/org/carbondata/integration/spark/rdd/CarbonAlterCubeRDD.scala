  ...
  override def compute(theSplit: Partition, context: TaskContext) = {
    val iter = new Iterator[(K, V)] {
      ...
      if (status && InMemoryTableStore.getInstance.isLevelCacheEnabled()) {
        val listOfLoadFolders = CarbonQueryUtil.getListOfSlices(CarbonUtil.readLoadMetadata(metaDataPath))
        CarbonQueryUtil.clearLevelLRUCacheForDroppedColumns(listOfLoadFolders, validDropDimList, schemaName, cubeName, partitioner.partitionCount)
      }
      var loadCount: Integer = -1
      if (addedDimensions.length > 0 || addedMeasures.length > 0) {
        loadCount = CarbonUtil.checkAndReturnCurrentLoadFolderNumber(getNewLoadFolderLocation(partitionID, factTableName, currentRestructNumber + 1) /*hdfsStoreLocation + File.separator + schemaNameWithPartition + File.separator + cubeNameWithPartition + File.separator + "RS_" + currentRestructNumber + File.separator + factTableName*/)
      } else {
        loadCount = CarbonUtil.checkAndReturnCurrentLoadFolderNumber(getNewLoadFolderLocation(partitionID, factTableName, currentRestructNumber))
      }
