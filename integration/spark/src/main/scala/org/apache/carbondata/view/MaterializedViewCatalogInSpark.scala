...
import org.apache.carbondata.core.view.{MaterializedViewCatalog, MaterializedViewSchema}
...
case class MaterializedViewCatalogInSpark(session: SparkSession)
  extends MaterializedViewCatalog[MaterializedViewSchemaWrapper] {

  @transient
  private val logger = MaterializedViewCatalogInSpark.LOGGER

  @transient
  private val lock = new ReentrantReadWriteLock

  @transient
  private val viewSchemas = new scala.collection.mutable.ArrayBuffer[MaterializedViewSchemaWrapper]

  @transient
  private val viewManager = MaterializedViewManagerInSpark.get(session)
  ...
  override def getValidSchemas: Array[MaterializedViewSchemaWrapper] = {
   ...
   */
  def registerSchema(mvSchema: MaterializedViewSchema): Unit = {
    withWriteLock {
      ...
      val logicalPlan = MaterializedViewHelper.dropDummyFunction(
        MaterializedViewQueryParser.getQueryPlan(mvSchema.getQuery, session))
      ...
      val modularPlan = Select(relation.outputList,
        ...
        Seq.empty,
        None)
      viewSchemas += MaterializedViewSchemaWrapper(
        mvSignature,
        mvSchema,
        logicalPlan,
        MaterializedViewPlanWrapper(modularPlan, mvSchema))
    }
  }
  ...
  def registerSchema(query: DataFrame, tableName: Option[String] = None): Unit = {
    withWriteLock {
      ...
      if (lookupSchema(logicalPlan).nonEmpty) {
        ...
        viewSchemas += MaterializedViewSchemaWrapper(signature, null, logicalPlan, null)
      }
    }
  }

  /** Returns feasible registered mv schemas for processing the given ModularPlan. */
  private def lookupSchema(plan: LogicalPlan): Option[MaterializedViewSchemaWrapper] = {
    withReadLock {
  ...
  /** Returns feasible registered mv schemas for processing the given ModularPlan. */
  def lookupFeasibleSchemas(plan: ModularPlan): Seq[MaterializedViewSchemaWrapper] = {
    withReadLock {
...
object MaterializedViewCatalogInSpark {

  val LOGGER: Logger =
    LogServiceFactory.getLogService(classOf[MaterializedViewCatalogInSpark].getCanonicalName)
