...
object IndexRebuildRDD {
  ...
  /**
   * Rebuild the index for all existing data in the table
   */
  def rebuild(
    ...
    val segments2DmStorePath = validSegments.asScala.map { segment =>
      val dataMapStorePath = CarbonTablePath.getIndexStorePath(carbonTable.getTablePath,
    ...
    segments2DmStorePath.foreach { case (_, dmPath) =>
      if (!FileFactory.mkdirs(dmPath)) {
        throw new IOException(
          s"Failed to create directory $dmPath for rebuilding index ${ schema.getDataMapName }")
      }
    }
    ...
    val failedSegments = status
      ...
      .map { task =>
        ...
        if (cleanResult.exists(!_)) {
          LOGGER.error(s"Failed to clean up index store for segment_$segmentId")
          false
        } else {
          true
        }
    ...
    if (failedSegments.nonEmpty) {
      throw new Exception(s"Failed to refresh index ${ schema.getDataMapName }")
    }
...
class IndexRebuildRDD[K, V](
  ...
  override def internalCompute(split: Partition, context: TaskContext): Iterator[(K, V)] = {
    ...
    if (segment.isDefined) {
      ...
      try {
        val segmentPropertiesFetcher = DataMapStoreManager.getInstance().getIndexInTable(
          carbonTable, BlockletIndexFactory.DATA_MAP_SCHEMA
        ).getIndexFactory.asInstanceOf[SegmentPropertiesFetcher]
        val segmentProperties = segmentPropertiesFetcher.getSegmentProperties(segment.get)

        // we use task name as shard name to create the folder for this index
        ...
        // skip clear index and we will do this adter rebuild
        reader.setSkipClearIndexAtClose(true)

        // Note that index rebuilding is based on query, the blockletId in rowWithPosition
