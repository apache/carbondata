 ...
 */
class DistributedCountRDD(@transient ss: SparkSession, dataMapFormat: DistributableIndexFormat)
  ...
  private def generateFuture(split: Seq[InputSplit])
    ...
    Future {
      val segments = split.map { inputSplit =>
        val distributable = inputSplit.asInstanceOf[IndexDistributableWrapper]
        distributable.getDistributable.getSegment
          .setReadCommittedScope(dataMapFormat.getReadCommittedScope)
        distributable.getDistributable.getSegment
      }
      val defaultIndex = DataMapStoreManager.getInstance
        .getIndexInTable(dataMapFormat.getCarbonTable, split.head
          .asInstanceOf[IndexDistributableWrapper].getDistributable.getDataMapSchema)
      defaultIndex.getBlockRowCount(segments.toList.asJava, dataMapFormat
