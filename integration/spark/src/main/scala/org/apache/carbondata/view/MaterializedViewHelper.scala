...
object MaterializedViewHelper {
  ...
  private def dropDummyFunction(expressions: Seq[NamedExpression]) = {
    expressions.map {
      case Alias(_: ScalaUDF, name)
        if name.equalsIgnoreCase(MaterializedViewUDF.DUMMY_FUNCTION) => None
      case reference: AttributeReference
        if reference.name.equalsIgnoreCase(MaterializedViewUDF.DUMMY_FUNCTION) => None
   ...
   * @param relationList list of associated table from query
  ...
  private def getFieldsMapFromProject(
      ...
      fieldCounter: AtomicInteger): mutable.LinkedHashMap[Field, MaterializedViewField] = {
    var fieldsMap = scala.collection.mutable.LinkedHashMap.empty[Field, MaterializedViewField]
  ...
  private def getFieldsMapFromProject(
      ...
      fieldCounter: AtomicInteger): mutable.LinkedHashMap[Field, MaterializedViewField] = {
    val fieldsMap = scala.collection.mutable.LinkedHashMap.empty[Field, MaterializedViewField]
    projectList.map {
      case reference: AttributeReference =>
        val relation = getRelation(relationList, reference)
        if (null != relation) {
          val associatedFields: ArrayBuffer[AssociatedFieldWrapper] =
            new ArrayBuffer[AssociatedFieldWrapper]()
          associatedFields += AssociatedFieldWrapper(
              relation.database,
          ...
          fieldsMap.put(
            newField(
              ...
              "",
              fieldCounter),
            MaterializedViewField("", associatedFields)
          )
        }
      ...
      case alias@Alias(agg: AggregateExpression, _) =>
        ...
        fieldsMap.put(
          newField(
            ...
            fieldCounter),
          MaterializedViewField(agg.aggregateFunction.nodeName, associatedFields)
        )
      ...
      case alias@Alias(_, _) =>
        ...
        fieldsMap.put(
          newField(
            ...
            "arithmetic",
            fieldCounter),
          MaterializedViewField("arithmetic", associatedFields)
        )
    }
    fieldsMap
  }
