...
import org.apache.spark.sql.catalyst.plans.logical.{
  CreateTable, InsertIntoStatement, Join,
  JoinHint, LogicalPlan, OneRowRelation, QualifiedColType, TableSpec
}
...
trait SparkVersionAdapter {
  ...
  def getUpdatedPlan(plan: LogicalPlan, sqlText: String): LogicalPlan = {
    plan match {
      case create@CreateTable(_, _, _, table@TableSpec(properties, _, _, location, _, _, _), _) =>
        if (location.isDefined &&
          ...
          create.copy(tableSpec = table.copy(properties = newProperties))
        } else if (table.options.contains("latestversion")) {
          // remove latestversion property in options if present
          val newOptions = table.options.filterNot(_._1.equalsIgnoreCase("latestversion"))
          create.copy(tableSpec = table.copy(options = newOptions))
        } else {
          create
        }
      case others => others
