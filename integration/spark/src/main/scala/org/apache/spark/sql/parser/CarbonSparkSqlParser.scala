...
class CarbonHelperSqlAstBuilder(conf: SQLConf,
   ...
   */
  override def visitPropertyKeyValues(ctx: PropertyListContext): Map[String, String] = {
    val props = visitPropertyKeyValues(ctx)
    CarbonSparkSqlParserUtil.visitPropertyKeyValues(ctx, props)
  }

  def getPropertyKeyValues(ctx: PropertyListContext): Map[String, String] = {
