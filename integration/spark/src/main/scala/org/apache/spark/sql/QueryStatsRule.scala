...
/**
 * This rule will get registed to unified context.
 * This will be parrent for all logical plan related to select queries
 */
class QueryStatsRule extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = {
    ...
    if ((plan.isInstanceOf[Aggregate]) && needToExclude(plan)) {
      ...
      plan match {
        ...
        case Aggregate(groupingExpressions, aggregateExpressions, child) => QueryStatsLogicalPlan(
          transformPlan(plan))
  ...
  /**
   * ways to identify for aggregate query if they are already included is,
   * if its child is filter,subquery or join
   */
