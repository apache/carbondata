...
import org.apache.spark.sql.optimizer.MaterializedViewRewriteRule
import org.apache.spark.sql.util.SparkSQLUtil

import org.apache.carbondata.common.exceptions.sql.MalformedCarbonCommandException
import org.apache.carbondata.view.MaterializedViewUDF

class MaterializedViewQueryParser extends StandardTokenParsers with PackratParsers {
  ...
  private lazy val queryAppendDummyFunction: Parser[String] =
    SELECT ~> query <~ opt(";") ^^ {
      case query =>
        "SELECT " +
        MaterializedViewUDF.DUMMY_FUNCTION + "() as " +
        MaterializedViewUDF.DUMMY_FUNCTION + ", " + query
    }
