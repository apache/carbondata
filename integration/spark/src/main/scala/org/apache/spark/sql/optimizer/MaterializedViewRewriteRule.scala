...
import org.apache.carbondata.core.view.{MaterializedViewCatalog, MaterializedViewCatalogFactory}
import org.apache.carbondata.mv.plans.modular.{ModularPlan, Select}
import org.apache.carbondata.view.{MaterializedViewCatalogInSpark, MaterializedViewManagerInSpark, MaterializedViewSchemaWrapper}
import org.apache.carbondata.view.MaterializedViewUDF.DUMMY_FUNCTION

/**
 * Analyzer rule to rewrite the query for MV datamap
 */
class MaterializedViewRewriteRule(session: SparkSession) extends Rule[LogicalPlan] {

  private val logger = MaterializedViewRewriteRule.LOGGER

  private val catalogFactory = new MaterializedViewCatalogFactory[MaterializedViewSchemaWrapper] {
  ...
  override def apply(logicalPlan: LogicalPlan): LogicalPlan = {
    ...
    if (viewCatalog != null && hasSuitableMaterializedView(logicalPlan, viewCatalog)) {
      val viewRewrite = new MaterializedViewRewrite(viewCatalog, logicalPlan, session)
   ...
   */
  private def hasSuitableMaterializedView(logicalPlan: LogicalPlan,
      catalog: MaterializedViewCatalogInSpark): Boolean = {
    if (!logicalPlan.isInstanceOf[Command] && !logicalPlan.isInstanceOf[DeserializeToObject]) {
      ...
      !isAssociatedTableSegmentsSetAsInput(catalogs) &&
      isAssociated(validSchemas, catalogs)
    } else {
      false
    }
  ...
  private def isRewritten(
      viewSchemas: Array[MaterializedViewSchemaWrapper],
  ...
  /**
   * Check whether any suitable mvs(like mv which associated tables are present in the plan)
   * exists for this plan.
   *
   * @return
   */
  private def isAssociated(mvSchemas: Array[MaterializedViewSchemaWrapper],
      tables: Seq[Option[CatalogTable]]): Boolean = {
    tables.exists {
      table =>
        mvSchemas.exists {
          mvSchema =>
          mvSchema.viewSchema.getAssociatedTables.asScala.exists {
            mvIdentifier =>
  ...
  /**
   * Check if any segments are set for associated table for Query. If any segments are set, then
   * skip mv mv table for query
   */
  private def isAssociatedTableSegmentsSetAsInput(tables: Seq[Option[CatalogTable]]): Boolean = {
    tables.foreach {
      table =>
...
object MaterializedViewRewriteRule {

  private val LOGGER =
    LogServiceFactory.getLogService(classOf[MaterializedViewRewriteRule].getCanonicalName)
