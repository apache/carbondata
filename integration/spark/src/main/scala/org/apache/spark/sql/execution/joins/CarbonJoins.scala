...
case class BroadCastFilterPushJoin(
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    val tableScan = carbonScan.collectFirst { case a: CarbonScan => a }
    val configuredFilteredRecordSize = CarbonProperties.getInstance.getProperty(
        CarbonCommonConstants.BROADCAST_RECORD_SIZE,
        CarbonCommonConstants.DEFAULT_BROADCAST_RECORD_SIZE)
    if (tableScan.isDefined && filters(0).size>100) {
      tableScan.get.addPushdownFilters(streamedKeys, filters, condition)
    }
