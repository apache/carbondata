...
import org.apache.spark.sql.parser.{
  CarbonHelperSqlAstBuilder, CarbonSpark2SqlParser,
  ...
  override def visitCreateTable(ctx: CreateTableContext): LogicalPlan = {
    ...
    if (fileStorage.equalsIgnoreCase("'carbondata'") ||
      ...
      val commentSpecContext = if (commentSpec.isEmpty) {
        ...
        ParserUtils.string(commentSpec.get(0).COMMENT())
      }
      val createTableTuple = (ctx.createTableHeader, createTableClauses.skewSpec(0),
        ...
        ctx.createOrReplaceTableColTypeList(), createTableClauses.tableProps,
        createTableClauses.locationSpec(0), Option(commentSpecContext), ctx.AS, ctx.query,
        fileStorage)
