...
object HorizontalCompaction {
  ...
  def tryHorizontalCompaction(
    ...
    var segsList = CarbonDataMergerUtil.getValidSegmentList(carbonTable)
      .asScala.map(_.getSegmentNo).toList

    if (null != updatedSegmentList) {
      segsList = segsList
        .filter(segmentNo => updatedSegmentList.contains(segmentNo))
    }
  ...
  private def performDeleteDeltaCompaction(sparkSession: SparkSession,
      ...
      factTimeStamp: Long,
      updatedSegsList: util.List[String]): Unit = {

    val db = carbonTable.getDatabaseName
    val table = carbonTable.getTableName
    val deletedBlocksList = CarbonDataMergerUtil.getSegListIUDCompactionQualified(updatedSegsList,
      absTableIdentifier,
      segmentUpdateStatusManager,
      compactionTypeIUD)
