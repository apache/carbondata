...
import org.apache.carbondata.view.{MaterializedViewManagerInSpark, MaterializedViewRefresher}

object MaterializedViewListeners {
  def getAssociatedColumns(mvSchema: MaterializedViewSchema,
      table: CarbonTable): mutable.Buffer[String] = {
    val columnList: mutable.Buffer[String] = new mutable.ArrayBuffer[String]()
    columnList.asJava.addAll(mvSchema.getAssociatedTableColumns.get(table.getTableName))
    columnList
  }
}

/**
 * Listener to trigger compaction on mv after associated table compaction
 */
object MVCompactionPostEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    val viewSchemas =
      MaterializedViewManagerInSpark.get(compactionEvent.sparkSession).getSchemasOnTable(table)
...
object MVLoadPreEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    if (table.isMaterializedView && !isInternalCall) {
...
/**
 * Listener to trigger data load on mv after associated table data load
 */
    ...
    viewSchemas.asScala.foreach {
      viewSchema =>
        ...
        if (!viewSchema.isRefreshOnManual) {
          try {
            MaterializedViewRefresher.refresh(viewSchema, SparkSession.getActiveSession.get)
            viewManager.setStatus(viewIdentifier, MaterializedViewStatus.ENABLED)
          } catch {
            case _: Exception =>
              viewManager.setStatus(viewIdentifier, MaterializedViewStatus.DISABLED)
          }
        } else {
          viewManager.setStatus(viewIdentifier, MaterializedViewStatus.DISABLED)
        }
...
object MVDeleteSegmentPreEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    if (table != null) {
      ...
      val viewSchemas = MaterializedViewManagerInSpark.get(session).getSchemasOnTable(table)
      if (!viewSchemas.isEmpty) {
        throw new UnsupportedOperationException(
          "Delete segment operation is not supported on table associated by mv " +
...
object MVDropColumnPreEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    val viewSchemas =
      MaterializedViewManagerInSpark.get(dropColumnEvent.sparkSession).getSchemasOnTable(table)
    if (viewSchemas.isEmpty) {
      return
    }
    viewSchemas.asScala.foreach {
      mvSchema =>
        val associatedColumns = MaterializedViewListeners.getAssociatedColumns(mvSchema, table)
        val associatedColumnsInDropModel = associatedColumns.collectFirst {
          case columnName if dropColumnModel.columns.contains(columnName.toLowerCase) => columnName
        }
        if (associatedColumnsInDropModel.isDefined) {
          val mvIdentifier = mvSchema.getIdentifier
          throw new UnsupportedOperationException(
            s"Column ${associatedColumnsInDropModel.head} cannot be dropped because it exists " +
...
object MVAlterColumnPreEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    val viewSchemas =
      MaterializedViewManagerInSpark.get(alterColumnEvent.sparkSession).getSchemasOnTable(table)
    if (viewSchemas.isEmpty) {
      return
    }
    viewSchemas.asScala.foreach {
      viewSchema =>
        val associatedColumns = MaterializedViewListeners.getAssociatedColumns(viewSchema, table)
        if (associatedColumns.contains(alterTableModel.columnName.toLowerCase)) {
          val viewIdentifier = viewSchema.getIdentifier
          throw new UnsupportedOperationException(
...
object MVDropPartitionMetaEventListener extends OperationEventListener {
  ...
  override def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    val viewSchemasWithoutPartitionColumns = viewSchemas.asScala.filter {
      viewSchema =>
        val associatedColumns = viewSchema.getAssociatedTableColumns.get(table.getTableName).asScala
        val associatedColumnInDropPartitions = dropPartitionEvent.specs.flatMap(_.keys).forall {
            partitionColumn =>
              associatedColumns.exists {
                associatedColumn => associatedColumn.equalsIgnoreCase(partitionColumn)
              }
          }
        !associatedColumnInDropPartitions
      }
    ...
    val viewSchemasWithoutPartition = viewSchemas.asScala.filter(_.getAssociatedTables.size() >= 2)
...
object MVDropPartitionPreEventListener extends OperationEventListener {
  ...
  override protected def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    if (dropPartitionCommands != null &&
      MaterializedViewManagerInSpark.get(dropPartitionEvent.sparkSession).hasSchemaOnTable(table)) {
...
object MVDropTablePreEventListener extends OperationEventListener {
  ...
  override protected def onEvent(event: Event, operationContext: OperationContext): Unit = {
    ...
    if (table.isMaterializedView && !dropTableEvent.isInternalCall) {
