...
import org.apache.carbondata.core.view.{MaterializedViewProperty, MaterializedViewSchema}
import org.apache.carbondata.view.MaterializedViewManagerInSpark

/**
 * Show Materialized View Command implementation
 */
case class CarbonShowMaterializedViewCommand(
    databaseNameOption: Option[String],
    associatedTableIdentifier: Option[TableIdentifier]) extends DataCommand {
  ...
  override def processData(session: SparkSession): Seq[Row] = {
    // Get mv schemas.
    val schemaList = new util.ArrayList[MaterializedViewSchema]()
    val viewManager = MaterializedViewManagerInSpark.get(session)
    associatedTableIdentifier match {
      case Some(table) =>
        val associatedTable = CarbonEnv.getCarbonTable(table)(session)
        setAuditTable(associatedTable)
        ...
        if (databaseNameOption.isDefined) {
          ...
          schemaList.addAll(viewManager.getSchemasOnTable(associatedTable))
        }
      case _ =>
        if (databaseNameOption.isDefined) {
    ...
    schemaList.asScala.map {
      schema =>
        Row(
          ...
          schema.getProperties.get(MaterializedViewProperty.REFRESH_MODE),
          schema.getProperties.get(MaterializedViewProperty.REFRESH_TRIGGER_MODE),
