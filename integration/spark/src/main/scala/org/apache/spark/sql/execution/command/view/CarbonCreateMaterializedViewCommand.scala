 ...
 * and register the MV schema in [[MaterializedViewManager]]
 */
case class CarbonCreateMaterializedViewCommand(
  ...
  private val logger = CarbonCreateMaterializedViewCommand.LOGGER

  private var viewSchema: MaterializedViewSchema = _
  ...
  override def processMetadata(session: SparkSession): Seq[Row] = {
    ...
    OperationListenerBus.getInstance().fireEvent(
      CreateMaterializedViewPreExecutionEvent(session, identifier),
      operationContext)

    val catalogFactory = new MaterializedViewCatalogFactory[MaterializedViewSchemaWrapper] {
      override def newCatalog(): MaterializedViewCatalog[MaterializedViewSchemaWrapper] = {
        new MaterializedViewCatalogInSpark(session)
      }
    }
    ...
    try {
      ...
      if (schema.isRefreshOnManual) {
        viewManager.setStatus(schema.getIdentifier, MaterializedViewStatus.DISABLED)
      }
    } catch {
    ...
    OperationListenerBus.getInstance().fireEvent(
      CreateMaterializedViewPostExecutionEvent(session, identifier),
      operationContext)
  ...
  override def processData(session: SparkSession): Seq[Row] = {
    if (this.viewSchema != null && !deferredRefresh) {
      MaterializedViewRefresher.refresh(this.viewSchema, session)
      val viewManager = MaterializedViewManagerInSpark.get(session)
      viewManager.setStatus(this.viewSchema.getIdentifier, MaterializedViewStatus.ENABLED)
    }
    Seq.empty
  }
  ...
  override def undoMetadata(sparkSession: SparkSession, exception: Exception): Seq[Row] = {
    if (this.schema != null) {
      CarbonDropMaterializedViewCommand(databaseNameOption, name, ifExistsSet = true).run(
        sparkSession)
    }
    Seq.empty
  }
  ...
  private def doCreate(session: SparkSession,
      ...
      viewManager: MaterializedViewManagerInSpark): MaterializedViewSchema = {
    val logicalPlan = MaterializedViewHelper.dropDummyFunction(
      MaterializedViewQueryParser.getQueryPlan(queryString, session))
    ...
    val associatedTables = getAssociatedTables(logicalPlan)
    val associatedTableList = toCarbonTables(session, associatedTables)
    val associatedTableNames = new util.ArrayList[String](associatedTableList.size())
    // Check if load is in progress in any of the parent table mapped to the datamap
    associatedTableList.asScala.foreach {
      table =>
        ...
        associatedTableNames.add(table.getTableName)
    }

    val viewRefreshMode = if (checkIsQueryNeedFullRefresh(logicalPlan) ||
      checkIsHasNonCarbonTable(associatedTables)) {
      MaterializedViewProperty.REFRESH_MODE_FULL
    } else {
    ...
    val viewRefreshTriggerMode = if (deferredRefresh) {
      ...
      properties.getOrElse(MaterializedViewProperty.REFRESH_TRIGGER_MODE,
        MaterializedViewProperty.REFRESH_TRIGGER_MODE_ON_COMMIT)
    }

    val viewProperties = mutable.Map[String, String]()
    viewProperties.put(
      CarbonCommonConstants.ASSOCIATED_TABLES, associatedTableNames.asScala.mkString(","))

    val (timeSeriesColumn, granularity) = checkTimeSeriesQuery(logicalPlan, viewRefreshTriggerMode)

    val fieldsMap = MaterializedViewHelper.getFieldsMapFromPlan(
    ...
    if (associatedTableList.size() == 1 && CarbonSource.isCarbonDataSource(associatedTables.head)) {
      inheritTablePropertiesFromAssociatedTable(
        associatedTableList.get(0),
        fieldsMap,
        viewSchema,
        viewProperties)
      if (granularity != null) {
        val timeSeriesDataType = associatedTableList.get(0).getTableInfo.getFactTable
    ...
    // Inherit partition from associated table if mv is mapped to single associated table
    val viewPartitionerFields = if (associatedTableList.size() == 1) {
      val associatedTablePartitionColumns =
        if (properties.getOrElse("partitioning", "true").toBoolean &&
          associatedTableList.get(0).isHivePartitionTable) {
          associatedTableList.get(0).getPartitionInfo
            .getColumnSchemaList.asScala.map(_.getColumnName)
        } else {
          Seq.empty
        }
      getViewPartitionerFields(associatedTablePartitionColumns, fieldsMap)
    } else {
      Seq.empty
    }
    ...
    val associatedTableToColumnsMap = new java.util.HashMap[String, util.Set[String]]()
    for (viewField <- fieldsMap.values) {
      viewField.associatedFieldList.foreach {
        associatedField =>
          if (null == associatedTableToColumnsMap.get(associatedField.tableName)) {
            val columns = new util.HashSet[String]()
            columns.add(associatedField.fieldName.toLowerCase())
            associatedTableToColumnsMap.put(associatedField.tableName, columns)
          } else {
            associatedTableToColumnsMap.get(associatedField.tableName)
              .add(associatedField.fieldName.toLowerCase())
          }
      }
    }
    val associatedTableIds = associatedTables.map { table =>
      val associatedTableId = new RelationIdentifier(table.database, table.identifier.table, "")
      associatedTableId.setTablePath(FileFactory.getUpdatedFilePath(table.location.toString))
      associatedTableId.setProvider(table.provider.get)
      associatedTableId
    }
    ...
    val schema = new MaterializedViewSchema(viewManager)
    schema.setIdentifier(viewIdentifier)
    schema.setProperties(mutable.Map[String, String](viewProperties.toSeq: _*).asJava)
    schema.setAssociatedTableColumnList(associatedTableToColumnsMap)
    schema.setColumnsOrderMap(columnOrderMap)
    schema.setAssociatedTables(new util.ArrayList[RelationIdentifier](associatedTableIds.asJava))
    schema.getProperties.put(MaterializedViewProperty.REFRESH_MODE, viewRefreshMode)
    schema.getProperties.put(MaterializedViewProperty.REFRESH_TRIGGER_MODE, viewRefreshTriggerMode)
  ...
  private def getAssociatedTables(logicalPlan: LogicalPlan): Seq[CatalogTable] = {
    logicalPlan.collect {
  ...
  private def getOutputSchema(logicalPlan: LogicalPlan): Seq[Field] = {
    ...
    logicalPlan.output.map { attribute =>
      ...
      val attributeName = MaterializedViewHelper.getUpdatedColumnName(attribute, attributeIndex)
      attributeIndex += 1
  ...
  private def getViewColumns(
      associatedTableColumns: Array[String],
      fieldsMap: scala.collection.mutable.LinkedHashMap[Field, MaterializedViewField],
      viewSchema: Seq[Field]) = {
    val viewColumns = associatedTableColumns.flatMap(
      associatedTableColumn =>
        viewSchema.collect {
          case viewField
            if fieldsMap(viewField).aggregateFunction.isEmpty &&
               fieldsMap(viewField).associatedFieldList.size == 1 &&
               fieldsMap(viewField).associatedFieldList.head.fieldName
                 .equalsIgnoreCase(associatedTableColumn) =>
            viewField.column
        })
    viewColumns
  }
   ...
   *
   */
  private def getViewPartitionerFields(associatedTablePartitionColumns: Seq[String],
    fieldsMap: mutable.LinkedHashMap[Field, MaterializedViewField]): Seq[PartitionerField] = {
      ...
      partitionColumn match {
        case head :: tail =>
          ...
          val validField = fieldsMap.zipWithIndex.collectFirst {
            ...
            viewField.associatedFieldList.nonEmpty &&
              head.equals(viewField.associatedFieldList.head.fieldName) &&
              viewField.aggregateFunction.isEmpty =>
              (PartitionerField(field.name.get,
                field.dataType,
                field.columnComment), associatedTablePartitionColumns.indexOf(head))
          }
    ...
    generatePartitionerField(associatedTablePartitionColumns.toList, Seq.empty)
  }
  ...
  private def checkTimeSeriesGranularity(timeSeriesFunction: String): Unit = {
    var found = false
    breakable {
      for (granularity <- MaterializedViewTimeGranularity.getAll()) {
  ...
  private def inheritTablePropertiesFromAssociatedTable(
      associatedTable: CarbonTable,
      fieldsMap: scala.collection.mutable.LinkedHashMap[Field, MaterializedViewField],
      viewSchema: Seq[Field],
      viewProperties: mutable.Map[String, String]): Unit = {
    var viewTableOrder = Seq[String]()
    val associatedTableOrder = associatedTable.getSortColumns.asScala
    val associatedTableProperties =
      associatedTable.getTableInfo.getFactTable.getTableProperties.asScala
    associatedTableOrder.foreach(
      associatedTableField =>
        viewSchema.filter(
          viewField =>
            fieldsMap(viewField).aggregateFunction.isEmpty &&
            fieldsMap(viewField).associatedFieldList.size == 1 &&
            fieldsMap(viewField).associatedFieldList.head.fieldName.equalsIgnoreCase(
                associatedTableField))
    ...
    val sortScope = associatedTableProperties.get("sort_scope")
    if (sortScope.isDefined) {
      viewProperties.put("sort_scope", sortScope.get)
    }
    viewProperties
      .put(CarbonCommonConstants.TABLE_BLOCKSIZE, associatedTable.getBlockSizeInMB.toString)
    viewProperties.put(CarbonCommonConstants.FLAT_FOLDER,
      associatedTableProperties.getOrElse(
    ...
    if (longStringColumn.isEmpty) {
      val longStringColumnInAssociatedTables = associatedTableProperties
        .getOrElse(CarbonCommonConstants.LONG_STRING_COLUMNS, "").split(",").map(_.trim)
      val varcharColumnInAssociatedTables = scala.collection.mutable.ArrayBuffer.empty[String]
      fieldsMap foreach (fields => {
        val aggregateFunction = fields._2.aggregateFunction
        val associatedFieldList = fields._2.associatedFieldList
        ...
        if (aggregateFunction.isEmpty &&
            associatedFieldList.size == 1 &&
            longStringColumnInAssociatedTables.contains(associatedFieldList.head.fieldName)) {
          varcharColumnInAssociatedTables += associatedFieldList.head.fieldName
        }
      })
      if (varcharColumnInAssociatedTables.nonEmpty) {
        longStringColumn = Option(varcharColumnInAssociatedTables.mkString(","))
      }
    }
    ...
    if (longStringColumn.isDefined) {
      ...
      val newLongStringColumn = longStringColumn.get.split(",").map(_.trim).map {
        columnName =>
          val newColumnName = associatedTable.getTableName.toLowerCase() + "_" + columnName
    ...
    val parentDictInclude = associatedTable.getTableInfo.getFactTable.getTableProperties.asScala
      .getOrElse(CarbonCommonConstants.LOCAL_DICTIONARY_INCLUDE, "").split(",")
    val parentDictExclude = associatedTable.getTableInfo.getFactTable.getTableProperties.asScala
    ...
    val parentInvertedIndex = associatedTable.getTableInfo.getFactTable.getTableProperties.asScala
    ...
    val parentNoInvertedIndex = associatedTable.getTableInfo.getFactTable.getTableProperties.asScala
...
object CarbonCreateMaterializedViewCommand {

  private val LOGGER: Logger = LogServiceFactory.getLogService(
    classOf[CarbonCreateMaterializedViewCommand].getCanonicalName)
