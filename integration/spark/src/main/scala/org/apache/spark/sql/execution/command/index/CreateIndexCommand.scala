...
import org.apache.carbondata.core.metadata.schema.datamap.{DataMapProperty, IndexProviderName}
...
case class CreateIndexCommand(
  ...
  override def processMetadata(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (DataMapStoreManager.getInstance().isIndexExist(parentTable, indexName)) {
      if (!ifNotExistsSet) {
    ...
    if (dataMapSchema.isIndexDataMap && parentTable == null) {
    ...
    val isBloomFilter = IndexProviderName.BLOOMFILTER.getShortName
