...
object DeleteExecution {
  ...
  (Array[List[(SegmentStatus, (SegmentUpdateDetails, ExecutionErrors, Long))]], BlockMappingVO) = {
    ...
    val externalSegmentPathMap = metadataDetails
      .filter(detail => StringUtils.isNotEmpty(detail.getPath))
      .map(f => (f.getLoadName, f.getPath))
      .toMap
      ...
      (index: Int, records: Iterator[(String, (Iterable[Row]))]) =>
        Iterator[List[(SegmentStatus, (SegmentUpdateDetails, ExecutionErrors, Long))]] {
          ...
          while (records.hasNext) {
            ...
            val segmentId = if (carbonTable.isHivePartitionTable) {
              blockId.split("-")(3)
            } else {
              blockId.split("/")(0)
            }
            val externalSegmentPath =
              if (!externalSegmentPathMap.get(segmentId).isEmpty) {
                externalSegmentPathMap.get(segmentId).get
              } else {
                ""
              }
            result = result ++
      ...
      val resultIter =
        ...
        try {
          while (iter.hasNext) {
            ...
            } else if (TID.contains("#/") && StringUtils.isNotEmpty(externalSegmentPath)) {
          ...
          val blockPath = if (StringUtils.isNotEmpty(externalSegmentPath)) {
            externalSegmentPath
          } else {
          ...
          val totalDeletedRows: Long = rowCountDetailsVO.getDeletedRowsInBlock + countOfRows
  ...
  def checkAndUpdateStatusFiles(
    ...
    val updateTableMetadataStatus = CarbonUpdateUtil.updateTableMetadataStatus(updatedSegmentList,
        carbonTable, timestamp, true, true, deletedSegmentList, newMetaEntry)
