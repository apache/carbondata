...
case class CarbonScan(
  ...
  */
case class CarbonCountStar(attributesRaw: Seq[Attribute], @transient relationRaw: CarbonRelation)
  (@transient val ocRaw: SQLContext) extends LeafNode {
  val carbonTable = relationRaw.metaData.carbonTable
  ...
  override def doExecute(): RDD[InternalRow] = {
    ...
    sparkContext.parallelize(
      Seq(new GenericMutableRow(Seq(Long.box(rowCount)).toArray.asInstanceOf[Array[Any]]))
    )
  }
