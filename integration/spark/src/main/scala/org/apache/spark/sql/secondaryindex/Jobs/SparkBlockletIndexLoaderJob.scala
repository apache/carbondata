...
import org.apache.carbondata.core.index.{AbstractIndexJob, DistributableIndexFormat}
...
class SparkBlockletIndexLoaderJob extends AbstractIndexJob {
  ...
  override def execute(carbonTable: CarbonTable,
    ...
    val loader: DistributableBlockletIndexLoader = indexFormat
      .asInstanceOf[DistributableBlockletIndexLoader]
    val indexWrappers = new IndexLoaderRDD(SparkSQLUtil.getSparkSession, loader).collect()
    val cacheableIndex = DataMapStoreManager.getInstance.getDefaultIndexInTable(carbonTable)
    ...
    try {
      indexWrappers.toList.foreach { indexWrapper =>
        executorService
          .submit(new IndexCacher(cacheableIndex,
            indexWrapper,
            carbonTable))
      }
    } finally {
  ...
  private def addSegmentProperties(
    ...
    val indexWrapperList = scala.collection.mutable.ArrayBuffer
    ...
    indexWrapperList.foreach { entry =>
  ...
  override def executeCountJob(indexFormat: DistributableIndexFormat): lang.Long = 0L
}
...
class IndexCacher(
  ...
  override def call(): Unit = {
    ...
    if (carbonTable.getTableInfo.isSchemaModified) {
      val indexes: util.List[BlockIndex] = indexWrapper._2.getBlockletIndexWrapper.getIndexes
      val wrapper = SegmentPropertiesAndSchemaHolder.getInstance()
...
/**
 * This RDD is used to load the secondary index of a segment
 */
class IndexLoaderRDD(
    @transient ss: SparkSession,
    indexFormat: DistributableBlockletIndexLoader)
