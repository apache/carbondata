...
import org.carbondata.core.metadata.MolapMetadata.{Dimension, Cube}
import org.carbondata.core.olap.MolapDef
import org.carbondata.core.util.{MolapUtil, MolapVersion, MolapProperties}
import org.carbondata.integration.spark.load.MolapLoaderUtil
import org.carbondata.integration.spark.util.MolapScalaUtil.OlapUtil
...
//      	tempMetadataFile.renameForce(oldMetadataFile.getAbsolutePath())
 ...
 def getAllCubes()(sqlContext: SQLContext): Seq[(String, String)] = {
  ...
  def readCubeMetaDataFile(cubeFolder: MolapFile, fileType: FileFactory.FileType): (String, String, String, String, Partitioner, Long) = {
    val cubeMetadataFile = cubeFolder.getAbsolutePath() + "/metadata"
