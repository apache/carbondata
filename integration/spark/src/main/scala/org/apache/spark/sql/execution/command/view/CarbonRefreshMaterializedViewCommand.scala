...
import org.apache.carbondata.common.exceptions.sql.{MalformedMaterializedViewCommandException, NoSuchMaterializedViewException}
import org.apache.carbondata.core.view.MaterializedViewStatus
import org.apache.carbondata.events.{OperationContext, OperationListenerBus}
import org.apache.carbondata.view.{MaterializedViewManagerInSpark, MaterializedViewRefresher, RefreshMaterializedViewPostExecutionEvent, RefreshMaterializedViewPreExecutionEvent}
 ...
 */
case class CarbonRefreshMaterializedViewCommand(
  ...
  override def processData(session: SparkSession): Seq[Row] = {
    ...
    val schema = try {
      ...
      case _: NoSuchMaterializedViewException =>
    ...
    MaterializedViewRefresher.refresh(schema, session)
    ...
    OperationListenerBus.getInstance().fireEvent(
      RefreshMaterializedViewPreExecutionEvent(session, identifier),
      operationContext)
    viewManager.setStatus(schema.getIdentifier, MaterializedViewStatus.ENABLED)
    OperationListenerBus.getInstance().fireEvent(
      RefreshMaterializedViewPostExecutionEvent(session, identifier),
      operationContext)
    Seq.empty
  }
