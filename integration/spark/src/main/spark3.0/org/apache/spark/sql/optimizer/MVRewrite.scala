...
class MVRewrite(catalog: MVCatalogInSpark, logicalPlan: LogicalPlan,
  ...
  (Seq[NamedExpression], Seq[Expression]) = {
    val outputList = for ((output1, output2) <- outputListMapping) yield {
      output1 match {
        case Alias(aggregate@AggregateExpression(function@Sum(_), _, _, _, _), _) =>
          val uFun = function.copy(child = output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(function@Max(_), _, _, _, _), _) =>
          val uFun = function.copy(child = output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(function@Min(_), _, _, _, _), _) =>
          val uFun = function.copy(child = output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@Count(Seq(_)), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(agg@AggregateExpression(_@Corr(_, _), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(agg.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@VariancePop(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@VarianceSamp(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@StddevSamp(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@StddevPop(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@CovPopulation(_, _), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@CovSample(_, _), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@Skewness(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case Alias(aggregate@AggregateExpression(_@Kurtosis(_), _, _, _, _), _) =>
          val uFun = Sum(output2)
          Alias(aggregate.copy(aggregateFunction = uFun), output1.name)(exprId = output1.exprId)
        case _ =>
          if (output1.name != output2.name) {
