...
case class CarbonIUDAnalysisRule(sparkSession: SparkSession) extends Rule[LogicalPlan] {
  ...
  private def processUpdateQuery(
    ...
    val completeTableName = table.tableName.split(".")
    val tableIdentifier = TableIdentifier(completeTableName(1), Option(completeTableName(0)))
    ...
    val selectPlan = parser.parsePlan(selectStmt) transform {
      case Project(projectList, child) if !includedDestColumns =>
        ...
        val tableName: Option[Seq[String]] = alias match {
          ...
          case _ => Some(Seq(child.asInstanceOf[UnresolvedRelation].tableName.split(".")(1)))
        }
        val list = Seq(
      ...
      case Filter(cond, child) if !includedDestRelation =>
        ...
        Filter(cond, Join(child, targetTable, Inner, None, JoinHint.NONE))
      case r@CarbonUnresolvedRelation(t) if !includedDestRelation && t != tableIdentifier =>
        includedDestRelation = true
        Join(r, targetTable, Inner, None, JoinHint.NONE)
    }
    val updatedSelectPlan: LogicalPlan = if (!includedDestRelation) {
      ...
      selectPlan transform {
        case relation: UnresolvedRelation
          if table.tableName == relation.tableName && !addedTupleId =>
          addedTupleId = true
          targetTable
      }
    } else {
  ...
  override def apply(logicalPlan: LogicalPlan): LogicalPlan = {
    ...
    logicalPlan transform {
      case UpdateTables(t, cols, sel, alias, where) => processUpdateQuery(t, cols, sel, alias,
        where)
...
case class CarbonPreInsertionCasts(sparkSession: SparkSession) extends Rule[LogicalPlan] {
  def apply(plan: LogicalPlan): LogicalPlan = {
    plan.transform {
      ...
      case p@InsertIntoStatement(relation: LogicalRelation, _, child, _, _)
        if relation.relation.isInstanceOf[CarbonDatasourceHadoopRelation] =>
        castChildOutput(p, relation, child)
    }
  }

  def castChildOutput(p: InsertIntoStatement,
    ...
    if (newChildOutput.size >= carbonDSRelation.carbonRelation.output.size ||
      ...
      InsertIntoCarbonTable(carbonDSRelation, p.partitionSpec, newChild, overwrite, true)
    } else {
      CarbonException.analysisException(
