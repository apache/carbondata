...
object CarbonToSparkAdapter {
  ...
  def createScalaUDF(s: ScalaUDF, reference: AttributeReference): ScalaUDF = {
    ScalaUDF(s.function, s.dataType, Seq(reference), s.inputEncoders, s.udfName)
  }
  ...
  def getExplainCommandObj() : ExplainCommand = {
    ExplainCommand(OneRowRelation(), ExplainMode.fromString("simple"))
  }
...
class CarbonOptimizer(
    ...
    optimizer: Optimizer,
    catalogManager: CatalogManager) extends Optimizer(catalogManager) {
