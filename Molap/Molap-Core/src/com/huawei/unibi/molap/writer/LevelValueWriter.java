...
public class LevelValueWriter {
  ...
  private static final LogService LOGGER =
      LogServiceFactory.getLogService(LevelValueWriter.class.getName());
  /**
   * memberFileName
   */
  ...
  public LevelValueWriter(String meString, String storeFolderLocation) throws KettleException {
    ...
    int rowSetSize = Integer.parseInt(instance.getProperty(MolapCommonConstants.GRAPH_ROWSET_SIZE,
  ...
  private int getIntValue(String val) {
    ...
    try {
      ...
      LOGGER.info(MolapCoreLogEvent.UNIBI_MOLAPCORE_MSG,
          "Level File is already renamed so there will not be" + "any need to keep the counter");
    }
  ...
  public void performRequiredOperation() {
    ...
    try {
      ...
      LOGGER.error(MolapCoreLogEvent.UNIBI_MOLAPCORE_MSG,
          "Not able to create the output stream for File :" + memberFileName + (counter - 1));
    }
  ...
  private void storeValueInCache(String value, int key, Object[] properties) throws IOException {
    ...
    for (Closeable stream : streams) {
      if (null != stream) {
        try {
          ...
          LOGGER.error(MolapCoreLogEvent.UNIBI_MOLAPCORE_MSG, e, "Unable to close Steam ");
        }
      }
    }
    // delete the file
