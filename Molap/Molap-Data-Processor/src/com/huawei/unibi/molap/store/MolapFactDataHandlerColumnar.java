...
import com.huawei.unibi.molap.store.writer.*;
...
public class MolapFactDataHandlerColumnar implements MolapFactHandler {
     ...
     * @param highCardCount
     */
    public MolapFactDataHandlerColumnar(String schemaName, String cubeName, String tableName,
        ...
        this(schemaName, cubeName, tableName, isGroupByEnabled, measureCount, mdkeyLength,
                ...
                isDataWritingRequest, currentRestructNum, highCardCount);
        this.dimensionCount = dimensionCount;
        this.complexIndexMap = complexIndexMap;

        this.aggKeyBlock = new boolean[dimLens.length];
        this.isAggKeyBlock = Boolean.parseBoolean(MolapProperties.getInstance()
                .getProperty(MolapCommonConstants.AGGREAGATE_COLUMNAR_KEY_BLOCK,
    ...
    public MolapFactDataHandlerColumnar(String schemaName, String cubeName, String tableName,
            ...
            boolean isMergingRequestForCustomAgg, boolean isUpdateMemberRequest, int[] dimLens,
            String[] factLevels, String[] aggLevels, boolean isDataWritingRequest,
            int currentRestructNum, int highCardCount) {
    ...
    private void addToStore(Object[] row) throws MolapDataWriterException {
        ...
        for (int k = 0; k < otherMeasureIndex.length; k++) {
            if (null == row[otherMeasureIndex[k]]) {
                ...
                dataHolder[otherMeasureIndex[k]].setWritableDoubleValueByIndex(entryCount,
                        (Double) row[otherMeasureIndex[k]]);
            }
        }
    ...
    private void writeDataToFile(byte[][] data, byte[][] dataHolderLocal, byte[][][] columnData,
        ...
        try {
            ...
            //   e.printStackTrace();
            LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, ex,
                    ex.getMessage());
        }
        ...
        try {
            ...
            // TODO Auto-generated catch block
            //            e.printStackTrace();
            LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, exception,
    ...
    public void finish() throws MolapDataWriterException {
        ...
        if (this.entryCount > 0) {
                    ...
                    for (int j = 0; j < splitKey.length; j++) {
                        ...
                        //complex types
                            ...
                            GenericDataType complexDataType =
                                    complexIndexMap.get(numberOfColumns + j);
                            if (complexDataType != null) {
            ...
            List<Future<IndexStorage>> submit =
                    new ArrayList<Future<IndexStorage>>(numberOfColumns + highCardCount);
            int i = 0;
            ...
            IndexStorage[] blockStorage =
                    new IndexStorage[numberOfColumns + highCardCount + complexColCount];
            try {
    ...
    private void setWritingConfiguration(int mdkeySize) throws MolapDataWriterException {
        ...
        if (dimLens.length > 0) {
            this.columnarSplitter = new MultiDimKeyVarLengthEquiSplitGenerator(
                    MolapUtil.getIncrementedCardinalityFullyFilled(dimLens.clone()), (byte) dimSet);
            this.keyBlockHolder =
