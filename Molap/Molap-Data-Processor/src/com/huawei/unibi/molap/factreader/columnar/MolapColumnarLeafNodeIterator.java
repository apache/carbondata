...
public class MolapColumnarLeafNodeIterator implements MolapIterator<AbstractColumnarScanResult> {
     ...
     */
    public MolapColumnarLeafNodeIterator(MolapFile[] factFiles, int mdkeyLength,
    ...
    private void intialiseColumnarLeafNodeIterator(MolapFile[] factFiles, int mdkeyLength,
        ...
        ColumnarSplitter columnarSplitter = new MultiDimKeyVarLengthEquiSplitGenerator(
                MolapUtil.getIncrementedCardinalityFullyFilled(iteratorInfo.getDimLens()),
                (byte) dimSet);
     ...
     */
    private void initialise(MolapFile[] factFiles, LeafNodeInfoColumnar leafNodeInfoColumnar) {
        this.leafNodeInfoList =
                new ArrayList<LeafNodeInfoColumnar>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        if (null != leafNodeInfoColumnar) {
            ...
            for (int i = 0; i < factFiles.length; i++) {
                leafNodeInfo =
                        MolapUtil.getLeafNodeInfoColumnar(factFiles[i], measureCount, mdKeyLength);
