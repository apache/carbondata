...
public class LevelGlobalSurrogateGeneratorThread implements Callable<Void>
{
    ...
    public Void call() throws Exception
    {
        ...
        Hierarchy[] extractHierarchies = MolapSchemaParser.extractHierarchies(
                schema, dimension);
        ...
        String hierarchyTable = relation == null ? tableName
                : ((Table)extractHierarchies[0].relation).name;
        String levelFileName = hierarchyTable + '_' + cubeLevel.name;

        List<PartitionMemberVo> partitionMemberVoList = new ArrayList<PartitionMemberVo>(MolapCommonConstants.CONSTANT_SIZE_TEN);

        ExecutorService ex = Executors.newFixedThreadPool(10);

        PartitionMemberVo memberVo = null;

        List<Future<Map<String, Integer>>> submitList = new ArrayList<Future<Map<String, Integer>>>(MolapCommonConstants.CONSTANT_SIZE_TEN);
        ...
        for(int i = 0;i < partitionLocation.length;i++)
        {
			...
			String path = partitionLocation[i][partitionLength - 1]
                            + '/' + levelFileName + ".level";
            ...
            Future<Map<String, Integer>> submit = ex
                    .submit(new ReaderThread(molapFile));
        ...
        int maxSeqenceKey = getMaxSequenceKeyAssigned(levelFileName
                + ".globallevel");
        int index=0;
        ...
        createGlobalSurrogateKey(currentTimeMillis, currentTimeMillis1,
				isPartitionColumn, levelFileName, partitionMemberVoList, ex,
				maxSeqenceKey);
	...
	private void createGlobalSurrogateKey(long currentTimeMillis,
			...
			int maxSeqenceKey) throws InterruptedException {
		int[] key = new int[partitionMemberVoList.get(0).getMembersMap().size()];
        int[] value = new int[partitionMemberVoList.get(0).getMembersMap()
                .size()];
        ...
        {
            for(Entry<String, Integer> entryInfo : partitionMemberVoList.get(0)
                    .getMembersMap().entrySet())
            {
            ...
            LOGGER.info(
                    ...
                    "Time Taken to generate global surrogate for Level: "
                            + levelFileName + " : "
                            + (System.currentTimeMillis() - currentTimeMillis));
            ...
            ex.submit(new WriterThread(localKey, localValue,
                    partitionMemberVoList.get(0).getPath(), levelFileName
                            + ".globallevel", maxSeqenceKey,minSeqenceKey));
            
            
            processNonPartitionedColumn(currentTimeMillis, levelFileName,
        ...
        else
        {
            for(int i = 0;i < partitionMemberVoList.size();i++)
            {
                ...
                key = new int[partitionMemberVoList.get(i).getMembersMap()
                        .size()];
                value = new int[partitionMemberVoList.get(i).getMembersMap()
                        .size()];
                for(Entry<String, Integer> entry : partitionMemberVoList.get(i)
                        .getMembersMap().entrySet())
                {
                ...
                currentTimeMillis = System.currentTimeMillis();
                ex.submit(new WriterThread(localKey, localValue,
                        partitionMemberVoList.get(i).getPath(), levelFileName
                                + ".globallevel", maxSeqenceKey,minSeqenceKey));
            }
        }

        LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG,
                "Time Taken to write global surrogate for Level: "
                        + levelFileName + " : "
	...
	private void processNonPartitionedColumn(long currentTimeMillis,
		...
		for(int i = 1;i < partitionMemberVoList.size();i++)
		{
		            ...
		            .getMembersMap().entrySet())
		    {
		        ...
		        for(int j = 0;j < i;j++)
		        {
		            surrogateKey = partitionMemberVoList.get(j)
		                    .getMembersMap().get(entry.getKey());
    ...
    private int getMaxSequenceKeyAssigned(String levelFileName)
    {
        ...
        for(int i = 0;i < partitionLocation.length;i++)
        {
            for(int j = 0;j < partitionLocation[i].length - 1;j++)
            {
                MolapFile molapFile = FileFactory
                        .getMolapFile(
                                partitionLocation[i][j]
                                        + '/' + levelFileName,
                                FileFactory
                                        .getFileType(partitionLocation[i][j]
                                                + '/' + levelFileName));
                if(molapFile.exists())
                {
    ...
    private final class WriterThread implements Callable<Void>
    {
        ...
        private WriterThread(int[] key, int[] value, String path,
                String fileName, int currentMaxKey, int minValue)
        {
            this.key = key;
            this.value = value;
            this.path = path;
    ...
    private void writeGlobalSurrogateKeyFile(String string, int[] key,
            int[] value, String fileName, int currentMaxKey, int minValue)
    {
