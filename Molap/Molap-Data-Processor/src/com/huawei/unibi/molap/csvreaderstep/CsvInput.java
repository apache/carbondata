...
public class CsvInput extends BaseStep implements StepInterface
{
	...
	public CsvInput(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans)
	{
		...
		LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, "** Using csv file **");
		...
		try {
			...
			else 
			{
		        ...
		        if (checkFeedback(getLinesInput())) 
		        {
		        	if(log.isBasic()) 
		        	{
		        		logBasic(BaseMessages.getString(PKG, "CsvInput.Log.LineNumber", Long.toString(getLinesInput()))); //$NON-NLS-1$
		        	}
		        }
			}
		}
	...
	*/
	...
	private Object[] readOneRow(boolean doConversions) throws KettleException {
		...
		try {
			...
			Object[] outputRowData = RowDataUtil.allocateRowData(data.outputRowMeta.size()-RowDataUtil.OVER_ALLOCATE_SIZE);
			int outputIndex=0;
			...
			while (!newLineFound && outputIndex<meta.getInputFields().length) {
			  ...
			  */
						...
						do {
							...
							if (!keepGoing)
							{
								...
								else
								{
								     ...
								     * 'emp'aa','comab'
								     * 'empbb','com'cd'								     
								     * Here enclosure after emp(emp') and after com(com') are not the last enclosures 								     * 							     
								     * </pre>
								     */
								    keepGoing=!(data.delimiterMatcher.matchesPattern(data.byteBuffer, data.endBuffer, data.delimiter)||data.crLfMatcher.isReturn(data.byteBuffer, data.endBuffer) || data.crLfMatcher.isLineFeed(data.byteBuffer, data.endBuffer));
								}
				...
				if (escapedEnclosureFound>0)
				{
					if (log.isRowLevel()) 
						{
						logRowlevel("Escaped enclosures found in "+new String(field, Charset.defaultCharset()));
						}
					...
					if (meta.isLazyConversionActive()) {
						...
						try {
							...
							if (conversionExceptions==null) {
								conversionExceptions = new ArrayList<Exception>(MolapCommonConstants.CONSTANT_SIZE_TEN);
								exceptionFields = new ArrayList<ValueMetaInterface>(MolapCommonConstants.CONSTANT_SIZE_TEN);
							}
			...
			if (!newLineFound && !checkBufferSize()) 
			{
				do {
					...
					// HANDLE: if we're using quoting we might be dealing with a very dirty file with quoted newlines in trailing fields. (imagine that)
...
//      fieldnr = 0;
      ...
      while (pos < length)
      {
        ...
        if (lenEncl > 0 && line.substring(from, from + lenEncl).equalsIgnoreCase(enclosure))
        {
                      if (log.isRowLevel())
                    	  {
                    	  log.logRowlevel(BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRow",line.substring(from, from + lenEncl))); //$NON-NLS-1$ //$NON-NLS-2$
                    	  }
          enclFound = true;
          int p = from + lenEncl;
          ...
          if (p >= length) 
        	  ...
        	  next = p + lenEncl;
                      ...
                      if (log.isRowLevel()) 
                    	  {
                    	  log.logRowlevel(BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "CsvInput.Log.EndOfEnclosure", ""+ p)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    	  }
        }
        else
        {
        ...
        if (enclFound)
        {
          pol = line.substring(from + lenEncl, next - lenEncl);
                      if (log.isRowLevel()) 
                    	  {
                    	  log.logRowlevel(BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "CsvInput.Log.EnclosureFieldFound", ""+ pol)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    	  }
        }
        else
        {
          pol = line.substring(from, next);
                      if (log.isRowLevel()) 
                    	  {
                    	  log.logRowlevel(BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "CsvInput.Log.NormalFieldFound",""+ pol)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    	  }
        }
...
//        fieldnr++;
      ...
      if (pos == length)
      {
        if (log.isRowLevel()) 
        	{
        	log.logRowlevel(BaseMessages.getString(PKG, "CsvInput.Log.ConvertLineToRowTitle"), BaseMessages.getString(PKG, "CsvInput.Log.EndOfEmptyLineFound")); //$NON-NLS-1$ //$NON-NLS-2$
        	}
