...
/*	public Object clone()
	...
	private void readData(Node stepnode) throws KettleXMLException
	{
		try
		{
			...
			escapeEnclosure ="Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode, getXmlCode("ESACAPE_ENCLOSURE")));
	...
	public String getXML()
	{
		...
		retval.append("    ").append(XMLHandler.addTagValue(getXmlCode("ESACAPE_ENCLOSURE"), escapeEnclosure));
	...
	public void readRep(Repository rep, ObjectId idStep, List<DatabaseMeta> databases, Map<String, Counter> counters) throws KettleException
	{
      ...
      newlinePossibleInFields = rep.getStepAttributeBoolean(idStep, 0, getRepCode("NEWLINE_POSSIBLE"), !runningInParallel);
			...
			for (int i = 0; i < nrfields; i++)
			{
				...
				inputFields[i].setType(ValueMeta.getType(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_TYPE"))));
				inputFields[i].setFormat(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_FORMAT")));
				inputFields[i].setCurrencySymbol(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_CURRENCY")));
				inputFields[i].setDecimalSymbol(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_DECIMAL")));
				inputFields[i].setGroupSymbol(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_GROUP")));
				inputFields[i].setLength((int) rep.getStepAttributeInteger(idStep, i, getRepCode("FIELD_LENGTH")));
				inputFields[i].setPrecision((int) rep.getStepAttributeInteger(idStep, i, getRepCode("FIELD_PRECISION")));
				inputFields[i].setTrimType(ValueMeta.getTrimTypeByCode(rep.getStepAttributeString(idStep, i, getRepCode("FIELD_TRIM_TYPE"))));
			}
		}
		catch (Exception e)
		{
	...
	{ 
		try 
		{
			...
			rep.saveStepAttribute(idTransformation, idStep, getRepCode("INCLUDE_FILENAME"), includingFilename);
			...
			rep.saveStepAttribute(idTransformation, idStep, getRepCode("ESACAPE_ENCLOSURE"), escapeEnclosure);
			...
			rep.saveStepAttribute(idTransformation, idStep, getRepCode("LAZY_CONVERSION"), lazyConversionActive);
			rep.saveStepAttribute(idTransformation, idStep, getRepCode("ADD_FILENAME_RESULT"), isaddresult);
			rep.saveStepAttribute(idTransformation, idStep, getRepCode("PARALLEL"), runningInParallel);
      rep.saveStepAttribute(idTransformation, idStep, getRepCode("NEWLINE_POSSIBLE"), newlinePossibleInFields);
			...
			for (int i = 0; i < inputFields.length; i++)
			{
				...
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_NAME"), field.getName());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_TYPE"), ValueMeta.getTypeDesc(field.getType()));
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_FORMAT"), field.getFormat());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_CURRENCY"), field.getCurrencySymbol());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_DECIMAL"), field.getDecimalSymbol());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_GROUP"), field.getGroupSymbol());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_LENGTH"), field.getLength());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_PRECISION"), field.getPrecision());
				rep.saveStepAttribute(idTransformation, idStep, i, getRepCode("FIELD_TRIM_TYPE"), ValueMeta.getTrimTypeCode(field.getTrimType()));
			}
		}
		catch (Exception e)
		{
    ...
    public void injectStepMetadataEntries(List<StepInjectionMetaEntry> metadata) {
      for (StepInjectionMetaEntry entry : metadata) {
		...
		if (entry.getValueType()!=ValueMetaInterface.TYPE_NONE) {
          if ("FILENAME".equals(attributeKey)) { filename = (String) entry.getValue(); } else
          if ("FILENAME_FIELD".equals(attributeKey)) { filenameField = (String) entry.getValue(); } else
          ...
          if ("currentRestructNumber".equals(attributeKey)) { currentRestructNumber = (Integer) entry.getValue(); } else
          { 
          ...
          if ("FIELDS".equals(attributeKey)) {
            ...
            for (int row=0;row<inputFieldEntries.size();row++) {
              ...
              for (int i=0;i<fieldAttributes.size();i++) {
				...
				if ("FIELD_NAME".equals(fieldAttributeKey)) { inputField.setName(attributeValue); } else 
                if ("FIELD_TYPE".equals(fieldAttributeKey)) { inputField.setType(ValueMeta.getType(attributeValue)); } else 
                if ("FIELD_FORMAT".equals(fieldAttributeKey)) { inputField.setFormat(attributeValue); } else 
                if ("FIELD_LENGTH".equals(fieldAttributeKey)) { inputField.setLength(attributeValue==null ? -1 : Integer.parseInt(attributeValue)); } else 
                if ("FIELD_PRECISION".equals(fieldAttributeKey)) { inputField.setPrecision(attributeValue==null ? -1 : Integer.parseInt(attributeValue)); } else 
                if ("FIELD_CURRENCY".equals(fieldAttributeKey)) { inputField.setCurrencySymbol(attributeValue); } else 
                if ("FIELD_DECIMAL".equals(fieldAttributeKey)) { inputField.setDecimalSymbol(attributeValue); } else 
                if ("FIELD_GROUP".equals(fieldAttributeKey)) { inputField.setGroupSymbol(attributeValue); } else 
                if ("FIELD_TRIM_TYPE".equals(fieldAttributeKey)) { inputField.setTrimType(ValueMeta.getTrimTypeByCode(attributeValue)); } else
                {
