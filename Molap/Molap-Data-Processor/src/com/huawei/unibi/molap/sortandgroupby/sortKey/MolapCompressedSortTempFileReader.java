...
public class MolapCompressedSortTempFileReader extends AbstractSortTempFileReader
{
    ...
    private static final LogService LOGGER = LogServiceFactory
            .getLogService(MolapCompressedSortTempFileReader.class.getName());
            ...
            for(int i = 0;i < measureCount;i++)
            {
                ...
                byteArrayInputStream[i] = new ByteArrayInputStream(
                        SnappyByteCompression.INSTANCE.unCompress(fileHolder
                                .readByteArray(filePath, readInt)));
                dataInputStream[i] = new DataInputStream(
                        byteArrayInputStream[i]);
            }
            readInt = fileHolder.readInt(filePath);
            mdkey = SnappyByteCompression.INSTANCE.unCompress(fileHolder
                    .readByteArray(filePath, readInt));
            factMdkey = null;
     ...
     * @return
     */
    private Object[][] prepareRecordFromStream(int recordSize,
            DataInputStream[] measureBuffer, byte[] completeMdkey,
            byte[] completeMdkeyFactMdkey)
    {
        ...
        try
        {
            for(int i = 0;i < recordSize;i++)
            {
                ...
                System.arraycopy(completeMdkey, mdkeyPosition, mdkey, 0,
                        mdKeyLenght);
                ...
                if(isFactMdkeyInSort)
                {
                    ...
                    System.arraycopy(completeMdkeyFactMdkey, factMdkeyPosition,
                            mdkey, 0, factMdkeyLength);
