...
public class MolapSeqGenStep extends BaseStep implements StepInterface {
    ...
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(MolapSeqGenStep.class.getName());
    /**
    ...
    private Map<String, HierarchyValueWriter> nrmlizedHierWriterMap =
            new HashMap<String, HierarchyValueWriter>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    /**
     * File manager
     */
     ...
     * @param dis
     */
    public MolapSeqGenStep(StepMeta s, StepDataInterface stepDataInterface, int c, TransMeta t,
            Trans dis) {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        try {
            ...
            if (r == null) {
                ...
                String logMessage =
                        "Summary: Molap DB Based Seq Gen Step: Read: " + readCounter + ": Write: "
                                + writeCounter;
            ...
            if (first) {
                ...
                molapInfo.setBaseStoreLocation(
                        updateStoreLocationAndPopulateMolapInfo(meta.getStoreLocation()));
                molapInfo.setTableName(meta.getTableName());
                molapInfo.setDimsPresent(meta.dimPresent);

                if (meta.timeIndex != -1) {
                    molapInfo
                            .setTimDimIndexEnd(molapInfo.getTimDimIndex() + meta.timeLevels.length);
                }

                molapInfo.setTimeOrdinalIndices(meta.timeOrdinalIndices);

                data.setSurrogateKeyGen(
                        new FileStoreSurrogateKeyGen(molapInfo, meta.getCurrentRestructNumber()));
    ...
    private void handleDimWithTime() {
        ValueMetaInterface[] metaInterface =
                new ValueMetaInterface[data.getOutputRowMeta().size() + meta.timeLevels.length - 1
                        + 1];
    ...
    private void updateAndWriteSliceMetadataFile() throws KettleException {
        String storeLocation = updateStoreLocationAndPopulateMolapInfo(meta.getStoreLocation());
        //
        ...
        String sliceMetaDataFilePath =
                storeLocation + File.separator + MolapCommonConstants.RESTRUCTRE_FOLDER
                        ...
                        + File.separator + MolapUtil.getSliceMetaDataFileName(restructFolderNumber);
        ...
        if (meta.timeIndex >= 0) {
            ...
            try {
                getTimeValue(outputRow, data.getOutputRowMeta().getValueMeta(meta.timeDimeIndex)
                                .getString((byte[]) r[meta.timeIndex]), timeOrdinalColValues,
                        meta.timeIndex);
            } catch (Exception e) {
    ...
    private void closeNormalizedHierFiles() throws KettleException {
        ...
        for (int i = 0; i < hierLen; i++) {
            ...
            HierarchyValueWriter hierarchyValueWriter =
    ...
    private void updateHierarichiesFromMetaDataFile(List<HierarchiesInfo> metahierVoList)
        ...
        try {
            for (int i = 0; i < metahierVoList.size(); i++) {
                ...
                if (null
                        == query) // table will be denormalized so no foreign key , primary key for this hierarchy
    ...
    public synchronized void connect(String group, String partitionId, Database database)
        ...
        if (!Const.isEmpty(group)) {
            ...
            connection =
                    ConnectionPoolUtil.getConnection(log, database.getDatabaseMeta(), partitionId);
        }
