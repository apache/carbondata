...
import com.huawei.unibi.molap.dimension.load.command.impl.CSVDimensionLoadCommand;
import com.huawei.unibi.molap.dimension.load.command.impl.DimenionLoadCommandHelper;
import com.huawei.unibi.molap.dimension.load.command.invoker.DimensionLoadActionInvoker;
...
public class MolapCSVBasedSeqGenStep extends BaseStep
{
     ...
     * LOGGER
     */
    private static final LogService LOGGER = LogServiceFactory.getLogService(MolapCSVBasedSeqGenStep.class.getName());
     ...
     * MolapSeqGenData
     */
    private MolapCSVBasedSeqGenData data;
     ...
     */
    private Map<String, HierarchyValueWriterForCSV> nrmlizedHierWriterMap = new HashMap<String, HierarchyValueWriterForCSV>(16);
...
//    private boolean isDBFactLoad;
    ...
    /**
     * MolapCSVBasedDimSurrogateKeyGen
     */
...
//    private static boolean isDone = false;
    ...
    static 
    {
        ...
        DRIVERS.put("com.microsoft.sqlserver.jdbc.SQLServerDriver", MolapCommonConstants.TYPE_MSSQL);
     ...
     * @param dis
     */
    public MolapCSVBasedSeqGenStep(StepMeta s, StepDataInterface stepDataInterface,
...
//                    isDBFactLoad = true;
                ...
                buffer = Integer.parseInt(MolapProperties.getInstance().getProperty(
                        MolapCommonConstants.SORT_SIZE,
                        MolapCommonConstants.SORT_SIZE_DEFAULT_VAL));
                
                checkPointSize = buffer/Integer.parseInt(MolapProperties.getInstance()
...
//                updateNullValueComparer();
    ...
    //            molapInfo.setConnectionString(meta.getMolapJNDIName());
                ...
                molapInfo.setDimHierRel(meta.getDimTableArray());
                molapInfo.setBaseStoreLocation(updateStoreLocationAndPopulateMolapInfo(meta.getSchemaName() +'/' +meta.getCubeName()));
                ...
                badRecordslogger = new BadRecordslogger(key, csvFilepath, getBadLogStoreLocation(meta.getSchemaName() +'/' +meta.getCubeName()));
                
                molapInfo.setTimeOrdinalIndices(meta.timeOrdinalIndices);
                surrogateKeyGen = new FileStoreSurrogateKeyGenForCSV(molapInfo, meta.getCurrentRestructNumber());
               ...
               if(null != getInputRowMeta())
                    {
                        ...
                        for(int i=0; i < meta.actualDimArray.length; i++)
                        {
                            ...
                            {
                                ValueMetaInterface x = new ValueMeta(
                                        meta.actualDimArray[i],
                                        ValueMetaInterface.TYPE_STRING,
                                        ValueMetaInterface.STORAGE_TYPE_BINARY_STRING);
                                x.setStorageMetadata((new ValueMeta(
                                        meta.actualDimArray[i],
                                        ValueMetaInterface.TYPE_STRING,
                                        ValueMetaInterface.STORAGE_TYPE_NORMAL)));
                                x.setStringEncoding(BYTE_ENCODING);
                                x.setStringEncoding(BYTE_ENCODING);
                                x.getStorageMetadata().setStringEncoding(
                                        BYTE_ENCODING);
                        ...
                        for(int j=0; j< meta.measureColumn.length; j++)
                        {
                            ...
                            {
                                if(meta.measureColumn[j].equalsIgnoreCase(data
    ...
    //                                        .getValueMeta(k);
                        ...
                        if(CheckPointHanlder.IS_CHECK_POINT_NEEDED)
                        {
                            ...
                            out[outCounter++] = data.getOutputRowMeta().getValueMeta(data.getOutputRowMeta().size()-2);
                            out[outCounter++] = data.getOutputRowMeta().getValueMeta(data.getOutputRowMeta().size()-1);
                            
                        }
                        
                        data.getOutputRowMeta()
                                .setValueMetaList(Arrays.asList(out));
...
//        MolapUtil.writeLevelCardinalityFile(loadFolderLoc, meta.getTableName(), data.getSurrogateKeyGen().max);
        ...
        String logMessage= "Summary: Molap CSV Based Seq Gen Step : " + readCounter + ": Write: "+ writeCounter;
...
//                }
    ...
    private boolean processWhenRowIsNull() throws KettleException
    {
        ...
        if(data.getSurrogateKeyGen() == null)
        {
            setOutputDone();
            LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, "Record Procerssed For table: "+ meta.getTableName());
            String logMessage= "Summary: Molap CSV Based Seq Gen Step:  Read: " + readCounter + ": Write: "+ writeCounter;
            LOGGER.info(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, logMessage);
            return false;
        }
        try
...
//    }
    ...
    private void updatePropMap(String[] actualDimArray)
    {
        ...
        for(int i = 0;i < getInputRowMeta().size();i++)
        {
            ...
            if(null != hier)
            {
                if(hier.indexOf(MolapCommonConstants.COMA_SPC_CHARACTER) > -1)
                {
                    String[] splittedHiers = hier.split(MolapCommonConstants.COMA_SPC_CHARACTER);
        ...
        String[] currentColNamesArray = currentColName
                .toArray(new String[currentColName.size()]);
...
//    }
    ...
    private void closeNormalizedHierFiles() throws KettleException
    {
        ...
        for(int i = 0;i < hierLen;i++)
        {
            ...
            HierarchyValueWriterForCSV hierarchyValueWriter = nrmlizedHierWriterMap
            ...
            List<ByteArrayHolder> holders = hierarchyValueWriter
                    .getByteArrayList();
            Collections.sort(holders);

            for(ByteArrayHolder holder : holders)
            {
                hierarchyValueWriter.writeIntoHierarchyFile(holder.getMdKey(),
                        holder.getPrimaryKey());
            }

            // now write the byte array in the file.
            FileChannel bufferedOutStream = hierarchyValueWriter
            ...
            hierInProgressFileName = changedFileName + MolapCommonConstants.FILE_INPROGRESS_STATUS;
    ...
    private void updateStoreLocation()
    {
        ...
        int loadCounter = MolapUtil.checkAndReturnCurrentLoadFolderNumber(store);
...
//                        System.out.println("Time taken to complete the thread pool : --" + (System.currentTimeMillis() -startTime));
                ...
                {
                    LOGGER.error(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, e, "Thread is terminated due to error");
...
//        }
    ...
    {
        String storeLocation = updateStoreLocationAndPopulateMolapInfo(meta.getSchemaName() +'/' +meta.getCubeName());
        //
        ...
        String sliceMetaDataFilePath = storeLocation + File.separator
                ...
                + MolapUtil.getSliceMetaDataFileName(meta.getCurrentRestructNumber());
...
//                    }
        ...
        {
            LOGGER.error(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, "Exception happened while processing rows in Do Process", e);
            throw e;
        }
        catch(Exception e)
        {
        ...
        {
            LOGGER.error(MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG, "Exception happened while processing rows in Do Process", t);
...
//    }
    ...
    { 
        //
        String tempLocationKey = meta.getSchemaName()+'_'+meta.getCubeName();
        String strLoc = MolapProperties.getInstance().getProperty(
                tempLocationKey,
                MolapCommonConstants.STORE_LOCATION_DEFAULT_VAL);
    ...
    private Object[] changeNullValueToNullString(Object[] rowValue)
    {
        ...
        for(Object obj : rowValue)
        {
            if(obj != null)
            {
                ...
                {
                    rowValue[i] = MolapCommonConstants.MEMBER_DEFAULT_VAL;
                }
            }
...
//        boolean needToUpdateHierCache = false;
		...
		Object[] out = new Object[meta.normLength + meta.msrs.length - meta.complexTypes.size() + checkPoint.getCheckPointInfoFieldCount()];
...
//                  Boolean isPresent = meta.getMeasureSurrogateRequired().get(columnName);
                if(measureSurrogateReqMapping[j] && !isNull)
                {
                    ...
                    else
                    {
                        surrogate = surrogateKeyGen
                                .generateSurrogateKeys(msr,
                                        foreignKeyColumnName, n,
                                        new Object[0]);
                    }
                ...
                else
                {
                    ...
                    try
                    {
                        out[memberMapping[dimLen - meta.complexTypes.size() + index]] = (isNull || msr == null || msr.length() == 0) ? null
                                        : Double.valueOf(msr);
                    }
            ...
            if(foreignKeyMappingColumnsForMultiple[j] != null)
            {
                    ...
                    {
                        Map<String, Integer> memberCache = allMemberCache
                                .get(hierForignKey);
                        ...
                        if(null != cache)
                        {
                             ...
                             if(null != keyFromCsv)
                            {
                                surrogateKeyForHierarchy = cache
                                        .get(keyFromCsv);
                            }
                            else
                            {
                        ...
                        for(int k = 0;k < surrogateKeyForHierarchy.length;k++)
                        {
                            ...
                            {
                                out[memberMapping[i]] = Integer
                                        .valueOf(surrogateKeyForHierarchy[k]);
                            }
...
//						out[memberMapping[i]] = byteArray.toByteArray();
        ...
        RemoveDictionaryUtil.prepareOut(newArray, byteBufferArr, out, dimLen - meta.complexTypes.size());
    ...
    private int[]  createPresentColumnMapIndex()
    {
        ...
        {
            String columnName = getInputRowMeta().getValueMeta(j)
                    .getName();
...
//              }
    ...
    private String[] createColumnArrayFromMeta()
    {
        ...
        for(int j = 0;j < getInputRowMeta().size();j++)
        {
            metaColumnNames[j] = getInputRowMeta().getValueMeta(j)
                    .getName();
        }
    ...
    private void createForeignKeyMappingColumns()
    {
        ...
        for(int j = 0;j < size;j++)
        {
            String columnName = getInputRowMeta().getValueMeta(j)
                    .getName();
            String foreignKeyColumnName = meta.foreignKeyPrimaryKeyMap
                    .get(columnName);
            if(foreignKeyColumnName != null)
            {
                if(foreignKeyColumnName.indexOf(MolapCommonConstants.COMA_SPC_CHARACTER) > -1)
                {
                    String[] splittedHiers = foreignKeyColumnName
...
//                        + MolapCommonConstants.FILE_INPROGRESS_STATUS;
                ...
                if(null == hierWriter)
                {
                    FileData fileData = new FileData(hierInprogName,
                            storeLocation);
                    hierWriter = new HierarchyValueWriterForCSV(hierInprogName,
                            storeLocation);
        ...
        for(int i = 0;i < currentColNamesArray.length;i++)
        {
            ...
            for(int j = 0;j < meta.actualDimArray.length;j++)
            {
                ...
                if (currentColNamesArray[i]
                        .equalsIgnoreCase(meta.dimColNames[j])) 
                {
...
//        }
    ...
    public int calculateMeasureOriginalIndexes(int[] memIndexes, String[] currentColNamesArray,
            int dimIndex, int toAddInIndex, int k,
            Map<String, Boolean> existsMap, int i) 
    {
        for(int j = 0;j < meta.measureColumn.length;j++)
        {
            if(currentColNamesArray[i].equalsIgnoreCase(meta.getTableName()
                    + '_' + meta.measureColumn[j]))
            {
    ...
    {       
        ...
        if(null != dimTableFileLoc && !dimTableFileLoc.isEmpty())
        {
          dimensionLoadInfo.setDimTableNames(DimenionLoadCommandHelper.getInstance()
           ...
           loadCommand = new CSVDimensionLoadCommand(dimensionLoadInfo, meta.getCurrentRestructNumber(), surrogateKeyGen.dimensionWriter);
            ...
            throws Exception
    {
        ...
        if (!Const.isEmpty(group))
        {
                                ...
                                // partition & database in this group
            {
                ...
                // later re-use.
                conn = ConnectionPoolUtil.getConnection(log ,
...
//       String[] tables = tableNames.split("&");
    ...
    public void dispose(StepMetaInterface smi, StepDataInterface sdi)
    {
        ...
        meta = (MolapCSVBasedSeqGenMeta)smi;
        data = (MolapCSVBasedSeqGenData)sdi;        
        MolapCSVBasedDimSurrogateKeyGen  surKeyGen = data.getSurrogateKeyGen();
        
        try
        {
            closeConnections();
            boolean isCacheEnabled = Boolean
            ...
            if(null != surKeyGen)
            {
                    ...
                    if(isCacheEnabled)
                    {
                        MolapSeqGenCacheHolder holder = new MolapSeqGenCacheHolder();
                        holder.setHierCache(surKeyGen.getHierCache());
                        holder.setHierCacheReverse(surKeyGen
                                .getHierCacheReverse());
                        ...
                        String cacheKey = meta.getSchemaName()
                                + '_'
                                + meta.getCubeName();
                        LOGGER.info(
                                MolapDataProcessorLogEvent.UNIBI_MOLAPDATAPROCESSOR_MSG,
...
//        }
	...
	private void writeDataFileVersion() throws KettleException {
		...
		try {
			versionWriter = new FileWriter(loadFolderLoc + File.separator
					+ ".version");
			versionWriter.write(MolapCommonConstants.DATA_VERSION + '='
