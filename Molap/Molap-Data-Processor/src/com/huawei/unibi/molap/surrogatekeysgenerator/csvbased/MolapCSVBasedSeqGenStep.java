...
public class MolapCSVBasedSeqGenStep extends BaseStep {
    ...
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(MolapCSVBasedSeqGenStep.class.getName());
    /**
     ...
     * @param dis
     */
    public MolapCSVBasedSeqGenStep(StepMeta s, StepDataInterface stepDataInterface, int c,
        ...
        try {
            ...
            if (first) {
                ...
                synchronized (dataProcessingLockObject) {
                    ...
                    buffer = Integer.parseInt(MolapProperties.getInstance()
                            .getProperty(MolapCommonConstants.SORT_SIZE,
                    ...
                    molapInfo.setBaseStoreLocation(updateStoreLocationAndPopulateMolapInfo(
                    ...
                    surrogateKeyGen = new FileStoreSurrogateKeyGenForCSV(molapInfo,
                    ...
                    if (null != r) {
                        MolapCSVBasedDimSurrogateKeyGen surrogateKeyGenObj =
            ...
            MolapUtil.writeLevelCardinalityFile(loadFolderLoc, meta.getTableName(),
    ...
    private List<String> getDenormalizedHierarchies() {
        ...
        for (Iterator<Map.Entry<String, int[]>> iterator =
    ...
    private void updateStoreLocation() {
        ...
        String store = MolapProperties.getInstance()
        ...
        store = store + File.separator + MolapCommonConstants.RESTRUCTRE_FOLDER + rsCounter
                + File.separator + meta.getTableName();

        int loadCounter = MolapUtil.checkAndReturnCurrentLoadFolderNumber(store);

        loadFolderLoc = store + File.separator + MolapCommonConstants.LOAD_FOLDER + loadCounter
    ...
    private String updateStoreLocationAndPopulateMolapInfo(String schemaCubeName) {
        //
        String tempLocationKey = meta.getSchemaName() + '_' + meta.getCubeName();
        String strLoc = MolapProperties.getInstance()
                .getProperty(tempLocationKey, MolapCommonConstants.STORE_LOCATION_DEFAULT_VAL);
            ...
            if (foreignKeyColumnName != null) {
                if (foreignKeyColumnName.indexOf(MolapCommonConstants.COMA_SPC_CHARACTER) > -1) {
                    String[] splittedHiers =
                            foreignKeyColumnName.split(MolapCommonConstants.COMA_SPC_CHARACTER);
    ...
    private void insertHierarichies(Object[] rowWithKeys) throws KettleException {
        ...
        try {
            for (String hierName : denormHierarchies) {
                ...
                String hierInprogName = hierName + MolapCommonConstants.HIERARCHY_FILE_EXTENSION;
    ...
    private void writeDataFileVersion() throws KettleException {
        ...
        try {
            ...
            versionWriter
                    .write(MolapCommonConstants.DATA_VERSION + '=' + MolapVersion.getDataVersion());
            versionWriter.flush();
        } catch (IOException e) {
    ...
    private boolean isDimensionHighCardinality(String[] highCardDims, String columnName) {
        for (String colName : highCardDims) {
            if (colName.equalsIgnoreCase(
                    meta.getTableName() + MolapCommonConstants.UNDERSCORE + columnName)) {
                return true;
            }
        }
        return false;
