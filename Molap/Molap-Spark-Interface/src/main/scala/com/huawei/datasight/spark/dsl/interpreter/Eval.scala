...
import com.huawei.datasight.spark.dsl.interpreter.string._
import java.io._
...
class Eval(target: Option[File]) {
  ...
  /**
    * val i: Int = new Eval()("1 + 1") // => 2
    */
  def getInstance[T](code: String): T = {
    val processed = sourceForString(code)
    getInstance(processed, true)
  }
  ...
  def getInstance[T](code: String, resetState: Boolean): T = {
    val id = uniqueId(code)
    val className = "Evaluator__" + id
    val cls = compiler(wrapCodeInClass(className, code), "com.huawei.datasight.spark.dsl.interpreter.temp." + className, resetState)
  ...
  private def wrapCodeInClass(className: String, code: String) = {
    ...
    "package com.huawei.datasight.spark.dsl.interpreter.temp \n" +
      ...
      "import com.huawei.datasight.spark.rdd.SqlUdf \n" +
      "import org.apache.spark.sql.catalyst.plans._ \n" +
      "import org.apache.spark.sql.catalyst.expressions._ \n" +
      "class " + className + " extends SparkEvaluator {\n" +
      ...
      "var curr = regAttrModel.trans.rdd.asInstanceOf[SchemaRDD] \n" +
      code + "\n" +
      "TransformHolder(curr,regAttrModel.trans.mataData)" + "\n" +
      "  }\n" +
      "}\n"
  }
