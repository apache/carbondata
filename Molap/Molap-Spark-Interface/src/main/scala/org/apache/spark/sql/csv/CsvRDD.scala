...
object CsvRDD extends Logging {
  ...
  private[sql] def inferSchema(
                                ...
                                useHeader: Boolean)(@transient sqlContext: SQLContext): LogicalPlan = {
    ...
    val schema = if (userSchema == null) {
      ...
      val header = if (useHeader) {
        logInfo(s"Using header line: $firstLine")
        firstRow
      } else {
    ...
    logInfo(s"Parsing CSV with $numFields.")
    ...
    LogicalRDD(asAttributes(schema), parsedCSV)(sqlContext)
  }
  ...
  private def parseCSV(
                        ...
                        row: GenericMutableRow): Iterator[InternalRow] = {
    iter.map { tokens =>
