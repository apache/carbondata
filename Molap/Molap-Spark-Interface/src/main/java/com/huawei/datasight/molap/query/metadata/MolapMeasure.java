...
public class MolapMeasure implements MolapColumn, Serializable
{
	 ...
	 * If user does not mention any aggregate type then by default count() would be taken as aggregator. 
	 */
	 ...
	 * User can mention sort order to only one measure . if user selects sort order to multiple measures then last measures 
	 * sort order only be taken.
	 */
	 ...
	 * @param measure
	 */
	public MolapMeasure(String measure)
	{
		String aggName = null;
	...
	public boolean equals(Object obj) 
	{
		...
		MolapMeasure other = (MolapMeasure) obj;
		if (measure == null) {
	...
	{
		SUM("SUM"),COUNT("COUNT"),AVG("AVG"),MAX("MAX"),MIN("MIN"),DISTINCT_COUNT("DISTINCT-COUNT"),SUM_DISTINCT("SUM-DISTINCT"),INSTANCE("INSTANCE"),CUSTOM("CUSTOM");
		...
		public AggregatorType getAggType(String aggType)
		{
			...
			else if("countmolap".equalsIgnoreCase(aggType) || "count".equalsIgnoreCase(aggType))
			{
				return COUNT;
			}
			...
			else if(aggType.equalsIgnoreCase("CountDistinctMolap") || aggType.equalsIgnoreCase("distinct_count"))
			{
				return DISTINCT_COUNT;
			}
			else if(("MinMolap").equalsIgnoreCase(aggType) || ("min").equalsIgnoreCase(aggType))
			{
				return MIN;
			}
			else if(("MaxMolap").equalsIgnoreCase(aggType) || ("max").equalsIgnoreCase(aggType))
			{
				return MAX;
			}
			else if("SumDistinctMolap".equalsIgnoreCase(aggType) || "sum-distinct".equalsIgnoreCase(aggType))
			{
				return SUM_DISTINCT;
			}
