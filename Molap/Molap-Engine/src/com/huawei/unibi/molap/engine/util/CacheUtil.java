...
public final class CacheUtil
{
     ...
     * LOGGER
     */
    private static final LogService LOGGER = LogServiceFactory.getLogService(CacheUtil.class.getName());
     ...
     */
    public static Member [][] getMembersList(String filesLocaton, byte nameColumnIndex, String dataType)
    {
        LOGGER.debug(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Reading members data from location: " + filesLocaton);
//        File decryptedFile = decryptEncyptedFile(filesLocaton);
    ...
    public static int getMinValue(String filesLocaton)
    {
        ...
        try
        {
            ...
            fileChannel = new DataInputStream(FileFactory.getDataInputStream(filesLocaton,FileFactory.getFileType(filesLocaton),10240));
            fileChannel.readInt();
...
//            e.printStackTrace();
    ...
    public static int getMaxValueFromLevelFile(String filesLocaton)
    {
        ...
        try
        {
            ...
            fileChannel = new DataInputStream(FileFactory.getDataInputStream(filesLocaton,FileFactory.getFileType(filesLocaton),10240));
            MolapFile memberFile=FileFactory.getMolapFile(filesLocaton, FileFactory.getFileType(filesLocaton));
...
//            e.printStackTrace();
    ...
    public static int[] getGlobalSurrogateMapping(String filesLocaton)
    {
        ...
        try
        {
            ...
            fileChannel = FileFactory.getDataInputStream(filesLocaton, FileFactory.getFileType(filesLocaton));
            fileChannel.readInt();
...
//            e.printStackTrace();
    ...
    public static Map<Integer,Integer> getGlobalSurrogateMappingMapBased(String filesLocaton)
    {
        ...
        DataInputStream fileChannel = null;
        Map<Integer,Integer> map = new HashMap<Integer, Integer>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        try
        {
            if(!FileFactory.isFileExist(filesLocaton, FileFactory.getFileType(filesLocaton)))
            {
...
//    }
           ...
           
    {
        ...
        {
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "@@@@@@  Member file is missing @@@@@@ : " + filename);
        }catch (IOException e) {
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "@@@@@@  Error while reading Member the file @@@@@@ : " + filename);
        }
        finally
        {
                MolapUtil.closeStreams(fileChannel);
        ...
        LOGGER.debug(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Time taken to process file " +filename +" is : "+(System.currentTimeMillis()-startTime));
        return members;
    }
    
    public static void processMemberFileNewImpl(int nameColumnIndex, String filename, Int2ObjectMap<Member> members, String dataType)
    
    {
        ...
        {
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "@@@@@@  Member file is missing @@@@@@ : "
                    + filename);
        }
        catch(IOException e)
        {
        ...
        LOGGER.debug(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Time taken to process file " + filename + " is : "
                + (System.currentTimeMillis() - startTime));

    }

    private static Member [][] populateMemberCache(
            DataInputStream fileChannel,MolapFile memberFile,String fileName,String dataType) throws IOException
    {
...
//        LOGGER.debug(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Bytes skipped " + skipSize);
        ...
        if(totalArraySize>MolapCommonConstants.LEVEL_ARRAY_SIZE)
        {
            ...
            {
                surogateKeyArrays[surogateKeyArrays.length-1]= new Member[MolapCommonConstants.LEVEL_ARRAY_SIZE];
            }
        }
        else
        {
...
//        int surrogateKeyIndex = minVal;
        ...
        while(currPositionIndex < size)
        {
            ...
            surogateKeyArrays[current/MolapCommonConstants.LEVEL_ARRAY_SIZE][index] = new Member(rowBytes);
            current++;
     ...
     * @throws IOException
     */
    public static List<int[][]> getLevelSortOrderAndReverseIndex(String levelFileName) throws IOException
    {
        if(!FileFactory.isFileExist(levelFileName
                + MolapCommonConstants.LEVEL_SORT_INDEX_FILE_EXT, FileFactory.getFileType(levelFileName
                        + MolapCommonConstants.LEVEL_SORT_INDEX_FILE_EXT)))
        {
            return null;
        }
        DataInputStream dataInputStream = null;
        List<int[][]> sortIndexAndReverseIndexArray = new ArrayList<int[][]>(
                MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        long size = 0;
        ByteBuffer buffer = null;
        try
        {
            dataInputStream = FileFactory.getDataInputStream(levelFileName
                    + MolapCommonConstants.LEVEL_SORT_INDEX_FILE_EXT, FileFactory.getFileType(levelFileName));
            size = FileFactory.getMolapFile(levelFileName + MolapCommonConstants.LEVEL_SORT_INDEX_FILE_EXT,
    ...
    private static int[][] getIndexArray(ByteBuffer buffer)
    {
        ...
        if(arraySize > MolapCommonConstants.LEVEL_ARRAY_SIZE)
        {
            ...
            {
                sortorderIndexArray[sortorderIndexArray.length - 1] = new int[MolapCommonConstants.LEVEL_ARRAY_SIZE];
            }
        }
        int index = 0;
        int prvArrayIndex = 0;
        ...
        {
            sortorderIndexArray[current / MolapCommonConstants.LEVEL_ARRAY_SIZE][index] = buffer.getInt();
            current++;
            if(current / MolapCommonConstants.LEVEL_ARRAY_SIZE > prvArrayIndex)
            {
                prvArrayIndex++;
     ...
     * @return
     */
    public static List<long[]> getHierarchiesList(String filesLocaton, int keyLength, KeyGenerator keyGen)
    {
        LOGGER.debug(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Reading hierarchies from location: " + filesLocaton);
        ...
        {
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Error while reading the file: " + filesLocaton);
        }
        finally
        {
            try
            ...
            {
                LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Error while closing the file stream for file " + filesLocaton);
            }
        }
    ...
    public static boolean checkAnyIncludeOrExists(Map<Dimension, MolapFilterInfo> constraints)
    {
        for(Map.Entry<Dimension, MolapFilterInfo> entry : constraints.entrySet())
        {
            // check if filter size is greater than zero, if it is more than
            // zero than include filter is present
            if(entry.getValue().getIncludedOrMembers() != null && entry.getValue().getIncludedOrMembers().size() > 0)
            {
                return true;
            }
        }
     ...
     * 
     */
    public static String getLevelActualName(MolapDef.Schema schema, MolapDef.CubeDimension dimension)
    {
        String levelActualName = null;
        com.huawei.unibi.molap.olap.MolapDef.Hierarchy[] extractHierarchies = MolapSchemaReader.extractHierarchies(
                schema, dimension);
