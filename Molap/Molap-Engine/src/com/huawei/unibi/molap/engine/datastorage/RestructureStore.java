...
public class RestructureStore implements Comparable<RestructureStore>
{
     ...
     * 
     */
    private Map<String, List<InMemoryCube>> slices = new ConcurrentHashMap<String, List<InMemoryCube>>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    
    /**
     * Map of table name and SliceMetaData (Fact table, all aggregate tables)
     */
    private Map<String, SliceMetaData> sliceMetaCacheMap = new ConcurrentHashMap<String, SliceMetaData>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    
    private Map<String, String> sliceMetaPathMap = new ConcurrentHashMap<String, String>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    ...
    public  synchronized void setSlice(InMemoryCube slice, String tableName)
    {
        ...
        for(Iterator<InMemoryCube> iterator = sliceList.iterator();iterator
                .hasNext();)
        {
            InMemoryCube loadedSlice = iterator.next();
            if(loadedSlice.getCubeSlicePathInfo().getLoadPath().equals(slice.getCubeSlicePathInfo().getLoadPath()))
            {
                iterator.remove();
                break;
            }
