...
public abstract class AbstractMolapExecutor implements MolapExecutor
{
     ...
     *
     */
    private void setStartKey(long[] key, long[][] incldPredKeys,long[][] incldOrPredKeys, long[][] excldPredKeys)
            throws IOException
    {
     ...
     * 
     * @see com.huawei.unibi.molap.engine.executer.MolapExecutor#setStartAndEndKeys(long[], long[], long[][], long[][], com.huawei.unibi.molap.metadata.MolapMetadata.Dimension[])
     *
     */
    public void setStartAndEndKeys(long[] startKey, long[] endKey, long[][] incldPredKeys,long[][] incldOrPredKeys,
     ...
     *
     */
    private void setEndKey(long[] key, long[][] incldPredKeys,long[][] incldOrPredKeys,long[][] excldPredKeys,
            Dimension[] tables) throws IOException
    {
        ...
        for(int i = 0;i < key.length;i++)
        {
            ...
            if(incld || incldOr)
            {
                ...
                {
                    incldOrGlobal = true;
                    key[i] = incldOrConstraints[incldOrConstraints.length - 1]>key[i]?incldOrConstraints[incldOrConstraints.length - 1]:key[i];
                }
            ...
            else
            {
                ...
                {
                    for(int filterIndex = excldConstraints.length - 1;filterIndex >= 0;filterIndex--)
                    {
...
//        return new MultiDimKeyVarLengthEquiSplitGenerator(MolapUtil.getIncrementedCardinalityFullyFilled(lens),(byte)1);
     ...
     *
     */
    protected boolean constraintsExistsOnAllDimensions(Map<Dimension, MolapFilterInfo> constraints)
    {
        for(Entry<Dimension, MolapFilterInfo> dimensionConstraint : constraints.entrySet())
        {
            MolapFilterInfo info = dimensionConstraint.getValue();
     ...
     *
     */
    protected void addPointKeyInList(long[][] predKeys, int count, long[] builder, List<long[]> result)
    {
        //
        count++;
