...
public class BlockWriterProcessor implements DataProcessorExt
{
    ...
    public void initialise(DataProcessorInfo model) throws DataProcessorException
    {
        ...
        this.dataWriter = new MolapDataWriter(outputLocation, model.getAggType().length,
                model.getKeySize(), queryId, MolapCommonConstants.QUERY_MERGED_FILE_EXT, false, false);
        try
        {
    ...
    private void createMsrDataOutStrms(int length)
    {
        ...
        {
            this.blockDataArray[i] = new ByteArrayOutputStream(model.getBlockSize()
    ...
    public void finish() throws DataProcessorException
    {
        ...
        finally
        {
            ...
            LOGGER.info(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, "Total row count for query " + queryId + " : "
                    + rowCount);
        }
    }
