...
import com.huawei.unibi.molap.engine.executer.impl.measure.filter.MeasureFilterProcessor;
...
public class TopNProcessorBytes implements DataProcessor//,Callable< Map<ByteArrayWrapper, MeasureAggregator[]>>
{
     ...
     * groups
     */
    private List<TopNHolderGroup> groups = new ArrayList<TopNProcessorBytes.TopNHolderGroup>(MolapCommonConstants.CONSTANT_SIZE_TEN);
...
//        }
        ...
        group = new TopNHolderGroup(groupMaskedBytes, topNCount, topNType,maskedBytesPosForGroup,topnOnColumns);
        groups.add(group);
...
//       addRow(key, measures);
    ...
    private static final class TopNHolderGroup
    {
         ...
         */
        private TopNHolderGroup(byte[] groupMaskedBytes,int topN,MolapTopNType topNType,int[] maskedBytesPosForGroup,boolean topnOnColumns)
        {
            this.topN = topN;
    ...
    public void finish() throws MolapPaginationException
    {
        ...
        if(processor instanceof MeasureFilterProcessor)
        {
            for(TopNHolderGroup holderGroup : groups)
            {
                ...
                List<GroupByHolder> list = new ArrayList<GroupByHolder>(MolapCommonConstants.CONSTANT_SIZE_TEN);
