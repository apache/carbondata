...
//      }
  ...
  public void testCsvLoadAndCarbonReadWithPrimitiveType() throws IOException, InvalidLoadOptionException, InterruptedException {
    ...
    CarbonWriter carbonWriter = carbonWriterBuilder.withCsvPath(filePath).
            outputPath(path).withCsvInput(new Schema(fields)).
            writtenBy("CSVCarbonWriter").build();
    carbonWriter.write();
    carbonWriter.close();
  ...
  public void testCsvLoadAndCarbonReadWithComplexType() throws IOException, InterruptedException, InvalidLoadOptionException {
    ...
    CarbonWriter carbonWriter = carbonWriterBuilder.withCsvPath(filePath).
            outputPath(path).withCsvInput(new Schema(fields)).writtenBy("CSVCarbonWriter")
            .withLoadOptions(options).build();
    carbonWriter.write();
    carbonWriter.close();
    File[] dataFiles = new File(path).listFiles();
    assert (Objects.requireNonNull(dataFiles).length == 2);
  ...
  @Test
  public void testSelectedCsvFileLoadInDirectory() throws IOException, InvalidLoadOptionException, InterruptedException {
    String path = "./testCsvFileLoad";
    String filePath = "./src/test/resources/file/csv_files";
    FileUtils.deleteDirectory(new File(path));
    CarbonWriterBuilder carbonWriterBuilder = new CarbonWriterBuilder();
    ...
    CarbonWriter carbonWriter = carbonWriterBuilder.withCsvPath(filePath, fileList).
            outputPath(path).withCsvInput(new Schema(fields)).
            writtenBy("CSVCarbonWriter").build();
    carbonWriter.write();
    carbonWriter.close();
