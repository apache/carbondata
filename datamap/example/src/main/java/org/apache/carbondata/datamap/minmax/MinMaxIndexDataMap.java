...
public class MinMaxIndexDataMap extends CoarseGrainDataMap {
  ...
  public List<Blocklet> prune(FilterResolverIntf filterExp, SegmentProperties segmentProperties,
    ...
    // for minmax datamap, all fields can have minmax index
    boolean[] isMinMaxSet =
        new boolean[segmentProperties.getDimensions().size()
            + segmentProperties.getMeasures().size()];
    ...
    for (int i = 0; i < minMaxIndexHolders.size(); i++) {
      ...
      BitSet bitSet = filterExecuter.isScanRequired(maxValues, minValues, isMinMaxSet);
      if (!bitSet.isEmpty()) {
