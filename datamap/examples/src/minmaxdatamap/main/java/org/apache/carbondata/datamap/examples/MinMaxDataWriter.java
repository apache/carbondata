...
public class MinMaxDataWriter extends DataMapWriter {
  ...
  public MinMaxDataWriter(String tablePath, DataMapMeta dataMapMeta, Segment segment,
      String shardName) {
    super(tablePath, dataMapMeta, segment, shardName);
    List<CarbonColumn> cols = dataMapMeta.getIndexedColumns();
    this.columnCnt = cols.size();
    for (CarbonColumn col : cols) {
  ...
  @Override
  public void addRow(int blockletId, int pageId, CarbonRow row) {
    ...
    if (this.dataTypeArray == null) {
      ...
      for (int i = 0; i < this.columnCnt; i++) {
        this.dataTypeArray[i] = row[i].getDataType();
      }
    }
    ...
    for (int rowId = 0; rowId < row[0].getPageSize(); rowId++) {
      for (int colIdx = 0; colIdx < columnCnt; colIdx++) {
        Object originValue = row[colIdx].getData(rowId);
        // for string & bytes_array, data is prefixed with length, need to remove it
        if (DataTypes.STRING == row[colIdx].getDataType()
            || DataTypes.BYTE_ARRAY == row[colIdx].getDataType()) {
        ...
        } else if (DataTypes.INT == row[colIdx].getDataType()) {
          updateMinMax(colIdx, originValue, row[colIdx].getDataType());
        } else {
