...
public class MinMaxDataMapFactory extends CoarseGrainDataMapFactory {
  ...
  public MinMaxDataMapFactory(CarbonTable carbonTable, DataMapSchema dataMapSchema)
    ...
    if (!carbonTable.isExternalFormatTable()) {
      LOGGER.warn("It is not recommended to use MinMax datamap for ordinary carbon table,"
          + " because default Block/Blocklet datamap already covers its function."
          + " As a result, it will not prune better than default Block/Blocklet datamap,"
          + " the query performance will be worse.");
    }
    ...
    try {
      ...
      LOGGER.error(e, "Failed to create cache for minmax datamap");
  ...
  public DataMapWriter createWriter(Segment segment, String shardName,
    ...
    LOGGER.error(String
        .format("Data of MinMaxDataMap %s for table %s will be written to %s", dataMapName,
            getCarbonTable().getTableName(), shardName));
  ...
  public List<CoarseGrainDataMap> getDataMaps(DataMapDistributable distributable)
    ...
    minMaxIndexDataMap.init(new MinMaxDataMapModel(indexPath, cache));
  ...
  private CarbonFile[] getAllIndexDirs(String tablePath, String segmentId) {
    ...
    try {
      ...
      LOGGER.error(ex, String
          .format("failed to get datamaps for tablePath %s, segmentId %s", tablePath, segmentId));
