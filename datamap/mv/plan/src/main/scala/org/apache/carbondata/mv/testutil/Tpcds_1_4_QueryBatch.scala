  ...
  val tpcds1_4Queries = Seq(
    ...
    ("q50",
      """
        ...
        |   ,sum(case when (sr_returned_date_sk - ss_sold_date_sk <= 30 ) then 1 else 0 end)  as
        |   `30 days`
        |   ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 30) and
        |                  (sr_returned_date_sk - ss_sold_date_sk <= 60) then 1 else 0 end )  as
        |                  `31-60 days`
        |   ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 60) and
        |                  (sr_returned_date_sk - ss_sold_date_sk <= 90) then 1 else 0 end)  as
        |                  `61-90 days`
        |   ,sum(case when (sr_returned_date_sk - ss_sold_date_sk > 90) and
        |                  (sr_returned_date_sk - ss_sold_date_sk <= 120) then 1 else 0 end)  as
        |                  `91-120 days`
        |   ,sum(case when (sr_returned_date_sk - ss_sold_date_sk  > 120) then 1 else 0 end)  as
        |   `>120 days`
        | from
    ...
    ("q51",
      """
        ...
        |       over (partition by ws_item_sk order by d_date rows between unbounded preceding
        ...
        |       over (partition by ss_item_sk order by d_date rows between unbounded preceding
        |       and current row) cume_sales
        | from store_sales, date_dim
        | where ss_sold_date_sk=d_date_sk
        |   and d_month_seq between 1200 and 1200+11
        ...
        |      ,max(web_sales)
        |          over (partition by item_sk order by d_date rows between unbounded preceding
        |          and current row) web_cumulative
        |      ,max(store_sales)
        |          over (partition by item_sk order by d_date rows between unbounded preceding
        |          and current row) store_cumulative
        |      from (select case when web.item_sk is not null then web.item_sk else store.item_sk
        |       end item_sk
        |                  ,case when web.d_date is not null then web.d_date else store.d_date
        |                  end d_date
    ...
    ("q56",
      """
        ...
        |     store_sales, date_dim, customer_address, item
        | where
        |    i_item_id in (select i_item_id from item where i_color in ('slate','blanched',
        |    'burnished'))
        | and     ss_item_sk              = i_item_sk
        | and     ss_sold_date_sk         = d_date_sk
        | and     d_year                  = 2001
    ...
    ("q61",
      s"""
         | select promotions,total,cast(promotions as decimal(15,4))/cast(total as decimal(15,4))
         | *100
         | from
    ...
    ("q62",
      """
        ...
        |                 (ws_ship_date_sk - ws_sold_date_sk <= 60) then 1 else 0 end )  as
        |                 `31-60 days`
        |  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 60) and
        |                 (ws_ship_date_sk - ws_sold_date_sk <= 90) then 1 else 0 end)  as `61-90
        |                  days`
        |  ,sum(case when (ws_ship_date_sk - ws_sold_date_sk > 90) and
        |                 (ws_ship_date_sk - ws_sold_date_sk <= 120) then 1 else 0 end)  as
        |                 `91-120 days`
    ...
    ("q64",
      """
        | with cs_ui as
        |  (select cs_item_sk
        |         ,sum(cs_ext_list_price) as sale,sum
        |         (cr_refunded_cash+cr_reversed_charge+cr_store_credit) as refund
        |   from catalog_sales
        |       ,catalog_returns
        ...
        | cross_sales as
        |  (select i_product_name product_name, i_item_sk item_sk, s_store_name store_name, s_zip
        |   store_zip,
        |          ad1.ca_street_number b_street_number, ad1.ca_street_name b_streen_name,
        |          ad1.ca_city b_city,
        |          ad1.ca_zip b_zip, ad2.ca_street_number c_street_number, ad2.ca_street_name
        |          c_street_name,
        |          ad2.ca_city c_city, ad2.ca_zip c_zip, d1.d_year as syear, d2.d_year as fsyear,
        |           d3.d_year s2year,
        |          count(*) cnt, sum(ss_wholesale_cost) s1, sum(ss_list_price) s2, sum
        |          (ss_coupon_amt) s3
    ...
    ("q66",
      """
        ...
        |      ,d_year as year
        |       ,sum(case when d_moy = 1 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       jan_sales
        |       ,sum(case when d_moy = 2 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       feb_sales
        |       ,sum(case when d_moy = 3 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       mar_sales
        |       ,sum(case when d_moy = 4 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       apr_sales
        |       ,sum(case when d_moy = 5 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       may_sales
        |       ,sum(case when d_moy = 6 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       jun_sales
        |       ,sum(case when d_moy = 7 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       jul_sales
        |       ,sum(case when d_moy = 8 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       aug_sales
        |       ,sum(case when d_moy = 9 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       sep_sales
        |       ,sum(case when d_moy = 10 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       oct_sales
        |       ,sum(case when d_moy = 11 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       nov_sales
        |       ,sum(case when d_moy = 12 then ws_ext_sales_price * ws_quantity else 0 end) as
        |       dec_sales
        ...
        |       ,sum(case when d_moy = 1 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       jan_net
        |       ,sum(case when d_moy = 2 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       feb_net
        |       ,sum(case when d_moy = 3 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       mar_net
        |       ,sum(case when d_moy = 4 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       apr_net
        |       ,sum(case when d_moy = 5 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       may_net
        |       ,sum(case when d_moy = 6 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       jun_net
        |       ,sum(case when d_moy = 7 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       jul_net
        |       ,sum(case when d_moy = 8 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       aug_net
        |       ,sum(case when d_moy = 9 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       sep_net
        |       ,sum(case when d_moy = 10 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       oct_net
        |       ,sum(case when d_moy = 11 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       nov_net
        |       ,sum(case when d_moy = 12 then cs_net_paid_inc_tax * cs_quantity else 0 end) as
        |       dec_net
        |     from
    ...
    ("q72",
      """
        ...
        | left outer join catalog_returns on (cr_item_sk = cs_item_sk and cr_order_number =
        | cs_order_number)
    ...
    ("q73",
      """
        ...
        |             household_demographics.hd_dep_count/ household_demographics
        |             .hd_vehicle_count else null end > 1
        |    and date_dim.d_year in (1999,1999+1,1999+2)
        |    and store.s_county in ('Williamson County','Franklin Parish','Bronx County','Orange
        |    County')
    ...
    ("q74",
      """
        ...
        |    c_customer_id, c_first_name, c_last_name, d_year)
        | select
        |    t_s_secyear.customer_id, t_s_secyear.customer_first_name, t_s_secyear
        |    .customer_last_name
        | from
    ...
    ("q75",
      """
        ...
        |   AND CAST(curr_yr.sales_cnt AS DECIMAL(17,2))/CAST(prev_yr.sales_cnt AS DECIMAL(17,2))
        |   <0.9
    ...
    ("q78",
      """
        ...
        | left join cs on (cs_sold_year=ss_sold_year and ss_item_sk=cs_item_sk and
    ...
    ("q80",
      """
        ...
        |      union all
        |      select
        |        'web channel' as channel, concat('web_site', web_site_id) as id, sales, returns,
        |         profit
        |      from  wsr) x
        | group by rollup (channel, id)
        | order by channel, id
    ...
    ("q82",
      """
        ...
        |   and inv_item_sk = i_item_sk
        |   and d_date_sk=inv_date_sk
        |   and d_date between cast('2000-05-25' as date) and (cast('2000-05-25' as date) +
        |   interval 60 days)
    ...
    ("q83",
      """
        ...
        |                 (select d_week_seq from date_dim where d_date in ('2000-06-30',
        |                 '2000-09-27','2000-11-17')))
    ...
    ("q88",
      """
        ...
        |     and ((household_demographics.hd_dep_count = 4 and household_demographics
        |     .hd_vehicle_count<=4+2) or
        |          (household_demographics.hd_dep_count = 2 and household_demographics
        |          .hd_vehicle_count<=2+2) or
        |          (household_demographics.hd_dep_count = 0 and household_demographics
    ...
    ("q92",
      """
        ...
        |       FROM web_sales, date_dim
        |       WHERE ws_item_sk = i_item_sk
        |         and d_date between '2000-01-27' and (cast('2000-01-27' as date) + interval 90
        |         days)
        |         and d_date_sk = ws_sold_date_sk
        |     )
    ...
    ("q93",
      """
        ...
        |         case when sr_return_quantity is not null then (ss_quantity-sr_return_quantity)
        |         *ss_sales_price
    ...
    ("q97",
      """
        ...
        |       ,sum(case when ssci.customer_sk is not null and csci.customer_sk is not null then
        |        1 else 0 end) store_and_catalog
    ...
    ("q99",
      """
        ...
        |                  `91-120 days`
        |   ,sum(case when (cs_ship_date_sk - cs_sold_date_sk  > 120) then 1 else 0 end)  as
        |   `>120 days`
        | from
    ...
    ("qMV1",
      """
        |SELECT cs_ship_addr_sk , d_date, d_year, d_qoy, d_moy, i_category, cs_ship_addr_sk,
        |i_item_sk, i_item_id,
        |       i_item_desc, i_class, i_current_price, i_brand_id, i_class_id, i_category_id,
        |       i_manufact_id,
        ...
        |WHERE cs_item_sk = i_item_sk
        |  AND cs_sold_date_sk = d_date_sk
        |GROUP BY i_brand_id, i_class_id, i_category_id, i_item_id, i_item_desc, i_category,
        |i_class,
        |         i_current_price, i_manufact_id, d_date, d_moy, d_qoy, d_year, cs_ship_addr_sk,
        |         i_item_sk
      """.stripMargin.trim),
    ("qMV2",
      """
      ...
      """.stripMargin.trim),
    // this query shows a bug in modular plan where qualifier subq not clean up in erasure of
    // sub-query alias in logical plan
    ("qMV3",
      """
        |SELECT first(subq.channel), subq.col_name, subq.d_year, subq.d_qoy, subq.i_category, SUM
        |(subq.ss_ext_sales_price) sales_amt
        |FROM
        | (SELECT
        |      'store' as channel, ss_store_sk col_name, d_year, d_qoy, i_category,
        |      ss_ext_sales_price
    ...
    ("qVerify",
      """
        |SELECT gen_subquery_0.`c_last_name`, gen_subquery_0.`c_first_name`,
        |gen_subquery_0.`d_date`, gen_subquery_1.`c_last_name`, gen_subquery_1.`c_first_name`,
        |gen_subquery_1.`d_date`
        |        FROM
        |          (SELECT gen_subsumer_0.`customer_last_name` AS `c_last_name`,
        |          gen_subsumer_0.`customer_first_name` AS `c_first_name`, gen_subsumer_0.`ddate`
        |           AS `d_date`
        |          FROM
        |            (SELECT `c_customer_id` AS `customer_id`, `c_first_name` AS
        |            `customer_first_name`, `c_last_name` AS `customer_last_name`,
        |            `c_preferred_cust_flag` AS `customer_preferred_cust_flag`, `c_birth_country`
        |             AS `customer_birth_country`, `c_login` AS `customer_login`,
        |             `c_email_address` AS `customer_email_address`, `d_year` AS `dyear`,
        |             `d_date` AS `ddate`, `d_month_seq`, sum((CAST((((CAST(`ss_ext_list_price`
        |             AS DECIMAL(8,2)) - CAST(`ss_ext_wholesale_cost` AS DECIMAL(8,2))) - CAST
        |             (`ss_ext_discount_amt` AS DECIMAL(8,2))) + CAST(`ss_ext_sales_price` AS
        |             DECIMAL(8,2))) AS DECIMAL(12,2)) / 2.00BD)) AS `year_total`, sum
        |             (`ss_net_paid`) AS `year_total_74`, 's' AS `sale_type`
        |            FROM
        |              customer
        ...
        |            `d_year`, `d_date`, `d_month_seq`) gen_subsumer_0
        |          WHERE
        |            (gen_subsumer_0.`d_month_seq` >= 1200) AND (gen_subsumer_0.`d_month_seq` <=
        |            1211)
        |          GROUP BY gen_subsumer_0.`customer_last_name`,
        |          gen_subsumer_0.`customer_first_name`, gen_subsumer_0.`ddate`) gen_subquery_0
        |          LEFT ANTI JOIN (SELECT customer.`c_last_name`, customer.`c_first_name`,
        |          date_dim.`d_date`
        |          FROM
        |            catalog_sales
        |            INNER JOIN date_dim ON (date_dim.`d_month_seq` >= 1200) AND (date_dim
        |            .`d_month_seq` <= 1211) AND (catalog_sales.`cs_sold_date_sk` = date_dim
        |            .`d_date_sk`)
        |            INNER JOIN customer ON (catalog_sales.`cs_bill_customer_sk` = customer
        |            .`c_customer_sk`)
        |          GROUP BY customer.`c_last_name`, customer.`c_first_name`, date_dim.`d_date`)
        |          gen_subquery_1  ON (gen_subquery_0.`c_last_name` <=>
        |          gen_subquery_1.`c_last_name`) AND (gen_subquery_0.`c_first_name` <=>
        |          gen_subquery_1.`c_first_name`) AND (gen_subquery_0.`d_date` <=>
        |          gen_subquery_1.`d_date`)
      """.stripMargin),
    ("qSsMax",
      """
    ...
    ("qTradeflow",
      """
        ...
        |          AND up.newdate >= '201412' THEN CASE WHEN isnan(colunm_2014) THEN 0 ELSE
        |     colunm_2014 END ELSE NULL END
        |      ) AS colunm_2014
        |    ,(
        |      CASE WHEN up.startdate <= '201501'
        |          AND up.newdate >= '201512' THEN CASE WHEN isnan(colunm_2015) THEN 0 ELSE
        |     colunm_2015 END ELSE NULL END
        |      ) AS colunm_2015
        |    ,(
        |      CASE WHEN up.startdate <= '201601'
        |          AND up.newdate >= '201612' THEN CASE WHEN isnan(colunm_2016) THEN 0 ELSE
        |     colunm_2016 END ELSE NULL END
        |      ) AS colunm_2016
        |    ,tb
    ...
    ("qAggPushDown",
      """
        | SELECT
        |    channel, d_year, d_qoy, i_category, SUM(ext_sales_price) sales_amt
        | FROM(
        |    SELECT
        |        'store' as channel, d_year, d_qoy, i_category,
        |        ss_ext_sales_price ext_sales_price
        |    FROM store_sales, item, date_dim
        |    WHERE ss_store_sk IS NULL
        |      AND ss_sold_date_sk=d_date_sk
        |      AND ss_item_sk=i_item_sk) foo
