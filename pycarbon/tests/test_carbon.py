...
def test_invalid_carbondataset_obs_parameters(carbon_obs_external_dataset):
  with pytest.raises(ValueError):
    CarbonDataset(carbon_obs_external_dataset.url)
...
def test_create_carbondataset_obs(carbon_obs_external_dataset):
  carbondataset_1 = CarbonDataset(carbon_obs_external_dataset.url,
                                  key=pytest.config.getoption("--access_key"),
                                  secret=pytest.config.getoption("--secret_key"),
                                  endpoint=pytest.config.getoption("--end_point"))

  carbondataset_2 = CarbonDataset(carbon_obs_external_dataset.url,
                                  key=pytest.config.getoption("--access_key"),
                                  secret=pytest.config.getoption("--secret_key"),
                                  endpoint=pytest.config.getoption("--end_point"),
                                  proxy="proxy.huawei.com",
...
def test_invalid_carbondatasetpiece_obs_parameters(carbon_obs_external_dataset):
  key = pytest.config.getoption("--access_key")
  secret = pytest.config.getoption("--secret_key")
  endpoint = pytest.config.getoption("--end_point")

  carbon_splits = ArrowCarbonReader().builder(carbon_obs_external_dataset.url)\
  ...
  carbon_schema = CarbonSchemaReader().readSchema(carbon_obs_external_dataset.url, configuration.conf)

  with pytest.raises(ValueError):
    CarbonDatasetPiece(carbon_obs_external_dataset.url, carbon_schema, carbon_splits[0])
...
def test_create_carbondatasetpiece_obs(carbon_obs_external_dataset):
  key = pytest.config.getoption("--access_key")
  secret = pytest.config.getoption("--secret_key")
  endpoint = pytest.config.getoption("--end_point")
  ...
  carbondatasetpiece_1 = CarbonDatasetPiece(carbon_obs_external_dataset.url, carbon_schema, carbon_splits[0],
                                            key=key, secret=secret, endpoint=endpoint)

  carbondatasetpiece_2 = CarbonDatasetPiece(carbon_obs_external_dataset.url, carbon_schema, carbon_splits[0],
                                            key=key, secret=secret, endpoint=endpoint,
                                            proxy="proxy.huawei.com",
                                            proxy_port="8080")
...
def test_carbondataset_dataset_url_not_exist(carbon_obs_external_dataset):
  # local
  ...
  # obs
  with pytest.raises(Exception):
    CarbonDataset(carbon_obs_external_dataset.not_exist_url,
                  key=pytest.config.getoption("--access_key"),
                  secret=pytest.config.getoption("--secret_key"),
                  endpoint=pytest.config.getoption("--end_point"))
