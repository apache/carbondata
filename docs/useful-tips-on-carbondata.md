...
## Configurations for Optimizing CarbonData Performance
  ...
  | carbon.is.sort.temp.file.compression.enabled | spark/carbonlib/carbon.properties | Data loading | Whether to compress the intermediate files during sort procedure in data loading. | If this is set to true, Carbondata will use snappy to compress the records in intermediate sorted files, thus will reduce disk write. This parameter will be useful if you encounter disk bottleneck. Besides, there is an additional parameter `carbon.sort.temp.file.no.of.records.for.compression` to configure the size of records to be compressed in a batch. |
