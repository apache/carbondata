...
abstract class LuceneIndexFactoryBase<T extends Index> extends IndexFactory<T> {
  ...
  public List<IndexDistributable> toDistributable(Segment segment) {
    List<IndexDistributable> lstDataMapDistribute = new ArrayList<>();
    ...
    if (segment.getFilteredIndexShardNames().size() == 0) {
      for (CarbonFile indexDir : indexDirs) {
        IndexDistributable luceneIndexDistributable =
            new LuceneIndexDistributable(tableIdentifier.getTablePath(),
                indexDir.getAbsolutePath());
        luceneIndexDistributable.setSegment(segment);
        luceneIndexDistributable.setDataMapSchema(getDataMapSchema());
        lstDataMapDistribute.add(luceneIndexDistributable);
      }
    ...
    for (CarbonFile indexDir : indexDirs) {
      ...
      IndexDistributable luceneIndexDistributable = new LuceneIndexDistributable(
  ...
  public void deleteSegmentIndexData(String segmentId) throws IOException {
    try {
      String datamapPath = CarbonTablePath
          .getIndexStorePath(tableIdentifier.getTablePath(), segmentId, dataMapName);
  ...
  private CarbonFile[] getAllIndexDirs(String tablePath, String segmentId) {
    ...
    try {
      ...
      // the path till the index file directories in all indexes folders present in each segment
      indexes = DataMapStoreManager.getInstance().getAllIndexes(getCarbonTable());
    } catch (IOException ex) {
      LOGGER.error("failed to get index");
    }
    if (indexes.size() > 0) {
      for (TableIndex index : indexes) {
        if (index.getDataMapSchema().getDataMapName().equals(this.dataMapName)) {
          List<CarbonFile> indexFiles;
          String dmPath = CarbonTablePath.getIndexStorePath(tablePath, segmentId,
              index.getDataMapSchema().getDataMapName());
