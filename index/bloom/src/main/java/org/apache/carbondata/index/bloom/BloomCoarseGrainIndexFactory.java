...
public class BloomCoarseGrainIndexFactory extends IndexFactory<CoarseGrainIndex> {
  ...
  public static Set<String> getAllShardPaths(String tablePath, String segmentId,
      String dataMapName) {
    String dataMapStorePath = CarbonTablePath.getIndexStorePath(
    ...
    try {
      ...
      for (String shard : shardPaths) {
        if (shard.endsWith(BloomIndexFileStore.MERGE_BLOOM_INDEX_SHARD_NAME) ||
          ...
          BloomCoarseGrainIndex bloom = new BloomCoarseGrainIndex();
          bloom.init(new BloomIndexModel(shard, cache, segment.getConfiguration()));
          bloom.initIndexColumnConverters(getCarbonTable(), indexMeta.getIndexedColumns());
          bloom.setFilteredShard(filteredShards);
          indexes.add(bloom);
        }
      }
    } catch (Exception e) {
  ...
  public List<CoarseGrainIndex> getIndexes(IndexDistributable distributable) {
    List<CoarseGrainIndex> indexes = new ArrayList<>();
    String indexPath = ((BloomIndexDistributable) distributable).getIndexPath();
    Set<String> filteredShards = ((BloomIndexDistributable) distributable).getFilteredShards();
    BloomCoarseGrainIndex bloom = new BloomCoarseGrainIndex();
    bloom.init(new BloomIndexModel(indexPath, cache, FileFactory.getConfiguration()));
    bloom.initIndexColumnConverters(getCarbonTable(), indexMeta.getIndexedColumns());
    bloom.setFilteredShard(filteredShards);
    indexes.add(bloom);
    return indexes;
  ...
  public List<IndexDistributable> toDistributable(Segment segment) {
    List<IndexDistributable> indexDistributableList = new ArrayList<>();
    Set<String> shardPaths = segmentMap.get(segment.getSegmentNo());
    if (shardPaths == null) {
      shardPaths =
          getAllShardPaths(getCarbonTable().getTablePath(), segment.getSegmentNo(), dataMapName);
    ...
    for (String shardPath : shardPaths) {
      ...
      if (shardPath.endsWith(BloomIndexFileStore.MERGE_BLOOM_INDEX_SHARD_NAME) ||
        ...
        IndexDistributable bloomIndexDistributable =
            new BloomIndexDistributable(shardPath, filteredShards);
        bloomIndexDistributable.setSegment(segment);
        bloomIndexDistributable.setDataMapSchema(getDataMapSchema());
        indexDistributableList.add(bloomIndexDistributable);
      }
    }
    return indexDistributableList;
  }
  ...
  public void deleteSegmentIndexData(String segmentId) throws IOException {
    try {
      String datamapPath = CarbonTablePath
          .getIndexStorePath(getCarbonTable().getTablePath(), segmentId, dataMapName);
  ...
  public void deleteIndexData() {
    ...
    try {
      List<Segment> validSegments =
          ssm.getValidAndInvalidSegments(getCarbonTable().isMVTable()).getValidSegments();
