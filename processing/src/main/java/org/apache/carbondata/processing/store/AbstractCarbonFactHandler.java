 ...
 */
public abstract class AbstractCarbonFactHandler implements CarbonFactHandler {

  /**
   * LOGGER
   */
  private static final LogService LOGGER =
      LogServiceFactory.getLogService(AbstractCarbonFactHandler.class.getName());
  /**
   * data writer
   */
  protected CarbonFactDataWriter dataWriter;
  /**
   * File manager
   */
   ...
   * keyBlockHolder
   */
  protected CarbonKeyBlockHolder[] keyBlockHolder;
  ...
  protected boolean useKettle;

  protected boolean isSorted;
  /**
   * CarbonFactDataHandler constructor
   */
  public AbstractCarbonFactHandler(CarbonFactDataHandlerModel carbonFactDataHandlerModel) {
  ...
  private void initParameters(CarbonFactDataHandlerModel carbonFactDataHandlerModel) {
    ...
    this.isSorted = carbonTable.isSort();
    dimensionType =
  ...
  protected NodeHolder processDataRows(List<Object[]> dataRows) throws CarbonDataWriterException {
  ...
  protected NodeHolder processDataRowsWithOutKettle(List<Object[]> dataRows)
      throws CarbonDataWriterException {
    Object[] max = new Object[measureCount];
    Object[] min = new Object[measureCount];
    int[] decimal = new int[measureCount];
    ...
    for (i = 0; i < dimensionType.length; i++) {
      if (dimensionType[i]) {
        ...
        if (colGrpModel.isColumnar(dictionaryColumnCount)) {
          submit.add(executorService
              .submit(new BlockSortThread(i, dataHolders[dictionaryColumnCount].getData(),
                  this.isSorted, isUseInvertedIndex[i])));
        } else {
          submit.add(
              executorService.submit(new ColGroupBlockStorage(dataHolders[dictionaryColumnCount])));
        }
