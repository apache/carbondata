...
public class IndexWriterListener {
  ...
  public void registerAllWriter(CarbonTable carbonTable, String segmentId,
    ...
    DataMapStoreManager.getInstance().clearIndexes(carbonTable.getTableId());
    List<TableIndex> tableIndices;
    try {
      tableIndices = DataMapStoreManager.getInstance().getAllIndex(carbonTable);
    } catch (IOException e) {
  ...
  /**
   * Pick corresponding column pages and add to all registered index writer
   *
