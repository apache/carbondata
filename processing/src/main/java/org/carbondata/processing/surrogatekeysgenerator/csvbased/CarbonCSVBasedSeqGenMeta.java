...
public class CarbonCSVBasedSeqGenMeta extends BaseStepMeta implements StepMetaInterface {
    ...
    private Map<String, String> getForeignKeyColumnNameMap(String foreignKeyColumnNameString) {
        ...
        for (int i = 0; i < foreignKeys.length; i++) {
            String[] foreignHierArray =
                    foreignKeys[i].split(CarbonCommonConstants.COLON_SPC_CHARACTER);
    ...
    private Map<String, Boolean> updatePrimaryKeyMap(String primaryKeysString) {
        ...
        String[] primaryKeys =
                primaryKeysString.split(CarbonCommonConstants.AMPERSAND_SPC_CHARACTER);
    ...
    private Map<String, String> getForeignKeyHierMap(String foreignKeyHierarchyString) {
        ...
        String[] hies =
                foreignKeyHierarchyString.split(CarbonCommonConstants.AMPERSAND_SPC_CHARACTER);
    ...
    private Map<String, String[]> getHierarchiesColumnMap(String molaphierColumn) {
        ...
        String[] hies = molaphierColumn.split(CarbonCommonConstants.AMPERSAND_SPC_CHARACTER);

        for (int i = 0; i < hies.length; i++) {
            String hie = hies[i];

            String hierName =
                    hie.substring(0, hie.indexOf(CarbonCommonConstants.COLON_SPC_CHARACTER));
    ...
    private void getMetaHierarichies(String molapMetaHier) {
        ...
        String[] metaHier = molapMetaHier.split(CarbonCommonConstants.AMPERSAND_SPC_CHARACTER);
            ...
            if (null != timeDimCols && "true".equalsIgnoreCase(timeDimCols[0])) {
                ...
                if (timeDimCols.length > 1 && !timeDimCols[1].isEmpty()) {
                    ...
                    for (String levelsColumns : levelTypeColumns) {
                        String[] split =
                                levelsColumns.split(CarbonCommonConstants.COMA_SPC_CHARACTER);
    ...
    private void updateMetaHierarichiesWithQueries(String molapLocation) {
        ...
        String[] hierWithQueries = molapLocation.split(CarbonCommonConstants.HASH_SPC_CHARACTER);
        //
    ...
    private void updateDimProperties() {
        ...
        if (molapProps != null && !"".equals(molapProps)) {
            String[] entries = molapProps.split(CarbonCommonConstants.AMPERSAND_SPC_CHARACTER);
            for (int i = 0; i < entries.length; i++) {
                ...
                for (int j = 1; j < levelEntry.length; j++) {
                    String[] propEntry =
                            levelEntry[j].split(CarbonCommonConstants.COMA_SPC_CHARACTER);
