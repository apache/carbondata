...
public class SchemaRestructurer {
  ...
  public SchemaRestructurer(String schemaName, String cubeName, String factTableName,
      String hdfsStoreLocation, int currentRestructFolderNum, long curTimeToAppendToDroppedDims) {
    ...
    newSliceMetaDataPath =
        pathTillRSFolderParent + File.separator + newRSFolderName + File.separator + factTableName;
  ...
  private static ByteBuffer getMemberByteBufferWithoutDefaultValue(String defaultValue) {
    ...
    if (enableEncoding) {
      try {
        data = Base64.encodeBase64(CarbonCommonConstants.MEMBER_DEFAULT_VAL.getBytes("UTF-8"));
  ...
  public boolean restructureSchema(List<CubeDimension> newDimensions, List<Measure> newMeasures,
    ...
    String sliceMetaDatapath =
        prevRSFolderPathPrefix + currentRestructFolderNumber + File.separator + factTableName;
    ...
    String newLevelFolderPath =
        newSliceMetaDataPath + File.separator + CarbonCommonConstants.LOAD_FOLDER + newLoadCounter
            + File.separator;
    ...
    for (CubeDimension aDimension : newDimensions) {
      try {
        ...
        defaultVal =
            defaultValues.get(aDimension.name) == null ? null : defaultValues.get(aDimension.name);
    ...
    newSliceMetaData
        .setMeasuresAggregator(measureAggregators.toArray(new String[measureAggregators.size()]));
    ...
    newSliceMetaData
        .setKeyGenerator(KeyGeneratorFactory.getKeyGenerator(newSliceMetaData.getDimLens()));
  ...
  /**
   * We need to modify the slicemetadata with the dimension/measure being dropped.The
   * dimension/measure can be a base dimension
  ...
  private boolean processDroppedDimsMsrs(String prevRSFolderPathPrefix,
      int currentRestructFolderNumber, List<String> validDropDimList, List<String> validDropMsrList,
      CarbonDef.Cube cube) {
    ...
    for (int folderNumber = currentRestructFolderNumber; folderNumber >= 0; folderNumber--) {
      ...
      if (validDropMsrList.size() > 0) {
        ...
        for (String aMsr : validDropMsrList) {
          ...
          if (null != currentSliceMetaData.getNewMeasures()) {
            for (int newMsrIdx = 0;
                 newMsrIdx < currentSliceMetaData.getNewMeasures().length; newMsrIdx++) {
      ...
      if (validDropDimList.size() > 0) {
        ...
        for (String aDim : validDropDimList) {
          ...
          RelationOrJoin relation = ((CarbonDef.Dimension) schemaDim).hierarchies[0].relation;
          ...
          if (null != currentSliceMetaData.getDimensions()) {
            for (int dimIdx = 0; dimIdx < currentSliceMetaData.getDimensions().length; dimIdx++) {
              ...
              if (dimInSliceMeta
                  .equals(tableName + '_' + schemaDim.hierarchies[0].levels[0].column)) {
                levelFilesToDelete
                    .add(tableName + '_' + schemaDim.hierarchies[0].levels[0].column + ".level");
          ...
          if (null != currentSliceMetaData.getNewDimensions()) {
            for (int newDimIdx = 0;
                 newDimIdx < currentSliceMetaData.getNewDimensions().length; newDimIdx++) {
        ...
        for (String levelFileName : levelFilesToDelete) {
          ...
          try {
            if (FileFactory.isFileExist(levelFilePath, carbonFileType)) {
              CarbonFile carbonFile = FileFactory.getCarbonFile(levelFilePath, carbonFileType);
  ...
  private void makeCopyOfSliceMetaData(String aggFolderSliceMetaDataPath, int currRestructFolderNum,
      int nextRestructFolderNum) {
    SliceMetaData readSliceMetaDataFile = null;
  ...
  private int[] readcurrentLevelCardinalityFile(String currentLoadFolderPath, String factTableName)
  ...
  private void updateSliceMetadata(List<String> newDimensions, List<Measure> newMeasures,
    ...
    oldSliceMetaData.setNewActualDimLens(
        ArrayUtils.toPrimitive(existingNewDimLens.toArray(new Integer[existingNewDimLens.size()])));
    oldSliceMetaData
        .setNewDimensions(existingNewDimensions.toArray(new String[existingNewDimensions.size()]));
    ...
    oldSliceMetaData
        .setNewMeasures(existingNewMeasures.toArray(new String[existingNewMeasures.size()]));
