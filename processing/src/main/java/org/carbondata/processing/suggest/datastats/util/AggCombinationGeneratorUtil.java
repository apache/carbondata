...
public final class AggCombinationGeneratorUtil
{
	...
	public static BigInteger getMaxPossibleRows(Level[] aggCombinationLevels)
	{
		...
		for (int i = 1; i < aggCombinationLevels.length; i++)
		{
			...
			{
				distinct = getDistinctData(aggCombinationLevels,
						aggCombinationLevels[i], i);
			}
	...
	private static int getDistinctData(Level[] aggCombinationLevels,
			Level level, int currentIndex)
	{
		int minValue = 1;
			...
			BigInteger maxPossibleRows, int benefitRatio)
	{
		List<AggSuggestion> aggCombinations = new ArrayList<AggSuggestion>(
				Preference.AGG_COMBINATION_SIZE);
		...
		for (long i = powerElements - 1; i >= 0; i--)
		{
			...
			if (innerSet.size() > 0)
			{
				AggSuggestion combination = new AggSuggestion(
						innerSet.toArray(new Level[innerSet.size()]),
						Request.DATA_STATS);
				int calculatedBenefitRatio=maxPossibleRows.divide(combination.getPossibleRows()).intValue();
...
//		String empty=new String("");
	 ...
	 * @return
	 */
	public static List<String> createAggregateScript(
			List<AggSuggestion> aggSuggest, Cube cube,String schemaName, String cubeName)
	{
		List<String> aggSuggestScripts = new ArrayList<String>(
				aggSuggest.size());
		for (AggSuggestion combination : aggSuggest)
		{
			...
			String script = MessageFormat.format(
					Preference.AGGREGATE_TABLE_SCRIPT,
					dims.toString(), measureStr, formattedCubeName);
	...
	public static boolean isVisible(Level level,Cube cube)
	{
		...
		MolapDef.Hierarchy[] hiers=((MolapDef.Dimension)dimensions[level.getOrdinal()]).hierarchies;
	...
	private static String getMeasures(Cube cube)
	{
		...
		for (int i = 0; i < measures.length; i++)
		{
			...
			measureStr.append(msr.aggregator).append("(")
					.append(msr.column).append(")");
	...
	public static List<String> getDimensionsWithMeasures(List<AggSuggestion> aggSuggest, Cube cube)
	{
