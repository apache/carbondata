...
public final class AggCombinationGeneratorUtil {
  ...
  private static int getDistinctData(Level[] aggCombinationLevels, Level level, int currentIndex) {
    int minValue = 1;
    ...
    for (long i = powerElements - 1; i >= 0; i--) {
      ...
      if (innerSet.size() > 0) {
        AggSuggestion combination =
            new AggSuggestion(innerSet.toArray(new Level[innerSet.size()]), Request.DATA_STATS);
