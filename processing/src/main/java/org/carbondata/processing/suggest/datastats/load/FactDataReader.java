...
public class FactDataReader {
  ...
  public HashSet<Integer> getSampleFactData(int dimension, int noOfRows) {
    ...
    String configFactSize = CarbonProperties.getInstance().getProperty(Preference.AGG_FACT_COUNT);
   ...
   */
  public void getSampleFactDataForNoDictionaryValKey(int dimension, int numberOfRows,
      int[] noDictionaryColIndexes, HashSet<byte[]> mergedData) {
    int[] dimensions = new int[] { dimension };
    boolean[] needCompression = new boolean[dimensions.length];
    Arrays.fill(needCompression, true);
    String configFactSize = CarbonProperties.getInstance().getProperty(Preference.AGG_FACT_COUNT);
    ...
    for (int i = 0; i < noOfFactsToRead; i++) {
      AggDataSuggestScanner scanner = new AggDataSuggestScanner(keySize, dimensions);
      ColumnarKeyStoreDataHolder dataHolder =
          factDataNodes.get(i).getColumnData(fileHolder, dimension, false, noDictionaryColIndexes);
      scanner.getLimitedDataBlockForNoDictionaryVals(numberOfRows, mergedData, dataHolder);
    }
