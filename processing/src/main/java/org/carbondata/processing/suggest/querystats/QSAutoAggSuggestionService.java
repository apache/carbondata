...
import org.carbondata.query.querystats.BinaryQueryStore;
import org.carbondata.query.querystats.Preference;
import org.carbondata.query.querystats.QueryDetail;
import org.carbondata.query.querystats.QueryNormalizer;
import org.carbondata.query.querystats.QueryStore;
...
public class QSAutoAggSuggestionService implements AutoAggSuggestionService {
   ...
   */
  @Override public List<String> getAggregateDimensions(LoadModel loadModel)
      throws AggSuggestException {
    try {
   ...
   * @throws AggSuggestException
   */
  @Override public List<String> getAggregateScripts(LoadModel loadModel)
      throws AggSuggestException {
    try {
      List<AggSuggestion> aggSuggest = generateAggregatesCombination(loadModel);
      List<String> aggSuggestsScript = AggCombinationGeneratorUtil
          .createAggregateScript(aggSuggest, loadModel.getCube(), loadModel.getSchemaName(),
  ...
  private List<AggSuggestion> generateAggregatesCombination(LoadModel loadModel)
    ...
    //TO-DO need to remove when we find solution to find no of rows scanned by query for filter
    // query
  ...
  private QueryDetail[] getNormalizedQueries(String queryStatsPath, Level[] distinctData) {
    ...
    for (QueryDetail queryDetail : queryDetails) {
      //if it is filter or limit query than noOfRowsScanned will not be there, use datastats to
