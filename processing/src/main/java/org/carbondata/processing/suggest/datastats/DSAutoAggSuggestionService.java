...
public class DSAutoAggSuggestionService implements AutoAggSuggestionService {
  ...
  @Override public List<String> getAggregateScripts(LoadModel loadModel)
    ...
    try {
      List<AggSuggestion> dimsCombination = getAggregateCombination(loadModel);
      return AggCombinationGeneratorUtil
          .createAggregateScript(dimsCombination, loadModel.getCube(), loadModel.getSchemaName(),
  ...
  private DriverDistinctData readFromStore(LoadModel loadModel) throws AggSuggestException {
    ...
    int currentRestructNumber = CarbonUtil
        .checkAndReturnCurrentRestructFolderNumber(loadModel.getMetaDataPath(), "RS_", false);
    ...
    try {
      ...
      LOGGER
          .info(CarbonEngineLogEvent.UNIBI_CARBONENGINE_MSG, "Error getting distinct relationship",
              e);
