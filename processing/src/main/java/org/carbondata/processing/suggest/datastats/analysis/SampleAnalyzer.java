...
public class SampleAnalyzer {
  ...
  public void execute(List<Level> dimOrdCard, String partitionId) throws AggSuggestException {
    ...
    LOGGER
        .info(CarbonEngineLogEvent.UNIBI_CARBONENGINE_MSG, "Processing cardinality:" + dimOrdCard);
    for (int i = 0; i < dimOrdCard.size(); i++) {
      ...
      // setting default value to 1. We have considered totalDimensions because there is
      // ignore_cardinality which is
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (null != dimDistinctData && null != dimDistinctData.getOtherDimesnionDistinctData()) {
          Integer distinctInt =
              dimDistinctData.getOtherDimesnionDistinctData().get(masterDim.getOrdinal());
          if (null != distinctInt) {
  ...
  private void queryData(Level master, ArrayList<Level> slaves,
    ...
    LOGGER
        .info(CarbonEngineLogEvent.UNIBI_CARBONENGINE_MSG, "Started with sample data:" + realDatas);
    //query on bplus tree
    ...
    try {
      ...
      if (InMemoryTableStore.getInstance().isLevelCacheEnabled()) {
        levelCacheKeys = DataStatsUtil
            .validateAndLoadRequiredSlicesInMemory(loadSampler.getAllLoads(), cubeUniqueName,
                columnsForQuery);
      }
  ...
  public CarbonQueryExecutorModel createQueryExecutorModel(Dimension master, List<String> datas,
    ...
    Expression left =
        new ColumnExpression(master.getDimName(), DataStatsUtil.getDataType(master.getDataType()));
