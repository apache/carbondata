...
public class DuplicateRecordHandler {
     ...
     *
     * @param leafNodeSize leafNodeSize
     * @param aggType      aggType
     */
    public DuplicateRecordHandler(int leafNodeSize, String[] aggType,
    ...
    public void addList(byte[] mdkey, double[] msrs, double[] uniqueValue) {
        ...
        for (int i = 0; i < msrs.length; i++) {
            if (("max".equalsIgnoreCase(aggType[i]) || "min".equalsIgnoreCase(aggType[i]))
                    && msrs[i] == uniqueValue[i]) {
     ...
     *
     * @param typeString agg type
     * @param firstValue first value
     * @param secValue   second value
     * @return agg value
     */
    private double mapAggregateType(String typeString, double firstValue, double secValue,
            double uniqueValue, double currentUniqueValueToBeUsed) {
        if ("max".equalsIgnoreCase(typeString)) {
            if (0 == Double.compare(currentUniqueValueToBeUsed, firstValue) && 0 == Double
                    .compare(uniqueValue, secValue)) {
                return firstValue;
            } else if (0 != Double.compare(currentUniqueValueToBeUsed, firstValue) && 0 == Double
                    .compare(uniqueValue, secValue)) {
                return firstValue;
            } else if (0 == Double.compare(currentUniqueValueToBeUsed, firstValue) && 0 != Double
                    .compare(uniqueValue, secValue)) {
