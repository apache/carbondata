...
public class CarbonAutoAggregateSliceMergerMeta extends BaseStepMeta implements StepMetaInterface {

  /**
   * for i18n purposes
   */
  private static final Class<?> PKG = CarbonAutoAggregateSliceMergerMeta.class;
   ...
   */
  public CarbonAutoAggregateSliceMergerMeta() {
    super();
  }
  ...
  public void initialise() {
    tableNames = tabelName.split(CarbonCommonConstants.HASH_SPC_CHARACTER);
    mapOfTableAndMdkeySize =
    ...
    mapOfAggTableAndAgg =
        new HashMap<String, String[]>(CarbonCommonConstants.DEFAULT_COLLECTION_SIZE);
    String[] split = mdkeySize.split(CarbonCommonConstants.HASH_SPC_CHARACTER);
    String[] split2 = null;
    for (int i = 0; i < split.length; i++) {
      split2 = split[i].split(CarbonCommonConstants.COMA_SPC_CHARACTER);
      mapOfTableAndMdkeySize.put(split2[0], Integer.parseInt(split2[1]));
    }

    split = measureCount.split(CarbonCommonConstants.HASH_SPC_CHARACTER);
    for (int i = 0; i < split.length; i++) {
      split2 = split[i].split(CarbonCommonConstants.COMA_SPC_CHARACTER);
      mapOfTableAndMeasureCount.put(split2[0], Integer.parseInt(split2[1]));
    }
    split = aggregatorString.split(CarbonCommonConstants.COLON_SPC_CHARACTER);
    for (int i = 0; i < split.length; i++) {
      split2 = split[i].split(CarbonCommonConstants.COMA_SPC_CHARACTER);
      mapOfAggTableAndAgg.put(split2[0], split2[1].split(CarbonCommonConstants.HASH_SPC_CHARACTER));
    }

    split = aggregatorClassString.split(CarbonCommonConstants.COLON_SPC_CHARACTER);
    for (int i = 0; i < split.length; i++) {
      split2 = split[i].split(CarbonCommonConstants.COMA_SPC_CHARACTER);
      mapOfAggTableAndAggClass
          .put(split2[0], split2[1].split(CarbonCommonConstants.HASH_SPC_CHARACTER));
    }
  ...
  public String getXML() {
    ...
    strBuff.append("    ").append(XMLHandler.addTagValue("factDimLensString", factDimLensString));
    strBuff.append("    ")
  ...
  @Override public void saveRep(Repository repository, ObjectId idTransformation, ObjectId idStep)
    ...
    try {
      repository.saveStepAttribute(idTransformation, idStep, "TableName", tabelName); //$NON-NLS-1$
      repository.saveStepAttribute(idTransformation, idStep, "MDKeySize", mdkeySize); //$NON-NLS-1$
      ...
      repository.saveStepAttribute(idTransformation, idStep, "cubeName", cubeName); //$NON-NLS-1$
      repository
          .saveStepAttribute(idTransformation, idStep, "schemaName", schemaName); //$NON-NLS-1$
      repository.saveStepAttribute(idTransformation, idStep, "aggregatorClassString",
          aggregatorClassString);
      repository.saveStepAttribute(idTransformation, idStep, "aggregatorString", aggregatorString);
      repository
          .saveStepAttribute(idTransformation, idStep, "factDimLensString", factDimLensString);
  ...
  @Override public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
    ...
    if (input.length > 0) {
      ...
      chkRes = new CheckResult(CheckResult.TYPE_RESULT_ERROR, "No input received from other steps!",
          stepMeta);
      remarks.add(chkRes);
    }
  ...
  @Override public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface,
    ...
    return new CarbonAutoAggregateSliceMergerStep(stepMeta, stepDataInterface, copyNr, transMeta,
        trans);
  }
  ...
  @Override public StepDataInterface getStepData() {
    return new CarbonAutoAggregateSliceMergerData();
  }
