...
public class CarbonCompressedSortTempFileWriter extends AbstractCarbonSortTempFileWriter {
   ...
   */
  public CarbonCompressedSortTempFileWriter(int measureCount, int mdkeyIndex, int mdKeyLength,
  ...
  public void writeSortTempFile(Object[][] records) throws CarbonSortKeyAndGroupByException {
    ...
    try {
      ...
      for (int i = 0; i < records.length; i++) {
        ...
        System.arraycopy((byte[]) row[mdkeyIndex], 0, completeMdkey, i * mdKeyLength, mdKeyLength);
        if (isFactMdkeyInSort) {
          System.arraycopy((byte[]) row[row.length - 1], 0, completeFactMdkey, i * factMdkeyLength,
              factMdkeyLength);
        }
      }
  ...
  private void writeCompressData(int recordSize, byte[] mdkey, byte[] factMdkey,
      ByteArrayOutputStream[] blockDataArray, boolean isFactMdkeyInSort, int writeFileBufferSize)
      throws IOException {
  ...
  private void initializeMeasureBuffers(ByteArrayOutputStream[] blockDataArray,
    ...
    for (int i = 0; i < blockDataArray.length; i++) {
      blockDataArray[i] =
          new ByteArrayOutputStream(recordSizePerLeaf * CarbonCommonConstants.DOUBLE_SIZE_IN_BYTE);
