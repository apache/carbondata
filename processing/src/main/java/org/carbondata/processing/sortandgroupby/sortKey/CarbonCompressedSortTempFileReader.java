    ...
    try {
      for (int i = 0; i < measureCount; i++) {
        ...
        byteArrayInputStream[i] = new ByteArrayInputStream(
            SnappyByteCompression.INSTANCE.unCompress(fileHolder.readByteArray(filePath, readInt)));
    ...
    try {
      for (int i = 0; i < recordSize; i++) {
        ...
        if (isFactMdkeyInSort) {
          ...
          System.arraycopy(completeMdkeyFactMdkey, factMdkeyPosition, mdkey, 0, factMdkeyLength);
