...
public class CarbonMDKeyGenStep extends BaseStep implements StepInterface {
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(CarbonMDKeyGenStep.class.getName());
    /**
    ...
    private CarbonMDKeyGenStepData data;
    /**
     * carbon mdkey generator step meta
     */
    private CarbonMDKeyGenStepMeta meta;
    /**
     * dimension length
     */
     ...
     * @param trans
     */
    public CarbonMDKeyGenStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr,
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        try {
            meta = (CarbonMDKeyGenStepMeta) smi;
            data = (CarbonMDKeyGenStepData) sdi;
            ...
            if (r == null) {
                ...
                String logMessage =
                        "CarbonSortKeyStep: Read: " + readCounter + ": Write: " + writeCounter;
            ...
            if (first) {
                ...
                if (CheckPointHanlder.IS_CHECK_POINT_NEEDED && !meta.isAutoAggRequest()) {
                    ...
                    this.threshold = Integer.parseInt(instance.getProperty(
                            CarbonCommonConstants.CARBON_CHECKPOINT_QUEUE_THRESHOLD,
                            CarbonCommonConstants.CARBON_CHECKPOINT_QUEUE_THRESHOLD_DEFAULT_VAL));
                    this.checkPointSize = Integer.parseInt(
                            instance.getProperty(CarbonCommonConstants.CARBON_CHECKPOINT_CHUNK_SIZE,
                                    CarbonCommonConstants.CARBON_CHECKPOINT_CHUNK_SIZE_DEFAULT_VAL));
                    this.initialCapacity = Integer.parseInt(instance.getProperty(
                            CarbonCommonConstants.CARBON_CHECKPOINT_QUEUE_INITIAL_CAPACITY,
                            CarbonCommonConstants.CARBON_CHECKPOINT_QUEUE_INITIAL_CAPACITY_DEFAULT_VAL));
                    this.toCopy = Integer.parseInt(instance.getProperty(
                            CarbonCommonConstants.CARBON_CHECKPOINT_TOCOPY_FROM_QUEUE,
                            CarbonCommonConstants.CARBON_CHECKPOINT_TOCOPY_FROM_QUEUE_DEFAULT_VAL));
                }
        ...
        while (true) {
            ...
            synchronized (getRowLock) {
                ...
                if (readCounter % logCounter == 0) {
                    String logMessage =
                            "Carbon Mdkey Generation Step: Record Read for table : " + this.tableName
                                    + " is : " + readCounter;
                    LOGGER.info(CarbonDataProcessorLogEvent.UNIBI_CARBONDATAPROCESSOR_MSG,
                            logMessage);
                }
    ...
    private void writeMeasureMetadataFileToTempLocation() {
        CarbonProperties carbonPropInstance = CarbonProperties.getInstance();
        String tempLocationKey = meta.getSchemaName() + '_' + meta.getCubeName();
        String sortTempFileLoc = carbonPropInstance
