"""
context_store_demo.py
=====================
End-to-end demonstration of the ContextStore multi-agent data exchange.
...
async def monitor_callback(key: str, record: ContextRecord) -> None:
    ...
    print(msg)
    _change_log.append({"key": key, "writer": record.writer, "value": record.value})
...
async def main() -> None:
    ...
    await ns.put(
        "result",
        {"task_id": task["id"], "summary": "Q1 revenue +12% YoY", "rows_processed": 14_320},
        writer="worker",
        tags=["output"],
    )
    ...
    assert records[0].value == "completed"
    assert records[-1].value == "pending"
    ...
    print(f"  coordinator→worker queue depth: {len(store.channel('coordinator', 'worker'))}")
    ...
    assert len(inbox_msgs) == 3
    print("  inbox() aggregation ✓")

    # 4. TTL
    section("4. TTL — heartbeat entry expires after 3 seconds")
    await ns.put("heartbeat", {"alive": True, "load": 0.42}, writer="worker", ttl=3, tags=["health"])
    ...
    for e in worker_entries:
        assert e.current.writer == "worker"
    ...
    remote_v = (await remote_store.get_entry("status", "data_pipeline_v1")).current.version
    local_v = (await store.get_entry("status", "data_pipeline_v1")).current.version
    print(f"  Before merge: local status v{local_v}, remote status v{remote_v}")
    ...
    if remote_v > local_v:
        assert merged_status == "archived_v2"
    ...
    assert local_keys == json_keys
    print("  Key set roundtrip matches ✓")

    # 8. stats
    section("8. Store stats")
    print(json.dumps(await store.stats(), indent=2, ensure_ascii=False))
