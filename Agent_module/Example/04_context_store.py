...
Tests every major feature of ContextStore and ContextNamespace:
  put / get / delete / history / keys / query
  P2P channels (send / receive / inbox / peek)
  Pub/Sub (subscribe / unsubscribe / callback ordering)
  TTL expiry (lazy + batch cleanup)
  Namespace isolation
  Merge (last-write-wins by version)
  Snapshot and to_json serialisation
  Stats

Run:
    python3 Agent_module/Example/04_context_store.py
"""
...
async def test_basic_put_get() -> None:
    ...
    val = await store.get("color")
    check("get returns current value",          val == "red")
    ...
    not_deleted = await store.delete("__missing__", writer="alice")
    check("delete returns False for missing key",   not not_deleted)
...
async def test_history() -> None:
    ...
    empty = await store.history("__missing__")
    check("history for missing key is []",          empty == [])
...
async def test_keys_and_query() -> None:
    ...
    all_keys = await store.keys(namespace=ns)
    check("keys() returns all 3 keys",              set(all_keys) == {"a", "b", "c"})
    ...
    no_match = await store.query(namespace=ns, writer="nobody")
    check("query with no match returns []",         no_match == [])
...
async def test_namespace_isolation() -> None:
    ...
    ns1_keys = await store.keys(namespace="ns1")
    ns2_keys = await store.keys(namespace="ns2")
    check("ns1 has exactly 1 key",      ns1_keys == ["key"])
    check("ns2 has exactly 1 key",      ns2_keys == ["key"])
...
async def test_context_namespace_proxy() -> None:
    ...
    hist = await ns.history("h")
    check("ns.history returns 2 records",           len(hist) == 2)

    keys = await ns.keys()
    check("ns.keys() returns ['h']",                keys == ["h"])
...
async def test_channels() -> None:
    ...
    # Send two messages from coordinator to worker
    ...
    # Drain with inbox across multiple senders
    ...
    # Empty channel
    empty = await store.receive("coordinator", "worker")
    check("receive on empty channel returns None",  empty is None)
...
async def test_pubsub() -> None:
    ...
    # Subscribe monitor (async) and logger (sync) to all keys
    ...
    # Writer does NOT receive their own notification
    log.clear()
...
async def test_ttl() -> None:
    ...
    await ns.put("c", 3, writer="x")          # no TTL — should survive
...
async def test_merge() -> None:
    ...
    # Merge a into b — a's status (v2) should overwrite b's status (v1)
