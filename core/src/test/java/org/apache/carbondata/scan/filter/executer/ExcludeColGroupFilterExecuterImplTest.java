...
public class ExcludeColGroupFilterExecuterImplTest {
    ...
    public void init() {
        ...
        columnSchema1 = getWrapperDimensionColumn(DataType.INT, "ID", true,
                Collections.<Encoding>emptyList(), false, -1);
        columnSchema2 = getWrapperDimensionColumn(DataType.INT, "salary", true,
                Collections.<Encoding>emptyList(), false, -1);
        columnSchema3 = getWrapperDimensionColumn(DataType.STRING, "country", false,
                encodeList, true, 0);
        columnSchema4 = getWrapperDimensionColumn(DataType.STRING, "serialname", false,
                encodeList, true, 0);

        List<ColumnSchema> wrapperColumnSchema = Arrays.asList(columnSchema1, columnSchema2, columnSchema3, columnSchema4);

        segmentProperties = new SegmentProperties(wrapperColumnSchema, new int[]{3, 11});
    }

    @Test
    public void testGetFilteredIndexes() {
        ...
        dimColumnResolvedFilterInfo.addDimensionResolvedFilterInstance(carbonDimension, dimColumnFilterInfo);
        ...
        excludeColGroupFilterExecuter = new ExcludeColGroupFilterExecuterImpl(dimColumnResolvedFilterInfo, segmentProperties);
        ...
        ColumnGroupDimensionDataChunk columnGroupDimensionDataChunk = new ColumnGroupDimensionDataChunk(dataChunk,
                dimensionChunkAttributes);

        BitSet result = excludeColGroupFilterExecuter.getFilteredIndexes(columnGroupDimensionDataChunk, 10);
    ...
    @Test
    public void testGetFilteredIndexesException() {

        CarbonDimension carbonDimension = new CarbonDimension(columnSchema4, 1, 1, 1, -1);

        DimColumnResolvedFilterInfo dimColumnResolvedFilterInfo = new DimColumnResolvedFilterInfo();
        dimColumnResolvedFilterInfo.addDimensionResolvedFilterInstance(carbonDimension, new DimColumnFilterInfo());
        dimColumnResolvedFilterInfo.setDimension(carbonDimension);
    ...
    public void testIsScanRequired() {
        ...
        BitSet result = excludeColGroupFilterExecuter.isScanRequired(new byte[][]{{96, 11}}, new byte[][]{{64, 2}});
    ...
    private ColumnSchema getWrapperDimensionColumn(DataType dataType, String columnName, boolean columnar, List<Encoding> encodeList,
                                                         boolean dimensionColumn, int columnGroup) {
