...
public class DataFileFooterConverterTest {
    ...
    public static void setUp() throws Exception{
        new File("../core/src/test/resources/testIndexFile.txt").createNewFile();
    }

    @Test
    public void testGetIndexInfo() throws Exception{
        DataFileFooterConverter dataFileFooterConverter = new DataFileFooterConverter();
        String filePath = "../core/src/test/resources/testIndexFile.txt";
        final ThriftReader thriftReader = new ThriftReader(filePath);
        ...
        new MockUp<CarbonIndexFileReader>(){
            ...
            @SuppressWarnings("unused")
            @Mock//int version, List<ColumnSchema> table_columns, SegmentInfo segment_info
        ...
        String [] arr = {"a","b","c"};
        TableBlockInfo tableBlockInfo = new TableBlockInfo(filePath,3,"id",arr,3);
        tableBlockInfo.getBlockletInfos().setNoOfBlockLets(3);
        List<TableBlockInfo> tableBlockInfoList = new ArrayList<>();
        tableBlockInfoList.add(tableBlockInfo);
        List<DataFileFooter> dataFileFooterList = dataFileFooterConverter.getIndexInfo("indexfile",tableBlockInfoList);
    ...
    public void testReadDataFileFooter() throws Exception{
        ...
        final FileFooter fileFooter = new FileFooter(1,3,columnSchemas,segmentInfo1,blockletIndexArrayList,blockletInfoArrayList);
        BlockletBTreeIndex blockletBTreeIndex = new BlockletBTreeIndex();
        blockletBTreeIndex.setStart_key("1".getBytes());
        blockletBTreeIndex.setEnd_key("3".getBytes());
        blockletIndex1.setB_tree_index(blockletBTreeIndex);
        ...
        DataFileFooter result =  dataFileFooterConverter.readDataFileFooter(filePath,1,1);
        assertEquals(result.getNumberOfRows(),3);
    }

    @AfterClass
    public static void testcleanUp() {
        new File("../core/src/test/resources/testIndexFile.txt").deleteOnExit();
    }
