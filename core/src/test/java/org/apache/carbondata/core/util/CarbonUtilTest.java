...
public class CarbonUtilTest {
  ...
  @Test public void testToGetFirstIndexUsingBinarySearchWithMatchUpLimitTrue() {
	...
	private void assertRangeIndex(byte[][] dataArr, byte[] dataChunk, FixedLengthDimensionDataChunk fixedLengthDimensionDataChunk,
		...
		int[] range;
		range = CarbonUtil.getRangeIndexUsingBinarySearch(fixedLengthDimensionDataChunk, 0, (dataChunk.length - 1)/keyWord.length ,
 				keyWord);
 		assertEquals(expectRangeIndex[0], range[0]);
 		assertEquals(expectRangeIndex[1], range[1]);
 		
		int index = CarbonUtil.binarySearch(dataArr, 0, dataChunk.length/keyWord.length ,
 				keyWord);
