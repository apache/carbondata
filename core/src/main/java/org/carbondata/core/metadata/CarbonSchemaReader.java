...
public final class CarbonSchemaReader {
  ...
  public static String getTableInputSQLQuery(CubeDimension[] dimensions, Measure[] measures,
    ...
    for (int i = 0; i < measures.length; i++) {
      ...
      if (isQuotesRequired) {
        query.append(
            ',' + QUOTES + factTableName + QUOTES + '.' + QUOTES + measures[i].column + QUOTES);
      } else {
    ...
    for (CubeDimension cubeDimension : dimensions) {
      ...
      for (Hierarchy hierarchy : hierarchies) {
        ...
        if (isQuotesRequired) {
          ...
          query.append(QUOTES + hierarchyTable + QUOTES + '.' + QUOTES + primaryKey + QUOTES);
          query.append(
              '=' + QUOTES + factTableName + QUOTES + '.' + QUOTES + dim.foreignKey + QUOTES);
        } else {
      ...
      for (Hierarchy hierarchy : dim.hierarchies) {
        ...
        for (int i = 1; i < hierarchy.levels.length; i++) {
          if (hasOrdinalColumn(hierarchy.levels[i])) {
            query.append(System.getProperty("line.separator"));
            query.append(',' + hierarchyTable + '.' + hierarchy.levels[i].ordinalColumn);
          }
          if (null != hierarchy.levels[i].nameColumn) {
            query.append(System.getProperty("line.separator"));
    ...
    for (CubeDimension cDim : dimensions) {
      ...
      for (Hierarchy hierarchy : dim.hierarchies) {
        ...
        if (null != hierarchy.levels[0].nameColumn) {
          query.append(System.getProperty("line.separator"));
          query.append(
              ',' + QUOTES + hierarchyTable + QUOTES + '.' + QUOTES + hierarchy.levels[0].nameColumn
                  + QUOTES);
        }
        properties = hierarchy.levels[0].properties;
        if (properties.length > 1) {
          for (int j = 0; j < properties.length; j++) {
            query.append(System.getProperty("line.separator"));
            query.append(
                ',' + QUOTES + hierarchyTable + QUOTES + '.' + QUOTES + properties[j].column
                    + QUOTES);
          }
        }
        for (int i = 1; i < hierarchy.levels.length; i++) {
    ...
    for (CubeDimension cDim : dimensions) {
      ...
      for (Hierarchy hierarchy : dim.hierarchies) {
        ...
        if (counter == 0) {
          if (isQuotesRequired) {
            query.append(
                QUOTES + hierarchyTable + QUOTES + '.' + QUOTES + hierarchy.levels[0].column
                    + QUOTES);
          } else {
        ...
        for (int i = 1; i < hierarchy.levels.length; i++) {
          ...
          if (isQuotesRequired) {
            query.append(
                ',' + QUOTES + hierarchyTable + QUOTES + '.' + QUOTES + hierarchy.levels[i].column
                    + QUOTES);
          } else {
    ...
    for (CubeDimension cDimension : dimensions) {
      ...
      for (Hierarchy hierarchy : dimension.hierarchies) {
        ...
        for (Level level : hierarchy.levels) {
          ...
          dimString.append(
              tableName + '_' + level.column + ':' + counter + ':' + level.levelCardinality);
          counter++;
          if (i > 1) {
            dimString.append(",");
  ...
  public static List<Map<String, String>> getAggTable(Cube cube) {
    ...
    AggTable[] aggTables = factTable.getAggTables();
  ...
  public static String getAggTableMeasureString(AggMeasure[] measures) {
    StringBuilder measureString = new StringBuilder();
