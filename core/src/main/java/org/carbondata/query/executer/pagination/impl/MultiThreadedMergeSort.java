...
public final class MultiThreadedMergeSort {
  ...
  public static KeyValueHolder[] sort(KeyValueHolder[] src, Comparator<KeyValueHolder> comparator)
    ...
    for (int i = 0; i < numberOfThreads; i++) {
      ...
      if (i == (numberOfThreads - 1)) {
        ...
        sortThreads.add(
            executorService.submit(new SortThread(part, comparator, srcPos, srcPos + equalParts)));
      } else {
        KeyValueHolder[] part = new KeyValueHolder[equalParts];
        System.arraycopy(src, srcPos, part, 0, equalParts);
        sortThreads.add(
    ...
    mergeFutures
        .add(executorService.submit(new MergeThread(parts.get(0), parts.get(1), comparator)));
    mergeFutures
        .add(executorService.submit(new MergeThread(parts.get(2), parts.get(3), comparator)));
    parts.clear();
   ...
   * @param c
   */
  private static void merge(KeyValueHolder[] src, KeyValueHolder[] array1, KeyValueHolder[] array2,
      Comparator c) {
  ...
  private static final class SortThread implements Callable<SortThread> {
    ...
    private SortThread(KeyValueHolder[] data, Comparator<KeyValueHolder> comparator, int startPos,
        int endPos) {
      this.data = data;
