...
/**
 * Abstract class used for query execution and to use to initialize all the properties used for
 * query execution
 */
public abstract class AbstractQueryExecutor implements QueryExecutor {
  ...
  protected void initQuery(CarbonQueryExecutorModel queryModel) throws QueryExecutionException {
    ...
    fillDimensionsFromExpression(queryModel.getExpressions(), executerProperties.aggExpDimensions,
        executerProperties.aggExpMeasures);
    ...
    QueryExecutorUtil
        .updateMaskedKeyRanges(executerProperties.maskedBytes, executerProperties.maskByteRanges);
    ...
    try {
      ...
      executerProperties.maxKey = QueryExecutorUtil
          .getMaxKeyBasedOnDimensions(queryModel.getDims(), executerProperties.globalKeyGenerator,
  ...
  protected void updateDimensionAggregatorInfo(
    ...
    while (iterator.hasNext()) {
      ...
      dimKeyGenerator = KeyGeneratorFactory.getKeyGenerator(
          new int[] { slice.getDimCardinality()[dimensionAggregatorInfo.getDim().getOrdinal()] });
      try {
