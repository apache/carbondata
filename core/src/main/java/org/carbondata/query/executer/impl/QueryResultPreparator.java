            ...
            for (int i = 0; i < dimensionCount; i++) {
                if (dims[i].isNoDictionaryDim() && null != keyWrapper
                    ...
                    GenericQueryType complexType = queryModel.getComplexDimensionsMap()
                            .get(queryModel.getDims()[i].getColName());
                    if (complexType == null) {
                        resultData[currentRow][i] =
                                (int) keyArray[executerProperties.hybridStoreModel
                                        .getMdKeyOrdinal(queryModel.getDims()[i].getOrdinal())];
                    } else {
                        resultData[currentRow][i] = keyWrapper.getComplexTypeData(
                                complexQueryIndexes.get(queryModel.getDims()[i].getColName()));
                    }
                }
            }
            //CHECKSTYLE:ON
        ...
        for (int columnIndex = 0; columnIndex < resultDataA[0].length; columnIndex++) {
            ...
            for (int i = 0; i < dimensionCount; i++) {
                ...
                if (surrogateResult[i][columnIndex] instanceof byte[] && complexType == null) {
                    ...
                    member = QueryExecutorUtility
                            .getActualMemberBySortedKey(queryModel.getDims()[i],
                                    (Integer) surrogateResult[i][columnIndex],
                                    executerProperties.slices);
                } else {
                    if (complexType == null) {
                        member = QueryExecutorUtility
                                .getMemberBySurrogateKey(queryModel.getDims()[i],
                                        (Integer) surrogateResult[i][columnIndex],
                                        executerProperties.slices);
                    } else {
            ...
            fillMeasureValueForAggGroupByQuery(queryModel, surrogateResult, dimensionCount,
                    columnIndex, msrAgg);
            ...
            if (!queryModel.isDetailQuery()) {
                ...
                for (int i = 0; i < queryModel.getDimensionAggInfo().size(); i++) {
                    DimensionAggregatorInfo dimensionAggregatorInfo =
                            queryModel.getDimensionAggInfo().get(i);
                ...
                for (int i = 0; i < queryModel.getExpressions().size(); i++) {
                    row[queryModel.getExpressions().get(i).getQueryOrder()] =
                            ((MeasureAggregator) surrogateResult[dimensionCount
                                    + executerProperties.aggExpressionStartIndex + i][columnIndex])
                                    .get();
                }
            } else {
                ...
                for (int i = 0; i < queryModel.getMsrs().size(); i++) {
                    if (msrAgg[executerProperties.measureStartIndex + i].isFirstTime() && (
                        ...
                        Object msrVal;
                        switch (executerProperties.dataTypes[executerProperties.measureStartIndex
                                + i]) {
                        case LONG:
                            msrVal =
                                    msrAgg[executerProperties.measureStartIndex + i].getLongValue();
                            break;
                        case DECIMAL:
                            msrVal = msrAgg[executerProperties.measureStartIndex + i]
                                    .getBigDecimalValue();
                            break;
                        default:
                            msrVal = msrAgg[executerProperties.measureStartIndex + i]
                ...
                if (!dimensionAggregatorInfo.getAggList().get(j)
                    ...
                    if (surrogateResult[dimensionCount
                        ...
                        Iterator<Integer> iterator =
                                ((DistinctCountAggregator) surrogateResult[dimensionCount
                                        + rowIndex][columnIndex]).getBitMap().iterator();
                        if (null != mappedDim) {
                            MeasureAggregator distinctCountAggregator =
                                    new DistinctCountAggregator(0);
                            while (iterator.hasNext()) {
                                distinctCountAggregator
                                        .agg(getGlobalSurrogates(mappedDim, iterator.next()));
                            }
                            v[index++] = distinctCountAggregator;
                            currentSliceIndex = 0;
                        } else {
                            MeasureAggregator distinctCountAggregatorObjct =
                                    new DistinctStringCountAggregator();
                            while (iterator.hasNext()) {
        ...
        for (int i = 0; i < msrCount; i++) {
            ...
            if (queryModel.getMsrs().get(i).isDistinctQuery()) {
                ...
                if (queryModel.getPartitionColumns().size() == 1) {
                    if (null != mappedDim) {
                        if (queryModel.getPartitionColumns().get(0)
                                .equals(mappedDim.getColName())) {
                ...
                if (null != mappedDim) {
                    Iterator<Integer> iterator =
                            ((DistinctCountAggregator) v[i]).getBitMap().iterator();
                    if (queryModel.isAggTable()) {
                        ...
                        // CHECKSTYLE:OFF Approval No:Approval-V1R2C10_006
                        int minValue = (int) executerProperties.msrMinValue[
                                executerProperties.measureStartIndex + i];
                        // CHECKSTYLE:ON

                        while (iterator.hasNext()) {
                            distinctCountAggregator.agg(getGlobalSurrogates(mappedDim,
                                    iterator.next() - minValue));
                        }
    ...
    private int getGlobalSurrogates(Dimension columnName, int surrogate) {
        ...
        for (int i = currentSliceIndex; i < executerProperties.slices.size(); i++) {
            globalSurrogates = executerProperties.slices.get(i).getMemberCache(
                    ...
                    .getGlobalSurrogateKey(surrogate);
