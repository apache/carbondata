...
public class QueryExecutorImpl extends AbstractQueryExecutor {
    ...
    public CarbonIterator<RowResult> execute(CarbonQueryExecutorModel queryModel)
        ...
        for (InMemoryTable slice : executerProperties.slices) {
            ...
            if (slice.getTableName().equals(queryModel.getFactTable())) {
                sliceMetadataPath =
                        slice.getRsStore().getSliceMetadataPath(queryModel.getFactTable());
    ...
    private SliceExecutionInfo getSliceExecutionInfo(CarbonQueryExecutorModel queryModel,
        ...
        Dimension[] currentDimTables = RestructureUtil
                .updateRestructureHolder(queryModel.getDims(), sliceMataData, currentDimList,
        ...
        RestructureUtil.updateMeasureInfo(sliceMataData, measures, measureOrdinal, msrExists,
                newMsrsDftVal);
        ...
        FilterEvaluatorInfo filterInfo =
                getFilterInfo(queryModel, currentSliceIndex, sliceMataData);
        ...
        info.setAllSelectedDimensions(QueryExecutorUtility
                .getAllSelectedDiemnsionStoreIndex(queryDimensions,
                        queryModel.getDimensionAggInfo(), executerProperties.aggExpDimensions,
                        info.getHybridStoreMeta()));
        ...
        if (!dimensionFilter.isEmpty()) {
            getStartKey(dimensionFilter, slice.getKeyGenerator(queryModel.getFactTable()),
                    startKey);
        ...
        if (null != queryModel.getDimensionAggInfo()) {
            RestructureUtil.updateDimensionAggInfo(queryModel.getDimensionAggInfo(),
                    sliceMataData.getDimensions());
        }
        //info.setQueryDimOrdinal(QueryExecutorUtility.getSelectedDimnesionIndex(queryDimensions));
