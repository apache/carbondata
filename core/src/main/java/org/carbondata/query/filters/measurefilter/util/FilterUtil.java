...
public final class FilterUtil {
	private static final LogService LOGGER = LogServiceFactory
			.getLogService(FilterUtil.class.getName());
	...
	private static FilterExecuter createFilterExecuterTree(
		...
		FilterExecuterType filterExecuterType = filterExpressionResolverTree
	...
	public static DimColumnFilterInfo getFilterValues(
			AbsoluteTableIdentifier tableIdentifier,
...

	{
		...
		for (String resultVal : evaluateResultList) {
			...
			Integer dictionaryVal = getDictionaryValue(resultVal,
					tableIdentifier, columnExpression.getDimension());
	...
	private static Integer getDictionaryValue(String value,
		...
		Dictionary forwardDictionary = getForwardDictionaryCache(
				tableIdentifier, dim);
	...
	public static DimColumnFilterInfo getFilterListForAllValues(
		...
		Dictionary forwardDictionary =FilterUtil.getForwardDictionaryCache(tableIdentifier, columnExpression.getDimension());
		List<String> evaluateResultListFinal = new ArrayList<String>(20);
		DictionaryChunksWrapper dictionaryWrapper = forwardDictionary
				.getDictionaryChunks();
		while (dictionaryWrapper.hasNext()) {
			...
			try {
				...
				row.setValues(new Object[] { DataTypeConverter
						.getDataBasedOnDataType(stringValue, columnExpression
								.getDim().getDataType()) });
		...
		return getFilterValues(tableIdentifier, columnExpression,
				evaluateResultListFinal, isIncludeFilter);
	}

	public static DimColumnFilterInfo getFilterList(
			AbsoluteTableIdentifier tableIdentifier, Expression expression,
			ColumnExpression columnExpression, boolean isIncludeFilter) {
		DimColumnFilterInfo resolvedFilterObject = null;
		List<String> evaluateResultListFinal = new ArrayList<String>(20);
		try {
			List<ExpressionResult> evaluateResultList = expression.evaluate(
					null).getList();
			...
			if (null != columnExpression.getDim()
					&& columnExpression.getDim().isNoDictionaryDim()) {
				resolvedFilterObject = getNoDictionaryValKeyMemberForFilter(
						tableIdentifier, columnExpression,
						evaluateResultListFinal, isIncludeFilter);
			} else {
				resolvedFilterObject = getFilterValues(tableIdentifier,
						columnExpression, evaluateResultListFinal,
						isIncludeFilter);
			}
		} catch (FilterUnsupportedException e) {
			LOGGER.audit(e.getMessage());
	...
	public static DimColumnFilterInfo getFilterListForRS(Expression expression,
			ColumnExpression columnExpression, String defaultValues,
	...
	public static DimColumnFilterInfo getFilterListForAllMembersRS(
			Expression expression, ColumnExpression columnExpression,
			String defaultValues, int defaultSurrogate, boolean isIncludeFilter) {
		List<Integer> filterValuesList = new ArrayList<Integer>(20);
		List<String> evaluateResultListFinal = new ArrayList<String>(20);
		DimColumnFilterInfo columnFilterInfo = null;
		...
		try {
			...
			row.setValues(new Object[] { DataTypeConverter
					.getDataBasedOnDataType(defaultValues, columnExpression
							.getDim().getDataType()) });
			Boolean rslt = expression.evaluate(row).getBoolean();
			if (null != rslt && !(rslt ^ isIncludeFilter)) {
	...
	public static byte[][] getKeyArray(DimColumnFilterInfo dimColumnFilterInfo,
		...
		int[] rangesForMaskedByte = getRangesForMaskedByte(
				(carbonDimension.getOrdinal()), blockLevelKeyGenerator);
	 ...
	 */
	public static long[] getStartKey(
			DimColumnResolvedFilterInfo dimColResolvedFilterInfo,
			KeyGenerator keyGen) {
		long[] startKey = new long[keyGen.getDimCount()];
		Map<CarbonDimension, List<DimColumnFilterInfo>> dimensionFilter = dimColResolvedFilterInfo
				.getDimensionResolvedFilterInstance();
		for (Entry<CarbonDimension, List<DimColumnFilterInfo>> entry : dimensionFilter
				.entrySet()) {
	...
	private static List<CarbonDimension> getCarbonDimsMappedToKeyGenerator(
			List<CarbonDimension> carbonDimensions) {
		List<CarbonDimension> listOfCarbonDimPartOfKeyGen = new ArrayList<CarbonDimension>(
				carbonDimensions.size());
		for (CarbonDimension carbonDim : carbonDimensions) {
			if (CarbonUtil.hasEncoding(carbonDim.getEncoder(),
					Encoding.DICTIONARY)) {
		...
		for (Entry<CarbonDimension, List<DimColumnFilterInfo>> entry : dimensionFilter
			...
			for (DimColumnFilterInfo info : values) {
				if (endKey[entry.getKey().getOrdinal()] > info.getFilterList()
					...
					endKey[entry.getKey().getOrdinal()] = info.getFilterList()
							.get(info.getFilterList().size() - 1);
				}
			}
		}
	...
	private static long getMaxValue(AbsoluteTableIdentifier tableIdentifier,
		...
		Dictionary forwardDictionary = getForwardDictionaryCache(
				tableIdentifier, carbonDimension);
		if (null == forwardDictionary) {
			return -1;
		}
	...
	public static Dictionary getForwardDictionaryCache(
		...
		Cache forwardDictionaryCache = cacheProvider.createCache(
				CacheType.FORWARD_DICTIONARY, tableIdentifier.getStorePath());
