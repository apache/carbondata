...
public class FilterExpressionProcessor implements FilterProcessor {
	...
	private FilterResolverIntf getFilterResolvertree(Expression expressionTree,
			AbsoluteTableIdentifier tableIdentifier) {
		FilterResolverIntf filterEvaluatorTree = createFilterResolverTree(
				expressionTree, tableIdentifier);
	 ...
	 * @param tableIdentifier
	 */
	private void traverseAndResolveTree(
			FilterResolverIntf filterResolverTree,
			AbsoluteTableIdentifier tableIdentifier) {
	...
	private FilterResolverIntf createFilterResolverTree(
			Expression expressionTree, AbsoluteTableIdentifier tableIdentifier) {
		ExpressionType filterExpressionType = expressionTree
		...
		case AND:
			...
			return new LogicalFilterResolverImpl(createFilterResolverTree(
					currentExpression.getLeft(), tableIdentifier),
					createFilterResolverTree(currentExpression.getRight(),
							tableIdentifier), filterExpressionType);
		case EQUALS:
		case IN:
			return getFilterResolverBasedOnExpressionType(
					ExpressionType.EQUALS, false, expressionTree,
		...
		case LESSTHAN_EQUALTO:
			return getFilterResolverBasedOnExpressionType(
					ExpressionType.EQUALS, true, expressionTree,
					tableIdentifier, expressionTree);

		case NOT_EQUALS:
		case NOT_IN:
			return getFilterResolverBasedOnExpressionType(
					ExpressionType.NOT_EQUALS, false, expressionTree,
					tableIdentifier, expressionTree);

		default:
			return getFilterResolverBasedOnExpressionType(
					ExpressionType.UNKNOWN, false, expressionTree,
					tableIdentifier, expressionTree);
		}
	}
	...
	private FilterResolverIntf getFilterResolverBasedOnExpressionType(
			...
			Expression expression, AbsoluteTableIdentifier tableIdentifier,
		...
		case EQUALS:
			...
			if (currentCondExpression.isSingleDimension()
					&& currentCondExpression.getColumnList().get(0).getDim()
							.getDataType() != Type.ARRAY
					&& currentCondExpression.getColumnList().get(0).getDim()
							.getDataType() != Type.STRUCT) {
				// getting new dim index.
				if (currentCondExpression.getColumnList().get(0).getDim()
						.isNoDictionaryDim()) {
					if (FilterUtil
							.checkIfExpressionContainsColumn(currentCondExpression
									.getLeft())
							|| FilterUtil
									.checkIfExpressionContainsColumn(currentCondExpression
											.getRight())) {
						return new RowLevelFilterResolverImpl(expression,
								isExpressionResolve, true, tableIdentifier);
					}
					...
					if (FilterUtil
							...
							|| FilterUtil
									.checkIfExpressionContainsColumn(currentCondExpression
											.getRight())) {
						return new RowLevelFilterResolverImpl(expression,
								isExpressionResolve, false, tableIdentifier);
					}
					if (expressionTree.getFilterExpressionType() == ExpressionType.GREATERTHAN
							|| expressionTree.getFilterExpressionType() == ExpressionType.LESSTHAN
							|| expressionTree.getFilterExpressionType() == ExpressionType.GREATERYHAN_EQUALTO
		...
		default:
			...
			if (condExpression.isSingleDimension()
				...
				if (condExpression.isSingleDimension()) {
					...
					if (condExpression.getColumnList().get(0).getDim()
							.isNoDictionaryDim()) {
						if (FilterUtil
								.checkIfExpressionContainsColumn(currentCondExpression
										.getLeft())
