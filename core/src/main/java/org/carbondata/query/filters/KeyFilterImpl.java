...
public class KeyFilterImpl implements InMemoryFilter {
    ...
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(KeyFilterImpl.class.getName());
    /**
     *
     */
     ...
     */
    protected void createTempByteAndRangeLengths(byte[][] temps, int[] lengths,
            int[] dimensionOffset, int[][] ranges) {
        if (lengths.length > 0) {
            for (int i = 0; i < dimensionOffset.length; i++) {
                lengths[i] =
                        ranges[dimensionOffset[i]] == null ? 0 : ranges[dimensionOffset[i]].length;
    ...
    @Override public boolean filterKey(KeyValue key) {
        ...
        for (int i = 0; i < dimensionOffset.length; i++) {
            ...
            int searchresult = binarySearch(allowedValuesForDim, key.backKeyArray, key.keyOffset,
                    maxKeyBytes[dimensionOffset[i]], ranges[dimensionOffset[i]], temps[i],
                    lengths[i]);
            if (searchresult < 0) {
                return false;
            }
    ...
    public boolean filterKey(byte[] key) {
        ...
        for (int i = 0; i < dimensionOffset.length; i++) {
            ...
            searchresult[i] =
                    binarySearch(allowedValuesForDim, key, 0, maxKeyBytes[dimensionOffset[i]],
                            ranges[dimensionOffset[i]], temps[i], lengths[i]);
        }
     ...
     * @return
     */
    protected int binarySearch(byte[][] array, byte[] key, int offset, byte[] maxKey, int[] range,
            byte[] temp, int length) {
        try {
            ...
            while (low <= high) {
                ...
                int cmp = UnsafeComparer.INSTANCE
                        .compareTo(midVal, temp);//ByteUtil.compare(midVal, temp);
                //
                if (cmp < 0) {
                    low = mid + 1;
                } else if (cmp > 0) {
            ...
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG, e,
                    "Execption occured while searching");
        }
        return 1;
    }
