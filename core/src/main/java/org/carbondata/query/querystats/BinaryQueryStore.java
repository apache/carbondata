...
public class BinaryQueryStore implements QueryStore {
  ...
  private static final LogService LOGGER =
      LogServiceFactory.getLogService(BinaryQueryStore.class.getName());
   ...
   * @return
   */
  private static String getQueryStatsPath(String metaPath, String schemaName, String cubeName,
  ...
  public void logQuery(QueryDetail queryDetail) {
    ...
    String queryStatsPath =
        getQueryStatsPath(queryDetail.getMetaPath(), queryDetail.getSchemaName(),
            queryDetail.getCubeName(), queryDetail.getFactTableName());
    ...
    String queryStatsfile = queryStatsPath + File.separator + Preference.QUERYSTATS_FILE_NAME;
    try {
  ...
  private synchronized void writeQueryToFile(QueryDetail queryDetail, DataOutputStream dos)
      throws IOException {
  ...
  public void writeQueryToFile(List<QueryDetail> queryDetails, String queryStatsPath) {
    ...
    try {
      ...
      if (FileFactory.getCarbonFile(queryStatsPath, fileType).delete()) {
        if (!FileFactory.getCarbonFile(tempQueryStatsPath, fileType).renameTo(queryStatsPath)) {
          LOGGER.info(CarbonEngineLogEvent.UNIBI_CARBONENGINE_MSG,
