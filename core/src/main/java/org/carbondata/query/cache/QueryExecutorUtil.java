...
public final class QueryExecutorUtil {
     ...
     */
    public static long[][][] getValidRanges(InMemoryCube slice, long[][] ranges, long[] startKey,
            long[] endKey) {
        ...
        if (ranges != null && ranges.length > 0) {
            ...
            for (int i = lowerIndex;
                 i <= upperindex; i++) {//CHECKSTYLE:OFF    Approval No:Approval-281
     ...
     * @return
     */
    public static void getMemberIdByName(List<InMemoryCube> slices, Dimension columnName,
     ...
     * @return
     */
    public static void getMemberIdByActualName(List<InMemoryCube> slices, Dimension columnName,
            String name, List<Long> surrogatesActual) {
        List<Long> surrogates = new ArrayList<Long>(10);
        if (null != slices && null != columnName)//Coverity Fix add
        {
    ...
    public static void getFiltersFromIterator(int[] ordinals, MolapResultHolder iter,
            MolapFilterInfo filterInfo, boolean include) {
...
//                        }
     ...
     * @return
     */
    public static long[] getSurrogates(List<String> dimMem, Dimension dimName,
            List<InMemoryCube> slices, boolean sortRequired, boolean isCompoundPredicate) {
    ...
    private static long[] convertToLongArraySurrogates(List<Long> surrogates, Dimension dimName) {
        ...
        for (int i = 0; i < surrogates.size(); i++) {
            ...
            if (surg > 0) {
                ...
                LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG,
                        " Member does not exist for level " + dimName.getName());
                ls[i] = Long.MAX_VALUE;
            }
        }
        return ls;
     ...
     * @return
     */
    private static void getSurrogate(Dimension dimName, List<InMemoryCube> slices, String dimension,
            List<Long> surrogates, boolean isCompoundPredicate) {
        ...
        if (null == dimName) {
            LOGGER.error(MolapEngineLogEvent.UNIBI_MOLAPENGINE_MSG,
                    dimension + " Member does not exist for level is null");
            return;
        }
     ...
     * @throws KeyGenException
     */
    public static void computeColIncludePredicateKeys(Map<Dimension, MolapFilterInfo> pred,
            InMemFilterModel filterModel, Dimension[] dimTables, KeyGenerator keyGenerator,
            List<InMemoryCube> slices) throws IOException, KeyGenException {
        //
        ...
        for (Entry<MolapMetadata.Dimension, MolapFilterInfo> entry : pred.entrySet()) {
            //
            Dimension dim = entry.getKey();
            List<String[]> updatedIncludedMembers =
                    getUpdatedConstarints(entry.getValue().getEffectiveIncludedMembers());
            List<String> effectiveIncludedMembers =
                    getEffectiveIncludedMembers(dim, updatedIncludedMembers);
            ...
            if (null != vals && vals.length > 0) {
                ...
                includeFiltersOnDim[dim.getOrdinal()] =
                        getFilterByteArray(vals, dim.getOrdinal(), dimTables, keyGenerator);
    ...
    private static List<String> getEffectiveIncludedMembers(Dimension dim,
            List<String[]> updatedIncludedMembers) {
     ...
     * @throws KeyGenException
     */
    public static void computeColIncludeOrPredicateKeys(Map<Dimension, MolapFilterInfo> pred,
            InMemFilterModel filterModel, Dimension[] dimTables, KeyGenerator keyGenerator,
            List<InMemoryCube> slices) throws IOException, KeyGenException {
        //
        List<Integer> dimFilterOffSet = new ArrayList<Integer>(10);
        ...
        for (Entry<MolapMetadata.Dimension, MolapFilterInfo> entry : pred.entrySet()) {
            ...
            Dimension dim = entry.getKey();
            List<String[]> updatedIncludedMembers =
                    getUpdatedConstarints(entry.getValue().getIncludedOrMembers());
            List<String> effectiveIncludedMembers =
                    getEffectiveIncludedMembers(dim, updatedIncludedMembers);
            long[] vals = getSurrogates(effectiveIncludedMembers, dim, slices, true, false);
     ...
     */
    public static Map<Dimension, MolapFilterInfo> getUpdatedConstraints(Cube cube,
            Map<Dimension, MolapFilterInfo> constraints) {
        Map<Dimension, MolapFilterInfo> newconstraints =
                new HashMap<MolapMetadata.Dimension, MolapFilterInfo>(constraints);
        ...
        for (Entry<Dimension, MolapFilterInfo> entry : constraints.entrySet()) {
            ...
            List<Dimension> dimList =
                    cube.getHierarchiesMapping(key.getDimName() + '_' + key.getHierName());
     ...
     * @throws KeyGenException
     */
    public static void computeColExcludePredicateKeys(Map<Dimension, MolapFilterInfo> pred,
            InMemFilterModel filterModel, Dimension[] dimTables, KeyGenerator keyGenerator,
            List<InMemoryCube> slices) throws IOException, KeyGenException {
        List<Integer> dimFilterOffSet = new ArrayList<Integer>(10);
        ...
        {
            Dimension dim = entry.getKey();
            List<String[]> updatedExcludedMembers =
                    getUpdatedConstarints(entry.getValue().getEffectiveExcludedMembers());
            List<String> effectiveExcludedMembers =
                    getEffectiveIncludedMembers(dim, updatedExcludedMembers);
            ...
            if (null != vals) {
                ...
                dimFilterOffSet.add(dim.getOrdinal());
                excludeFiltersOnDim[dim.getOrdinal()] =
                        getFilterByteArray(vals, dim.getOrdinal(), dimTables, keyGenerator);
     ...
     * @throws KeyGenException
     */
    private static byte[][] getFilterByteArray(long[] filter, int dimensionOrdinal,
            Dimension[] dimTables, KeyGenerator keyGenerator) throws KeyGenException {
        ...
        for (int j = 0; j < filter.length; j++) {
            ...
            dimFil[j] = getMaskedKey(
                    getRangesForMaskedByte(new int[] { dimensionOrdinal }, keyGenerator),
                    keyGenerator.generateKey(keys));
        }
        return dimFil;
    }
    ...
    public static void computeColIncludePredicateKeys(Map<Dimension, MolapFilterInfo> pred,
            InMemFilterModel filterModel, Dimension[] dimTables, KeyGenerator keyGenerator,
            int[][] groups, List<InMemoryCube> slices, Cube cube, String factTableName)
        ...
        for (Entry<MolapMetadata.Dimension, MolapFilterInfo> entry : pred.entrySet()) {
            ...
            Dimension dim = entry.getKey();
            List<String[]> effectiveIncludedMembers =
                    getUpdatedConstarints(entry.getValue().getIncludedMembers());
            ...
            if (effectiveIncludedMembers.size() > 0) {
                ...
                if (null != factTableName) {
                    setFilters(effectiveIncludedMembers, includedMembers, dim, groups, dimTables,
                            cube, factTableName);
                } else {
                    setFilters(effectiveIncludedMembers, includedMembers, dim, groups, dimTables,
                            cube, null);
        ...
        for (int i = 0; i < groups.length; i++) {
            ...
            for (int j = 0; j < groups[i].length; j++) {
                if (groups[i][j] != -1) {
                    ...
                    fillFilters(includedMembers[groups[i][j]], vals, j, groups[i], keymax, slices,
                            dimTables, preds, ranges);

                    includeFiltersOnDim[groups[i][j]] =
                            getFilterByteArray(vals, groups[i], j, dimTables, keyGenerator);
                    if (includeFiltersOnDim[groups[i][j]].length > 0) {
                        //                        filterModel.getMaxKey()[groups[i][j]] = keyGenerator.generateKey(keymax);

                        byte[] maxKey = keyGenerator.generateKey(keymax);
                        int[] rangeArry =
                                getRangesForMaskedByte(convertIntegerListToIntArray(ranges),
                                        keyGenerator);
     ...
     * @param preds
     */
    private static void fillFilters(List<String[]> includedMember, long[][] vals, int groupIndex,
            int[] group, long[] keymax, List<InMemoryCube> slices, Dimension[] dimTables,
            Map<Integer, long[]> preds, List<Integer> ranges) {
        ...
        for (int k = 0; k < includedMember.size(); k++) {
            ...
            for (int i = size - 1; i >= 0; i--, ordinal--) {
                ...
                tempMems[i] =
                        getSurrogates(members, getDimensionByOrdinal(dimTables, ordinal), slices,
                                false, false);
     ...
     * @param preds
     */
    private static void fillFiltersForSubtotal(List<String[]> includedMember, long[][] vals,
            int groupIndex, int[] group, long[] keymax, List<InMemoryCube> slices,
            Dimension[] dimTables, Map<Integer, long[]> preds, List<Integer> ranges) {
        if (includedMember == null || includedMember.size() == 0) {
            return;
        }
     ...
     * @param i
     */
    private static void checkAndUpdateFilters(long[][] vals, Map<Integer, long[]> preds,
            int ordinal, long[] mems, boolean updatePreds) {
    ...
    public static void computeColExcludePredicateKeys(Map<Dimension, MolapFilterInfo> pred,
        ...
        //for(Iterator<Dimension> iterator = pred.keySet().iterator();iterator.hasNext();)
        {
            Dimension dim = entry.getKey();
            List<String[]> effectiveExcludedMembers =
                    getUpdatedConstarints(entry.getValue().getExcludedMembers());

            if (effectiveExcludedMembers.size() > 0) {
                setFilters(effectiveExcludedMembers, excludedMembers, dim, groups, dimTables, cube,
                        factTableName);
            }
        }
        Map<Integer, long[]> preds = new HashMap<Integer, long[]>(10);
        for (int i = 0; i < groups.length; i++) {
            ...
            for (int j = 0; j < groups[i].length; j++) {
                if (groups[i][j] != -1) {
                    ...
                    List<Integer> ranges = new ArrayList<Integer>(10);
                    fillFilters(excludedMembers[groups[i][j]], vals, j, groups[i], keymax, slices,
                            dimTables, preds, ranges);

                    excludeFiltersOnDim[groups[i][j]] =
                            getFilterByteArray(vals, groups[i], j, dimTables, keyGenerator);
                    if (excludeFiltersOnDim[groups[i][j]].length > 0) {
                        ...
                        int[] rangeArray =
                                getRangesForMaskedByte(convertIntegerListToIntArray(ranges),
                                        keyGenerator);
                        filterModel.getMaxKeyExclude()[groups[i][j]] =
                                getMaskedKey(rangeArray, maxKey);
     ...
     * @throws KeyGenException
     */
    public static void computeColIncludePredicateKeysForSubtotal(
            Map<Dimension, MolapFilterInfo> pred, InMemFilterModel filterModel,
            Dimension[] dimTables, KeyGenerator keyGenerator, int[][] groups,
            List<InMemoryCube> slices, Cube cube, String factTableName)
            throws IOException, KeyGenException {
        //
        List<Integer> dimFilterOffSet = new ArrayList<Integer>(10);
        byte[][][] includeFiltersOnDim = new byte[dimTables.length][][];
        ...
        Map<Integer, long[]> preds =
                new HashMap<Integer, long[]>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        for (int i = 0; i < groups.length; i++) {
            ...
            for (int j = 0; j < groups[i].length; j++) {
                if (groups[i][j] != -1) {
                    ...
                    fillFiltersForSubtotal(includedMembers[groups[i][j]], vals, j, groups[i],
                            keymax, slices, dimTables, preds, ranges);
    ...
    private static void setFilters(List<String[]> filter, List<String[]>[] filters, Dimension dim,
            int[][] groups, Dimension[] dimTables, Cube cube, String factTableName) {

        Map<Integer, List<String[]>> filterMap =
                new HashMap<Integer, List<String[]>>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        ...
        List<Dimension> hierarchiesMapping =
                cube.getHierarchiesMapping(dim.getDimName() + '_' + dim.getHierName());
        for (Entry<Integer, List<String[]>> entry : filterMap.entrySet()) {
            ...
            if (null == factTableName) {
                updatedOrdinal =
                        cube.getDimensionByLevelName(dimLocal.getDimName(), dimLocal.getHierName(),
                                dimLocal.getName()).getOrdinal();
            } else {
                updatedOrdinal =
                        cube.getDimensionByLevelName(dimLocal.getDimName(), dimLocal.getHierName(),
                                dimLocal.getName(), factTableName).getOrdinal();
            }
            ...
            for (int i = 0; i < entry.getValue().size(); i++) {
                ...
                int k = 0;
                for (int j = string.length - 1; j >= 0; j--, k++) {
                    if (dimTables[updatedOrdinal - k].getDataType()
                            .equals(SqlStatement.Type.DOUBLE)) {
                        convertToDouble(string, j);
                    } else if (dimTables[updatedOrdinal - k].getDataType()
                            .equals(SqlStatement.Type.INT)) {
                        convertToInt(string, j);
                    }
                }
    ...
    private static void setFiltersForSubtotal(List<String[]> filter, List<String[]>[] filters,
            Dimension dim, Dimension[] dimTables) {
     ...
     */
    private static byte[][] getFilterByteArray(long[][] filter, int[] dimensionOrdinal,
            int groupIndex, Dimension[] dimTables, KeyGenerator keyGenerator)
            throws KeyGenException {
        int len = 0;
        ...
        for (int j = 0; j < len; j++) {
            ...
            dimFil.add(getMaskedKey(
                    getRangesForMaskedByte(convertIntegerListToIntArray(range), keyGenerator),
                    keyGenerator.generateKey(keys)));
        }
     ...
     * @param constraints
     */
    public static int[][] getDimensionGroups(Cube cube,
            Map<Dimension, MolapFilterInfo> constraints) {
        ...
        for (Entry<Dimension, MolapFilterInfo> entry : constraints.entrySet()) {
            ...
            String keyStr =
                    firstLevel.getDimName() + '_' + firstLevel.getHierName() + '_' + firstLevel
                            .getName();
     ...
     * @param constraints
     */
    public static int[][] getDimensionGroupsForDifferentDimAsOneGroup(Cube cube,
            Map<Dimension, MolapFilterInfo> constraints, String factTableName) {
        Map<String, int[]> map = new LinkedHashMap<String, int[]>();
        List<Dimension> dimList = cube.getDimensions(factTableName);
        Dimension firstLevel = dimList.get(0);
        String keyStr = firstLevel.getDimName() + '_' + firstLevel.getHierName() + '_' + firstLevel
                .getName();
    ...
    private static int getDimIndex(List<Dimension> dimList, Dimension dim) {
        ...
        for (Dimension dimLoc : dimList) {
            if (dim.getName().equals(dimLoc.getName()) && dim.getDimName()
                    .equals(dimLoc.getDimName()) && dim.getHierName().equals(dimLoc.getHierName())
                    && dim.getTableName().equals(dimLoc.getTableName())) {
                return i;
            }
            i++;
        }
     ...
     * @return
     */
    public static Map<Dimension, MolapPredicates> getMolapPredicates(
            Map<Dimension, MolapFilterInfo> constraints, Dimension[] dimTables,
            List<InMemoryCube> slices, boolean isActual, boolean isFilterinGroup, Cube cube,
            String factTableName) {
        Map<Dimension, MolapPredicates> preds =
                new LinkedHashMap<MolapMetadata.Dimension, MolapPredicates>();
        ...
        for (Entry<Dimension, MolapFilterInfo> entry : constraints.entrySet()) {
            ...
            List<String[]> effectiveExcludedMembers =
                    getUpdatedConstarints(entry.getValue().getExcludedMembers());
            List<String[]> effectiveIncludedOrMembers =
                    getUpdatedConstarints(entry.getValue().getIncludedOrMembers());

            Map<Integer, List<String>> incPreds =
                    getUpdatePredicateList(entry.getKey(), effectiveIncludedMembers,
                            isFilterinGroup, cube, factTableName);
            Map<Integer, List<String>> exPreds =
                    getUpdatePredicateList(entry.getKey(), effectiveExcludedMembers,
                            isFilterinGroup, cube, factTableName);
            Map<Integer, List<String>> incOrPreds =
                    getUpdatePredicateList(entry.getKey(), effectiveIncludedOrMembers,
                            isFilterinGroup, cube, factTableName);
    ...
    private static boolean isActualInDimension(Map<Dimension, MolapFilterInfo> constraints,
            Dimension key) {
    ...
    private static Map<Integer, List<String>> getUpdatePredicateList(Dimension dim,
            List<String[]> effectiveIncludedMembers, boolean isFilterinGroup, Cube cube,
            String factTableName) {
        Map<Integer, List<String>> incPreds =
                new HashMap<Integer, List<String>>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
        for (String[] mems : effectiveIncludedMembers) {
            ...
            if (isFilterinGroup) {
                ...
                for (int i = mems.length - 1; i >= 0; i--, indexOf--) {
                    ...
                    ordinal = cube.getDimensionByLevelName(dimLocal.getDimName(),
                            dimLocal.getHierName(), dimLocal.getName(), factTableName).getOrdinal();
     ...
     * @return
     */
    public static Map<Dimension, MolapFilterInfo> updateConstraintsAsperDimensions(
            Map<Dimension, MolapFilterInfo> constraints, List<Dimension> allDims) {
        Map<Dimension, MolapFilterInfo> updatedCons =
        ...
        for (Entry<Dimension, MolapFilterInfo> entry : constraints.entrySet()) {
            ...
            for (Dimension dimension : allDims) {
                if (key.getDimName().equals(dimension.getDimName()) && key.getHierName()
                        .equals(dimension.getHierName()) && key.getName()
     ...
     *
     * @param newConstraints    newConstraints
     * @param currentDimeTables currentDimeTables
     * @return boolean
     */
    public static boolean updateFilterForOlderSlice(Map<Dimension, MolapFilterInfo> newConstraints,
        ...
        List<Entry<Dimension, MolapFilterInfo>> entryList =
                new ArrayList<Entry<Dimension, MolapFilterInfo>>(
    ...
    public static boolean containWithSameColumnName(Dimension dimension, List<Dimension> dimList) {
        for (Dimension dim : dimList) {
            if (dim.getHierName().equals(dimension.getHierName()) && dim.getDimName()
                    .equals(dimension.getDimName()) && dim.getColName()
                    .equals(dimension.getColName())) {
                return true;
            }
        }
        return false;
     ...
     * @throws KeyGenException
     */
    public static byte[] getMaskedBytesForTopN(Dimension[] queryDimensions, KeyGenerator generator,
            int topNdimIndex, Dimension[] actualDims, Dimension[] actualDimsRows,
            Dimension[] actualDimsCols, Dimension topNDim, boolean considerTopN,
        ...
        for (int i = 0; i <= index; i++) {
            ...
            for (int j = 0; j < queryDimensions.length; j++) {
                if (queryDimensions[j].getHierName().equals(dimension.getHierName())
                        && queryDimensions[j].getDimName().equals(dimension.getDimName())) {
                    if (queryDimensions[j].getHierName().equals(topNDim.getHierName())
                            && queryDimensions[j].getDimName().equals(topNDim.getDimName())) {
        ...
        byte[] maskedKey =
                getMaskedKeyForTopN(generator, maskedKeyRanges, ranges, considerList, byteIndexs);
        return maskedKey;
    }
    ...
    public static int getTopNIndex(Dimension[] actualDims, Dimension topNDim) {
        ...
        for (int i = 0; i < actualDims.length; i++) {
            if (actualDims[i].getHierName().equals(topNDim.getHierName()) && actualDims[i]
                    .getDimName().equals(topNDim.getDimName()) && actualDims[i].getName()
                    .equals(topNDim.getName())) {
                index = i;
                break;
            }
        }
    ...
    private static byte[] getMaskedKeyForTopN(KeyGenerator generator, int[] maskedKeyRanges,
            List<Integer> ranges, List<Dimension> considerList, int[] byteIndexs)
            throws KeyGenException {
