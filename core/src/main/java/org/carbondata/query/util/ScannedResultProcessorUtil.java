...
public final class ScannedResultProcessorUtil {
    ...
    public static String[] getDimensionUniqueNames(Dimension[] queryDimension) {
        ...
        for (int i = 0; i < uniqueDims.length; i++) {
            uniqueDims[i] =
                    queryDimension[i].getTableName() + '_' + queryDimension[i].getColName() + '_'
                            + queryDimension[i].getDimName() + '_' + queryDimension[i]
                            .getHierName();
        }
        return uniqueDims;
    }
     ...
     */
    public static DataFileWriter.KeyValueHolder[] getSortedResult(
            DataProcessorInfo dataProcessorInfo, Result scannedResult, Comparator comparator)
            throws QueryExecutionException {
        ...
        try {
            while (scannedResult.hasNext()) {
                ...
                if (null != comparator) {
                    ...
                    long[] keyArray = keyGenerator
                            .getKeyArray(maskedKey, dataProcessorInfo.getMaskedByteRange());
                    for (int i = 0; i
                        ...
                        if (sortedDimensionIndex[i] == 1) {
                            keyArray[queryDimension[i].getOrdinal()] =
                                    getSortIndexById(dimensionUniqueNames[i],
     ...
     * @return
     */
    public static ComparatorChain getResultTempFileReaderComprator(
            int[][] maskedByteRangeForSorting, byte[] dimensionSortOrder, byte[][] dimensionMasks) {
        if (dimensionSortOrder.length < 1) {
            return null;
        }
        ...
        for (int i = 0; i < maskedByteRangeForSorting.length; i++) {
            ...
            keyComparator = new MaksedByteComparatorForReader(maskedByteRangeForSorting[i],
                    dimensionSortOrder[i], dimensionMasks[i]);
            compratorList.add(keyComparator);
        }
        return new ComparatorChain(compratorList);
    }
