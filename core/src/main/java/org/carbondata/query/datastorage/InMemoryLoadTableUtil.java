...
public final class InMemoryLoadTableUtil {
  ...
  public static int getQueryMaxTimeOut() {
    int queryTimeOut = 0;
    String timeOutValue =
        CarbonProperties.getInstance().getProperty(CarbonCommonConstants.MAX_QUERY_EXECUTION_TIME);
    try {
  ...
  public static List<InMemoryTable> getQuerySlices(List<InMemoryTable> activeSlices,
    ...
    for (InMemoryTable inMemoryTable : activeSlices) {
      Long modificationTimeStamp = loadNameAndModicationTimeMap.get(inMemoryTable.getLoadName());
