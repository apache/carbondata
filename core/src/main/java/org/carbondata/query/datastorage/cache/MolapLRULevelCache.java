...
public final class MolapLRULevelCache {
    ...
    private MolapLRULevelCache() {
        try {
            levelCacheMemorySize = Integer.parseInt(MolapProperties.getInstance()
                    .getProperty(MolapCommonConstants.CARBON_MAX_LEVEL_CACHE_SIZE,
                            MolapCommonConstants.CARBON_MAX_LEVEL_CACHE_SIZE_DEFAULT));
        } catch (NumberFormatException e) {
            levelCacheMemorySize =
                    Integer.parseInt(MolapCommonConstants.CARBON_MAX_LEVEL_CACHE_SIZE_DEFAULT);
        }
    ...
    private void initCache() {
        levelCache = Collections.synchronizedMap(
                new LinkedHashMap<String, LevelInfo>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE,
                        1.0f, true));
    }
