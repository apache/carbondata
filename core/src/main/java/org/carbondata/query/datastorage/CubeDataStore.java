...
public class CubeDataStore {
    ...
    private static final LogService LOGGER =
            LogServiceFactory.getLogService(CubeDataStore.class.getName());
    /**
     *
     */
    ...
    public CubeDataStore(String table, Cube metaCube, SliceMetaData smd, KeyGenerator keyGenerator,
    ...
    protected DataStore getDataStoreDS(KeyGenerator keyGen, int msrCount, int[] keyblockSize,
        ...
        boolean isForcedInMemoryCube = Boolean.parseBoolean(MolapProperties.getInstance()
                .getProperty(MolapCommonConstants.IS_FORCED_IN_MEMORY_CUBE,
    ...
    public boolean loadDataFromFile(String filesLocaton, int startAndEndKeySize) {
        ...
        List<DataInputStream> streams =
                new ArrayList<DataInputStream>(MolapCommonConstants.CONSTANT_SIZE_TEN);
        if (file.isDirectory()) {
            ...
            for (MolapFile aFile : files) {
                streams.add(MolapDataInputStreamFactory
            ...
            if (molapFile.getName().endsWith(MolapCommonConstants.FACT_DELETE_EXTENSION)) {
                for (MolapFile molapArrayFiles : files) {
                    String factFileNametoRemove = molapArrayFiles.getName().substring(0,
                            molapFile.getName()
                                    .indexOf(MolapCommonConstants.FACT_DELETE_EXTENSION));
    ...
    private void checkIsColumnar(int numberOfValues) {
        ...
        if (isColumnar) {
            ...
            if (this.dimCardinality.length > 0) {
                keyBlockSize = new MultiDimKeyVarLengthVariableSplitGenerator(MolapUtil
