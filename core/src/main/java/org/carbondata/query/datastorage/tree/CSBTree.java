...
public class CSBTree implements DataStore {
   ...
   */
  private static final LogService LOGGER = LogServiceFactory.getLogService(CSBTree.class.getName());
  /**
  ...
  private void setRangeSplitvalue() {
    ...
    LOGGER.info("Range Split value for parallel execution of a tree : " + rangeSplitValue);
  ...
  public void buildColumnar(List<DataInputStream> sources, boolean hasFactCount, Cube metaCube) {
    ...
    for (DataInputStream source : sources) {
      ...
      if (null != blockletInfoList) {
        if (blockletInfoList.size() > 0) {
          ...
          LOGGER.info("Processing : " + (blockletInfoList.get(0).getFileName()) + " : " + (
    ...
    if (num == 0) {
      root =
          new CSBInternalNode(upperMaxEntry, keyGenerator.getStartAndEndKeySizeWithOnlyPrimitives(),
              tableName);
      return;
    }
    ...
    LOGGER.info("Compress Time:" + (compressionEnd - compressionStart) + "ms");
  }
  ...
  public void build(List<DataInputStream> sources, boolean hasFactCount) {
    ...
    nTotalKeys = num;
    LOGGER.info("*************************Total Number Rows In " + tableName + " : " + nTotalKeys);
    this.rangeValues =
        (rangeVals.size() == 0) ? null : rangeVals.toArray(new long[rangeVals.size()][]);
    long compressionStart = System.currentTimeMillis();
    long compressionEnd = System.currentTimeMillis();
  ...
  private List<long[]> caclulateRanges(long num, ArrayList<CSBNode[]> nodeGroups,
    ...
    LOGGER.info("New Range Split Value: " + rangeSplitValue);
    ...
    while (low <= high) {
      ...
      if (compareRes < 0) {
        ...
        while (currentPos - 1 >= 0 && keyGenerator
            .compare(bakArray, ((currentPos - 1) * len), len, bakArray, ((currentPos) * len), len)
            == 0) {
          currentPos--;
        }
        mid = currentPos;
  ...
  private class CSBTreePart {
     ...
     *
     */
    private List<long[]> rangeVals = new ArrayList<long[]>(CarbonCommonConstants.CONSTANT_SIZE_TEN);
