...
public final class InMemoryCubeStore {
    ...
    private static ConcurrentHashMap<String, CubeLockInstance> mapOfCubeInstance =
            new ConcurrentHashMap<String, CubeLockInstance>(
     ...
     *
     */
    private Map<String, MolapDef.Cube> cubeNameAndCubeMap =
            new ConcurrentHashMap<String, MolapDef.Cube>(
                    MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    /**
    ...
    private Map<String, Byte> queryExecuteStatusMap =
            new ConcurrentHashMap<String, Byte>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    /**
     * contains the mapping of cubeName to Schema Map<cubeName, schema>
     *
     * @author Sojer z00218041
     */
    private Map<String, MolapDef.Schema> mapCubeToSchema =
            new ConcurrentHashMap<String, MolapDef.Schema>(
                    MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
    private Map<String, Integer> tableAndCurrentRSMap =
            new ConcurrentHashMap<String, Integer>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);

    private Map<String, Long> cubeNameAndCreationTime =
            new ConcurrentHashMap<String, Long>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
     ...
     *                              RS_0
     */
    public void loadCube(MolapDef.Schema schema, Cube metadataCube, String partitionId,
        ...
        synchronized (mapOfCubeInstance.get(cubeUniqueName)) {
            ...
            if (null != file && file.exists() && null != list && list.length != 0) {
                if (null == slices) {
                    slices =
                            new ArrayList<RestructureStore>(MolapCommonConstants.CONSTANT_SIZE_TEN);
                }
                    ...
                    Collections.sort(slices, new Comparator<RestructureStore>() {
                        public int compare(RestructureStore o1, RestructureStore o2) {
                            ...
                            int lastIndexOffile1 = firstFileName
                                    .lastIndexOf(MolapCommonConstants.RESTRUCTRE_FOLDER);
     ...
     * @return
     */
    public Cube loadCubeMetadataIfRequired(MolapDef.Schema schema, MolapDef.Cube cube,
        ...
        CubeLockInstance lockReference =
                mapOfCubeInstance.putIfAbsent(cubeUniqueName, cubeLockInstance);
    ...
    public List<String> loadRequiredLevels(final String cubeUniqueName, Set<String> columns,
        ...
        List<LevelInfo> notLoadedLevels =
                new ArrayList<LevelInfo>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
            ...
            if (isLevelDeletionFromCacheRequired(info, levelCache)) {
                ...
                for (String key : keysToBeRemoved) {
                    String cubeUniqueName =
                            key.substring(0, (key.indexOf(MolapCommonConstants.LOAD_FOLDER) - 1));
     ...
     * @return
     */
    public List<RestructureStore> loadSliceFromFile(final MolapDef.Cube rolapCube, String basePath,
            final MolapDef.Schema schema, final List<String> loadFolderNames,
        ...
        for (final MolapFile rsFolder : files) {
            ...
            for (MolapFile tableFolder : tableFiles) {
                ...
                SliceMetaData smd = readSliceMetaDataFile(
                        tableFolder.getAbsolutePath() + '/' + MolapUtil
                                ...
                                if (!isLoadFolderAlreadyPresent(slices, sliceUpdatedLoadPaths,
                                    ...
                                    if (null != loadFolderNames) {
                                        if (!loadFolderNames.contains(loadFolder.getName())) {
                                            ...
                                            InMemoryCube cubeCache =
                                                    new InMemoryCube(schema, rolapCube,
                                                            metadataCube);
                                        ...
                                        InMemoryCube cubeCache =
                                                new InMemoryCube(schema, rolapCube, metadataCube);
                                        cubeCache.setLoadName(loadFolder.getName());
                                        cubeCache.setRsStore(rsStore);
    ...
    private boolean isLoadFolderAlreadyPresent(List<RestructureStore> slices,
            List<String> sliceUpdatedLoadPaths, MolapFile rsFolder, String tableName,
            String loadName) {
     ...
     * @return
     */
    private MolapFile[] getSortedFolderListList(String path, final String folderStsWith,
    ...
    public void informSchemaPublished(String schemaName) {
        ...
        for (Iterator<Entry<String, MolapDef.Schema>> iter = entrySet.iterator(); iter
                .hasNext(); ) {
