...
public class ArrayQueryType implements GenericQueryType {
    ...
    public void parseBlocksAndReturnComplexColumnByteArray(
        ...
        if (dataLength == 0) {
            ...
            for (int i = 0; i < dataLength; i++) {
                children.parseBlocksAndReturnComplexColumnByteArray(columnarKeyStoreDataHolder,
                        columnIndex++, dataOutputStream);
            }
        }
    }
    ...
    public Object getDataBasedOnDataTypeFromSurrogates(List<InMemoryTable> slices,
        ...
        for (int i = 0; i < dataLength; i++) {
            data[i] = children.getDataBasedOnDataTypeFromSurrogates(slices, surrogateData,
                    dimensions);
        }
