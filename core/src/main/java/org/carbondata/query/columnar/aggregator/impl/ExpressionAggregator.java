    ...
    for (int i = 0; i < columnaraggreagtorInfo.getCustomExpressions().size(); i++) {
      ...
      for (int j = 0; j < referredColumns.size(); j++) {
        ...
        if (dimension instanceof Measure) {
          ...
          byte[] noDictionaryVal = keyValue.getNo_DictionayDimDataForAgg(dimension.getOrdinal());
          row[j] = DataTypeConverter
              .getDataBasedOnDataType(new String(noDictionaryVal), dimension.getDataType());
        } else {
          if (dimension.getDataType() != Type.ARRAY && dimension.getDataType() != Type.STRUCT) {
            ...
            if (dimSurrogate == 1) {
              ...
              String member = QueryExecutorUtility.getMemberBySurrogateKey(dimension, dimSurrogate,
                  columnaraggreagtorInfo.getSlices(), columnaraggreagtorInfo.getCurrentSliceIndex())
                  .toString();
              row[j] = DataTypeConverter.getDataBasedOnDataType(member, dimension.getDataType());
            }
          } else {
            try {
              GenericQueryType complexType = null;
              complexType =
                  this.columnaraggreagtorInfo.getComplexQueryDims().get(dimension.getOrdinal());
              ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
              DataOutputStream dataOutputStream = new DataOutputStream(byteStream);
              keyValue.getComplexDimDataForAgg(complexType, dataOutputStream);
              row[j] = complexType
                  .getDataBasedOnDataTypeFromSurrogates(this.columnaraggreagtorInfo.getSlices(),
