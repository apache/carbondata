...
public class ColumnarIncludeExculeFilterProcessor extends ColumnarIncludeFilterProcessor {
        ...
        if (null != filterModel.getFilter()[keyBlockIndx]) {
            ...
            if (isExcludeFilterPresent) {
                setFilterdIndexToBitSet(keyBlockArray[0], numerOfRows,
                        filterModel.getExcludeFilter()[keyBlockIndx], bitset);
            }
        }
        return bitset;
    }
    ...
    private void setFilterdIndexToBitSet(ColumnarKeyStoreDataHolder keyBlockArray, int numerOfRows,
        ...
        for (int i = 0; i < filter.length; i++) {
            int start = MolapUtil
                    .getFirstIndexUsingBinarySearch(keyBlockArray, 0, numerOfRows - 1, filter[i]);
            int last = MolapUtil
                    .getLastIndexUsingBinarySearch(keyBlockArray, 0, numerOfRows - 1, filter[i],
                            numerOfRows);
            if (start == -1 || last == -1) {
                continue;
            }
