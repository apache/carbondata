    ...
    public void getFilteredIndexes(int keyBlockIndex, int numerOfRows, BitSet set,
       ...
       /* if(useBitSet(set, filter, numerOfRows) && false)
        ...
        if (null == keyBlockArray[0].getColumnarKeyStoreMetadata().getColumnIndex()) {
            ...
            setFilterdIndexToBitSetWithColumnIndex(keyBlockArray[0], numerOfRows, filter,
                    localBitSet);
        }
        set.and(localBitSet);
    }

    private void setFilterdIndexToBitSetWithColumnIndex(ColumnarKeyStoreDataHolder keyBlockArray,
            int numerOfRows, byte[][] filter, BitSet bitSet) {
        ...
        for (int i = 0; i < filter.length; i++) {
            ...
            for (int j = start + 1; j < numerOfRows; j++) {
                if (ByteUtil.UnsafeComparer.INSTANCE
                        .compareTo(keyBlockArray.getKeyBlockData(), j * filter[i].length,
                                filter[i].length, filter[i], 0, filter[i].length) == 0) {
