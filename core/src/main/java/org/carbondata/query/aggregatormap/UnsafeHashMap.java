...
package org.carbondata.query.aggregatormap;
...
public class UnsafeHashMap {
    ...
    public UnsafeHashMap(int initialCapacity, int keySize) {
        ...
        map = new BytesToBytesMap(memoryManager, SparkEnv.get().shuffleMemoryManager(), 10000000,
                1.0d, SparkEnv.get().shuffleMemoryManager().pageSizeBytes(), false);
    ...
    public int putKey(byte[] key) {
        ...
        if (!lookup.isDefined()) {
            ++counter;
            lookup.putNewKey(key, BYTE_ARRAY_OFFSET, keySize, new int[] { counter },
                    INT_ARRAY_OFFSET, 4);
            return counter;
        } else {
            int[] index = new int[1];
            Platform.copyMemory(lookup.getValueAddress().getBaseObject(),
                    lookup.getValueAddress().getBaseOffset(), index, INT_ARRAY_OFFSET, 12);
            return index[0];
    ...
    public Map<ByteArrayWrapper, MeasureAggregator[]> getData() {
        ...
        Map<ByteArrayWrapper, MeasureAggregator[]> finalMap =
                new HashMap<ByteArrayWrapper, MeasureAggregator[]>(map.numElements(), 1.0f);
        byte[] key = null;
        int[] index = new int[1];
        Location next = null;
        ...
        while (iterator.hasNext()) {
            ...
            Platform.copyMemory(next.getValueAddress().getBaseObject(),
                    next.getValueAddress().getBaseOffset(), index, INT_ARRAY_OFFSET, 4);

            Platform.copyMemory(next.getKeyAddress().getBaseObject(),
                    next.getKeyAddress().getBaseOffset(), key, INT_ARRAY_OFFSET, keySize);
