...
public class FilterScanner extends AbstractDataBlocksProcessor {
	 ...
	 * if it present then only we can apply filter on complete data. this will be very useful in case of sparse
	 * data when rows are repeating.
	 */
	...
	public FilterScanner(BlockExecutionInfo blockExecutionInfo) {
		...
		String minMaxEnableValue = CarbonProperties.getInstance().getProperty(
				"carbon.enableMinMax");
		if (null != minMaxEnableValue) {
			isMinMaxEnabled = Boolean.parseBoolean(minMaxEnableValue);
		}
		// get the filter tree
		this.filterEvaluator = blockExecutionInfo
				.getFilterEvaluatorTree();
	}

	/**
	 * Below method will be used to process the block
	 * @param blocksChunkHolder
	 * 			block chunk holder which holds the data 
	 */
	@Override
	...
	private void fillScannedResult(BlocksChunkHolder blocksChunkHolder) {
		...
		// apply min max 
		if (isMinMaxEnabled) {
			BitSet bitSet = this.filterEvaluator.isScanRequired(
		...
		int[] allSelectedDimensionBlocksIndexes = tableBlockExecutionInfos
				.getAllSelectedDimensionBlocksIndexes();
		DimensionColumnDataChunk[] dimensionColumnDataChunk = new DimensionColumnDataChunk[tableBlockExecutionInfos
				.getTotalNumberDimensionBlock()];
		...
		for (int i = 0; i < allSelectedDimensionBlocksIndexes.length; i++) {
			if (null == blocksChunkHolder.getDimensionDataChunk()[allSelectedDimensionBlocksIndexes[i]]) {
				dimensionColumnDataChunk[allSelectedDimensionBlocksIndexes[i]] = blocksChunkHolder
						.getDataBlock().getDimensionChunk(fileReader,
								allSelectedDimensionBlocksIndexes[i]);
			} else {
				dimensionColumnDataChunk[allSelectedDimensionBlocksIndexes[i]] = blocksChunkHolder
						.getDimensionDataChunk()[allSelectedDimensionBlocksIndexes[i]];
			}
		}
		MeasureColumnDataChunk[] measureColumnDataChunk = new MeasureColumnDataChunk[tableBlockExecutionInfos
				.getTotalNumberOfMeasureBlock()];
		int[] allSelectedMeasureBlocksIndexes = tableBlockExecutionInfos
		...
		for (int i = 0; i < allSelectedMeasureBlocksIndexes.length; i++) {
			...
			if (null == blocksChunkHolder.getMeasureDataChunk()[allSelectedMeasureBlocksIndexes[i]]) {
				measureColumnDataChunk[allSelectedMeasureBlocksIndexes[i]] = blocksChunkHolder
						.getDataBlock().getMeasureChunk(fileReader,
								allSelectedMeasureBlocksIndexes[i]);
			} else {
				measureColumnDataChunk[allSelectedMeasureBlocksIndexes[i]] = blocksChunkHolder
						.getMeasureDataChunk()[allSelectedMeasureBlocksIndexes[i]];
			}
		}
