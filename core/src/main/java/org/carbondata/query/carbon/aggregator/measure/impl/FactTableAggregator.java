...
public class FactTableAggregator extends MeasureDataAggregator {
	...
	public FactTableAggregator(BlockExecutionInfo tableBlockExecutionInfos) {
		...
		isMeasureExistsInCurrentBlock = tableBlockExecutionInfos
				.getAggregatorInfo().getMeasureExists();
	 ...
	 */
	@Override
	public void aggregateMeasure(AbstractScannedResult scannedResult,
		...
		for (short i = 0; i < measuresOrdinal.length; i++) {
			...
			if (isMeasureExistsInCurrentBlock[i]) {
				aggrgeator[measureColumnStartIndex + i].agg(
						scannedResult.getMeasureChunk(measuresOrdinal[i]),
