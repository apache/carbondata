...
public class InternalQueryExecutorFactory {
	...
	private static final LogService LOGGER = LogServiceFactory
			.getLogService(InternalQueryExecutorFactory.class.getName());
	...
	public static InternalQueryExecutor getQueryExecutor(QueryModel queryModel,
		...
		// if all the other query property like query dimension dimension aggregation expression are empty 
		// and is count start query is true with one measure on which counter will be executed 
		// then its a counter start query 
		if (queryModel.isCountStarQuery()
				&& null == queryModel.getFilterEvaluatorTree()
				&& queryModel.getQueryDimension().size() < 1
				&& queryModel.getQueryDimension().size() < 2
		...
		// in that case we need to return empty record of size number of records present in the carbon data file
		else if (null == queryModel.getFilterEvaluatorTree()
				&& queryModel.getQueryDimension().size()==0
		...
		// to handle detail with order by query 
				...
				&& queryModel.getSortDimension().size() > 0) {
			LOGGER.info(CarbonEngineLogEvent.UNIBI_CARBONENGINE_MSG,
					"Detail with order by query: ");
