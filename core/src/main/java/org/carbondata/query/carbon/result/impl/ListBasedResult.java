...
public class ListBasedResult implements Result<List<ListBasedResultWrapper>> {
	...
	public ListBasedResult() {
		currentRowPointer = new ArrayList<ListBasedResultWrapper>(
				CarbonCommonConstants.DEFAULT_COLLECTION_SIZE);
		allRowsResult = new ArrayList<List<ListBasedResultWrapper>>(
				CarbonCommonConstants.DEFAULT_COLLECTION_SIZE);
	}
	 ...
	 */
	@Override
	public void addScannedResult(List<ListBasedResultWrapper> listBasedResult) {
	...
	public boolean hasNext() {
		...
		// As we are storing data in list of list, below code is to check whether any more result is present 
		// in the result. 
		...
		// if more list are present and all the list of current list is processed than 
		// it will take a new list from all row result list 
		totalRecordCounter++;
		listRecordCounter++;
		if (listRecordCounter == 0
				|| (listRecordCounter >= currentRowPointer.size() && currentListCounter < allRowsResult
						.size())) {
			listRecordCounter = 0;
	 ...
	 */
	@Override
	public void merge(Result<List<ListBasedResultWrapper>> otherResult) {
	 ...
	 */
	@Override
	public List<ListBasedResultWrapper> getResult() {
