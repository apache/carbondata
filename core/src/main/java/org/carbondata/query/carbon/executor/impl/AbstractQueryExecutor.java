...
public abstract class AbstractQueryExecutor implements QueryExecutor {
	...
	private static final LogService LOGGER = LogServiceFactory
			.getLogService(AbstractQueryExecutor.class.getName());
	/**
	 ...
	 * @param queryModel
	 */
	protected void initQuery(QueryModel queryModel)
			throws QueryExecutionException {
		/**
		 * setting the table unique name
		 */
		queryProperties.tableUniqueName = queryModel
				.getAbsoluteTableIdentifier().getCarbonTableIdentifier()
				.getDatabaseName()
				+ '_'
		...
		if (!queryModel.isDetailQuery()) {
			...
			for (DimensionAggregatorInfo dimensionAggregationInfo : queryModel
					.getDimAggregationInfo()) {
				for (int i = 0; i < dimensionAggregationInfo.getAggList()
						.size(); i++) {
					aggTypes[currentIndex] = dimensionAggregationInfo
							.getAggList().get(i);
					dataTypes[currentIndex] = dimensionAggregationInfo.getDim()
							.getDataType();
				}
			}
		...
		queryProperties.measureAggregators = MeasureAggregatorFactory
				.getMeassureAggregator(aggTypes, dataTypes,
						queryModel.getExpressions());
		...
		queryProperties.aggExpressionStartIndex = aggTypes.length
				- queryModel.getExpressions().size()
	...
	private KeyStructureInfo getKeyStructureInfo(QueryModel queryModel,
		...
		int[] maskByteRanges = QueryUtil.getMaskedByteRange(
				queryModel.getQueryDimension(), keyGenerator);

		// getting the masked bytes for query dimension dictionary column
		int[] maskedBytes = QueryUtil.getMaksedByte(
				keyGenerator.getKeySizeInBytes(), maskByteRanges);
		...
		try {
			...
			// masked key
			maxKey = QueryUtil.getMaxKeyBasedOnDimensions(
					queryModel.getQueryDimension(), keyGenerator);
		} catch (KeyGenException e) {
			LOGGER.error(CarbonCoreLogEvent.UNIBI_CARBONCORE_MSG, e,
					"problem while getting the max key");
		}
