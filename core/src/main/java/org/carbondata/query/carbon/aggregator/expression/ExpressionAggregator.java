...
public class ExpressionAggregator {
	...
	public ExpressionAggregator(BlockExecutionInfo blockExecutionInfo) {
		...
		this.expressionStartIndex = blockExecutionInfo
				.getExpressionStartIndex();
	 ...
	 *            measure aggregate to aggregate the data
	 */
	public void aggregateExpression(AbstractScannedResult scannedResult,
		...
		for (int i = 0; i < this.blockExecutionInfo
				.getCustomAggregateExpressions().size(); i++) {
			List<CarbonColumn> referredColumns = this.blockExecutionInfo
			...
			for (int j = 0; j < referredColumns.size(); j++) {
				...
				if (!carbonColumn.isDimesion()) {
					...
					// was null or not
					if (!scannedResult.isNullMeasureValue(carbonColumn
							.getOrdinal())) {
						...
						case LONG:
							row[j] = scannedResult
									.getLongMeasureValue(carbonColumn.getOrdinal());
							break;
						case DECIMAL:
							row[j] = scannedResult
									.getBigDecimalMeasureValue(carbonColumn
											.getOrdinal());
							break;
						default:
							row[j] = scannedResult
									.getDoubleMeasureValue(carbonColumn
											.getOrdinal());
						}
					}
				} else if (!CarbonUtil.hasEncoding(carbonColumn.getEncoder(),
						Encoding.DICTIONARY)) {
					// for dictionary column get the data
					String noDictionaryColumnData = new String(
							scannedResult.getDimensionKey(carbonColumn
									.getOrdinal()));
					// if data is equal to default value then its null so no
					// need to do any thing
					if (!CarbonCommonConstants.MEMBER_DEFAULT_VAL
							.equals(noDictionaryColumnData)) {
						row[j] = DataTypeUtil
								.getDataBasedOnDataType(noDictionaryColumnData,
										carbonColumn.getDataType());
					}
				} else {

					// get the surrogate key of the dictionary column from data
					int surrogateKey = CarbonUtil.getSurrogateKey(scannedResult
							.getDimensionKey(carbonColumn.getOrdinal()), buffer);
						...
						row[j] = DataTypeUtil
								.getDataBasedOnDataType(
										blockExecutionInfo
												...
												.get(carbonColumn.getColumnId())
												.getDictionaryValueForKey(
														surrogateKey),
										carbonColumn.getDataType());
					}
				}
				CustomMeasureAggregator agg = (CustomMeasureAggregator) measureAggregator[expressionStartIndex
						+ i];
				rowImpl = new RowImpl();
