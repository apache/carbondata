...
public class QueryExecutorImpl extends AbstractQueryExecutor {
  ...
  @Override public CarbonIterator<RowResult> execute(QueryModel queryModel)
    ...
    InternalQueryExecutor queryExecutor =
        InternalQueryExecutorFactory.getQueryExecutor(queryModel, queryProperties.dataBlocks);
    ...
    for (int i = 0; i < queryProperties.dataBlocks.size(); i++) {
      blockExecutionInfoList
          .add(getBlockExecutionInfoForBlock(queryModel, queryProperties.dataBlocks.get(i), false));
    }
    blockExecutionInfoList.add(getBlockExecutionInfoForBlock(queryModel,
        queryProperties.dataBlocks.get(queryProperties.dataBlocks.size() - 1), true));
    ...
    if (blockExecutionInfoList.size() > 0) {
      if (!queryModel.isDetailQuery() || (queryModel.isDetailQuery() && null != queryModel
        ...
        queryResultIterator = queryExecutor.executeQuery(blockExecutionInfoList, null);
      } else {
    ...
    ChunkBasedResultIterator chunkBasedResultIterator =
        new ChunkBasedResultIterator(queryResultIterator, queryProperties, queryModel);
  ...
  private BlockExecutionInfo getBlockExecutionInfoForBlock(QueryModel queryModel,
      AbstractIndex blockIndex, boolean isLastBlock) throws QueryExecutionException {
    ...
    int[] maksedByte =
        QueryUtil.getMaksedByte(blockKeyGenerator.getKeySizeInBytes(), maskByteRangesForBlock);
   ...
   */
  private SortInfo getSortInfos(List<CarbonDimension> orderByDimension, QueryModel queryModel)
      throws QueryExecutionException {
