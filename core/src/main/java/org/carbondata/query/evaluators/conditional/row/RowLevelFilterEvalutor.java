...
import org.carbondata.query.evaluators.*;
...
public class RowLevelFilterEvalutor extends AbstractConditionalEvalutor {
        ...
        for (DimColumnEvaluatorInfo dimColumnEvaluatorInfo : dimColEvaluatorInfoList) {
            if (dimColumnEvaluatorInfo.getDims().getDataType() != Type.ARRAY
                ...
                GenericQueryType complexType =
                        dimColumnEvaluatorInfo.getComplexTypesWithBlockStartIndex()
            ...
            if (msrColumnEvalutorInfo.isMeasureExistsInCurrentSlice() && null == blockDataHolder
                ...
                blockDataHolder.getMeasureBlocks()[msrColumnEvalutorInfo.getColumnIndex()] =
                        blockDataHolder.getLeafDataBlock()
                                ...
                                .getValues()[msrColumnEvalutorInfo.getColumnIndex()];
            }
        }
        //CHECKSTYLE:ON
    ...
    private void createRow(BlockDataHolder blockDataHolder, RowIntf row, int index) {
        Object[] record =
                new Object[dimColEvaluatorInfoList.size() + msrColEvalutorInfoList.size()];
            ...
            if (dimColumnEvaluatorInfo.getDims().getDataType() != Type.ARRAY
                ...
                if (dimColumnEvaluatorInfo.getDims().isNoDictionaryDim()) {
                    ...
                    Member member = QueryExecutorUtility
                            .getMemberBySurrogateKey(dimColumnEvaluatorInfo.getDims(),
                                    blockDataHolder.getColumnarKeyStore()[dimColumnEvaluatorInfo
                                            .getColumnIndex()].getSurrogateKey(index),
                ...
                try {
                    ...
                    complexType.parseBlocksAndReturnComplexColumnByteArray(
                            blockDataHolder.getColumnarKeyStore(), index, dataOutputStream);
        ...
        for (MsrColumnEvalutorInfo msrColumnEvalutorInfo : msrColEvalutorInfoList) {
            ...
            if (!msrColumnEvalutorInfo.isMeasureExistsInCurrentSlice()) {
                record[msrColumnEvalutorInfo.getRowIndex()] =
                        msrColumnEvalutorInfo.getDefaultValue();
            } else {
                if (msrColumnEvalutorInfo.isCustomMeasureValue()) {
                    MeasureAggregator aggregator =
                            AggUtil.getAggregator(msrColumnEvalutorInfo.getAggregator(), false,
                    ...
                    case LONG:
                        msrValue = blockDataHolder.getMeasureBlocks()[msrColumnEvalutorInfo
                                .getColumnIndex()].getReadableLongValueByIndex(index);
                        break;
                    case DECIMAL:
                        msrValue = blockDataHolder.getMeasureBlocks()[msrColumnEvalutorInfo
                                .getColumnIndex()].getReadableBigDecimalValueByIndex(index);
                        break;
                    default:
                        msrValue = blockDataHolder.getMeasureBlocks()[msrColumnEvalutorInfo
