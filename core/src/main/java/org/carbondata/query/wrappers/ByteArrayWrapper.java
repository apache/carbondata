...
public class ByteArrayWrapper implements Comparable<ByteArrayWrapper>, Serializable {
    ...
    if (null != listOfNoDictionaryValVal) {
      if (listOfNoDictionaryValVal.size() != otherList.size()) {
        ...
        for (int i = 0; i < listOfNoDictionaryValVal.size(); i++) {
          result =
              UnsafeComparer.INSTANCE.equals(listOfNoDictionaryValVal.get(i), otherList.get(i));
          if (!result) {
            return false;
          }
        }
    ...
    if (null != complexTypesData) {
      if (complexTypesData.size() != otherComplexTypesData.size()) {
        ...
        for (int i = 0; i < complexTypesData.size(); i++) {
          result =
              UnsafeComparer.INSTANCE.equals(complexTypesData.get(i), otherComplexTypesData.get(i));
          if (!result) {
            return false;
          }
        }
  ...
  @Override public int compareTo(ByteArrayWrapper other) {
    ...
    if (compareTo == 0) {
      if (null != listOfNoDictionaryValVal) {
        for (int i = 0; i < listOfNoDictionaryValVal.size(); i++) {
          compareTo = UnsafeComparer.INSTANCE
              .compareTo(listOfNoDictionaryValVal.get(i), other.listOfNoDictionaryValVal.get(i));
          if (compareTo != 0) {
            return compareTo;
          }
        }
