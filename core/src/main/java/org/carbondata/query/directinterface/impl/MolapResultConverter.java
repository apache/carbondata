...
import org.carbondata.query.queryinterface.query.metadata.MolapMember;
import org.carbondata.query.queryinterface.query.metadata.MolapTuple;
import org.carbondata.query.queryinterface.query.result.impl.MolapResultChunkImpl;
import org.carbondata.query.queryinterface.query.result.impl.MolapResultStreamImpl;
...
public final class MolapResultConverter {
    ...
    public static MolapResultStreamImpl convertInternal(MolapResultHolder rs,
            List<MolapMetadata.Dimension> queryDimsRows,
            List<MolapMetadata.Dimension> queryDimsCols, List<Measure> queryMsrs,
            boolean propertiesRequired, boolean useNamesFromHolder) {
        ...
        if (useNamesFromHolder && rs.getColHeaders() != null && !rs.getColHeaders().isEmpty()) {
            ...
            calculateIndexesUsingNameOrder(queryDimsRows, propertiesRequired, rowsIndexes,
                    rowsProps, namesOrder);
            calculateIndexesUsingNameOrder(queryDimsCols, propertiesRequired, colsIndexes,
                    colsProps, namesOrder);
        ...
        if (colsIndexes.length > 0) {
            ...
            Map<MolapTuple, Integer> colTupleMappings =
                    new HashMap<MolapTuple, Integer>(MolapCommonConstants.DEFAULT_COLLECTION_SIZE);
            ...
            chunkImpl.setData(
                    new ArrayList<Object[]>(rowTupleMap.values()).toArray(new Object[0][0]));
        } else {
            List<MolapTuple> rowTuples =
                    new ArrayList<MolapTuple>(MolapCommonConstants.CONSTANT_SIZE_TEN);
     ...
     */
    public static MolapResultStreamImpl convert(MolapResultHolder rs,
            List<MolapMetadata.Dimension> queryDimsRows,
            List<MolapMetadata.Dimension> queryDimsCols, List<Measure> queryMsrs,
            boolean propertiesRequired) {
        return convertInternal(rs, queryDimsRows, queryDimsCols, queryMsrs, propertiesRequired,
                false);
    }
     ...
     * @return MolapResultStreamImpl
     */
    public static MolapResultStreamImpl convertUsingNameMapping(MolapResultHolder rs,
            List<MolapMetadata.Dimension> queryDimsRows,
            List<MolapMetadata.Dimension> queryDimsCols, List<Measure> queryMsrs,
            boolean propertiesRequired) {
        return convertInternal(rs, queryDimsRows, queryDimsCols, queryMsrs, propertiesRequired,
     ...
     * @param colTuples
     */
    private static void createColTuples(MolapResultHolder rs, boolean propertiesRequired,
            int[] colsIndexes, int[][] colsProps, Set<MolapTuple> colTuples) {
     ...
     * @return
     */
    private static List<MolapTuple> getActualColTuples(List<MolapTuple> molapTuples,
     ...
     * @return
     */
    private static int calculateIndexes(List<MolapMetadata.Dimension> queryDimsRows,
            boolean propertiesRequired, int[] rowsIndexes, int[][] rowsProps, int k) {
        for (int i = 0; i < queryDimsRows.size(); i++) {
            ...
            if (propertiesRequired) {
                if (dimension.getPropertyCount() > 0) {
                    props = new int[dimension.isHasNameColumn() ?
                            (dimension.getPropertyCount() - 1) :
                            dimension.getPropertyCount()];
    ...
    private static void calculateIndexesUsingNameOrder(List<MolapMetadata.Dimension> queryDimsRows,
            boolean propertiesRequired, int[] rowsIndexes, int[][] rowsProps,
