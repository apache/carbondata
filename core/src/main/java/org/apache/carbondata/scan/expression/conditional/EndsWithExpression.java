...
public class EndsWithExpression extends BinaryConditionalExpression {

  private static final long serialVersionUID = 1L;

  public EndsWithExpression(Expression left, Expression right) {
    super(left, right);
  }

  public EndsWithExpression(Expression left, Expression right, boolean isNull) {
    super(left, right);
    this.isNull = isNull;
  }
  ...
  @Override public ExpressionResult evaluate(RowIntf value)
    ...
    switch (val1.getDataType()) {
      case STRING:
        result = val1.getString().endsWith(val2.getString());
        break;
      case SHORT:
        result = val1.getShort().toString().endsWith(val2.getShort().toString());
        break;
      case INT:
        result = val1.getInt().toString().endsWith(val2.getInt().toString());
        break;
      case DOUBLE:
        result = val1.getDouble().toString().endsWith(val2.getDouble().toString());
        break;
      case TIMESTAMP:
        result = val1.getTime().toString().endsWith(val2.getTime().toString());
        break;
      case LONG:
        result = val1.getLong().toString().endsWith(val2.getLong().toString());
        break;
      case DECIMAL:
        result = val1.getDecimal().toEngineeringString()
         .endsWith(val2.getDecimal().toEngineeringString());
        break;
      default:
  ...
  @Override public ExpressionType getFilterExpressionType() {
    return ExpressionType.ENDS_WITH;
  }

  @Override public String getString() {
    return "endsWith(" + left.getString() + ',' + right.getString() + ')';
  }
