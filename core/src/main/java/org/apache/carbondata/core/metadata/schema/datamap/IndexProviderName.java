...
public enum IndexProviderName {
  ...
  IndexProviderName(String className, String shortName) {
  ...
  public static IndexProviderName get(String indexProviderName) {
    if (LUCENE.isEqual(indexProviderName)) {
      return LUCENE;
    } else if (BLOOMFILTER.isEqual(indexProviderName)) {
      return BLOOMFILTER;
    } else {
      throw new UnsupportedOperationException("Unknown datamap provider" + indexProviderName);
    }
  }
}
