...
public interface Index<T extends Blocklet> {
  ...
  /**
   * Prune the segment for finding the row count. It returns a Map of
   * blockletpath and the row count
   */
  long getRowCount(Segment segment, List<PartitionSpec> partitions);

  /**
   * Prune the segment for finding the row count for each block. It returns a Map of
   * blockletpath and the row count
   */
   ...
   * So index can have multiple rows if they store information of multiple files.
