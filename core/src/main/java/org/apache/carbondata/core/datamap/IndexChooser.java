...
public class IndexChooser {
  ...
  public IndexChooser(CarbonTable carbonTable) throws IOException {
    ...
    cgIndexes = new ArrayList<>(visibleDataMaps.size());
    fgIndexes = new ArrayList<>(visibleDataMaps.size());
    for (TableIndex visibleDataMap : visibleDataMaps) {
      DataMapStatusDetail status = map.get(visibleDataMap.getDataMapSchema().getDataMapName());
      if (status != null && status.isEnabled()) {
        IndexLevel level = visibleDataMap.getIndexFactory().getDataMapLevel();
        if (level == IndexLevel.CG) {
          cgIndexes.add(visibleDataMap);
        } else {
          fgIndexes.add(visibleDataMap);
        }
      }
    }
  }
