...
public class ArrayDataRefNodeFinder implements DataRefNodeFinder {
  ...
  public ArrayDataRefNodeFinder(int[] eachColumnValueSize, int numberOfSortColumns,
  ...
  @Override public DataRefNode findFirstDataBlock(DataRefNode dataRefBlock, IndexKey searchKey) {
    ...
    BlockIndexNodeWrapper rootNode = (BlockIndexNodeWrapper) dataRefBlock;
    IndexStore indexStore = rootNode.getIndexStore();
    return findFirstLeafNode(searchKey, indexStore);
  }
  ...
  @Override public DataRefNode findLastDataBlock(DataRefNode dataRefBlock, IndexKey searchKey) {
    ...
    BlockIndexNodeWrapper rootNode = (BlockIndexNodeWrapper) dataRefBlock;
    IndexStore indexStore = rootNode.getIndexStore();
    return findLastLeafNode(searchKey, indexStore);
  }
  ...
  private DataRefNode findFirstLeafNode(IndexKey key, IndexStore indexStore) {
    int childNodeIndex;
    int low = 0;
    int high = indexStore.getIndexKeyCount() - 1;
    int mid = 0;
    int compareRes = -1;
    //
    while (low <= high) {
      mid = (low + high) >>> 1;
      // compare the entries
      compareRes = compareIndexes(key, indexStore.getIndexKey(mid));
      if (compareRes < 0) {
        high = mid - 1;
    ...
    // get the leaf child
    return new BlockIndexNodeWrapper(indexStore, (short) childNodeIndex);
  }
  ...
  private DataRefNode findLastLeafNode(IndexKey key, IndexStore indexStore) {
    ...
    while (low <= high) {
      ...
      if (compareRes < 0) {
        ...
        int currentPos = mid;
        // if key is matched then get the first entry
        while (currentPos + 1 < indexStore.getIndexKeyCount()
            && compareIndexes(key, indexStore.getIndexKey(currentPos + 1)) == 0) {
          currentPos++;
        }
        mid = currentPos;
