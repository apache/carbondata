 ...
 */
public class FixedLengthDimensionDataChunkStore extends AbstractDimensionDataChunkStore {
   ...
   */
  public FixedLengthDimensionDataChunkStore(int columnValueSize, boolean isInvertedIdex,
      int numberOfRows, int dataSize) {
    super(dataSize, isInvertedIdex, numberOfRows);
  ...
  @Override public int getSurrogate(int index) {
    if (isExplictSorted) {
      index = CarbonUnsafe.unsafe.getInt(invertedIndexReverseMemoryBlock.getBaseObject(),
          invertedIndexReverseMemoryBlock.getBaseOffset() + (index
              * CarbonCommonConstants.INT_SIZE_IN_BYTE));
    }
