...
/**
 * Input format for index, it makes the pruning distributable.
 */
public class DistributableIndexFormat extends FileInputFormat<Void, ExtendedBlocklet>
    implements Serializable, Writable {

  private static final transient Logger LOGGER =
      LogServiceFactory.getLogService(DistributableIndexFormat.class.getName());
  ...
  private boolean isJobToClearIndex = false;
  ...
  DistributableIndexFormat() {

  }

  DistributableIndexFormat(CarbonTable table,
      List<Segment> validSegments, List<String> invalidSegments, boolean isJobToClearIndex,
      String indexToClear) {
    this(table, null, validSegments, invalidSegments, null, isJobToClearIndex, null, false, false);
    this.indexToClear = indexToClear;
  }

  public DistributableIndexFormat(CarbonTable table, FilterResolverIntf filterResolverIntf,
      List<Segment> validSegments, List<String> invalidSegments, List<PartitionSpec> partitions,
      boolean isJobToClearIndex, IndexLevel indexLevel, boolean isFallbackJob,
      boolean isAsyncCall) {
    this.table = table;
    ...
    this.isJobToClearIndex = isJobToClearIndex;
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    List<IndexDistributableWrapper> distributables;
    distributables =
  ...
  public RecordReader<Void, ExtendedBlocklet> createRecordReader(InputSplit inputSplit,
    ...
    return new RecordReader<Void, ExtendedBlocklet>() {
      ...
      public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
          throws IOException {
        IndexDistributableWrapper distributable = (IndexDistributableWrapper) inputSplit;
        ...
        if (indexLevel == null) {
          TableIndex defaultIndex = DataMapStoreManager.getInstance()
              .getIndexInTable(table, distributable.getDistributable().getDataMapSchema());
          blocklets = defaultIndex
              .prune(segmentsToLoad, new IndexFilter(filterResolverIntf), partitions);
          blocklets = IndexUtil
              .pruneByIndex(table, filterResolverIntf, segmentsToLoad, partitions, blocklets,
  ...
  public void write(DataOutput out) throws IOException {
    ...
    out.writeBoolean(isJobToClearIndex);
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this.isJobToClearIndex = in.readBoolean();
   ...
   */
  public boolean isJobToClearIndex() {
    return isJobToClearIndex;
  }
