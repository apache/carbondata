...
public class DiskBasedDataMapStatusProvider implements DataMapStatusStorageProvider {
   ...
   */
  @Override
  public void updateDataMapStatus(List<IndexSchema> indexSchemas, DataMapStatus dataMapStatus)
      throws IOException {
    ...
    try {
      ...
      if (locked) {
        ...
        if (dataMapStatus == DataMapStatus.ENABLED && !indexSchemas.get(0).isIndex()) {
          // Enable datamap only if datamap tables and main table are in sync
          if (!DataMapSyncStatus.canDataMapBeEnabled(indexSchemas.get(0))) {
            return;
          }
        }
        ...
        for (IndexSchema indexSchema : indexSchemas) {
          ...
          if (!exists) {
            newStatusDetails
                .add(new DataMapStatusDetail(indexSchema.getIndexName(), dataMapStatus));
          }
        }
