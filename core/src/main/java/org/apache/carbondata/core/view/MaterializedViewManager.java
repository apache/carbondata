...
public abstract class MaterializedViewManager {
  ...
  private static final Logger LOGGER =
      LogServiceFactory.getLogService(MaterializedViewManager.class.getName());

  private final MaterializedViewProvider schemaProvider =
      MaterializedViewProvider.get();

  private volatile MaterializedViewCatalog<?> catalog;
  ...
  public boolean hasSchemaOnTable(CarbonTable table) throws IOException {
    List<MaterializedViewSchema> schemas = getSchemas();
    for (MaterializedViewSchema schema : schemas) {
      for (RelationIdentifier associatedTable : schema.getAssociatedTables()) {
        if (associatedTable.getDatabaseName().equalsIgnoreCase(table.getDatabaseName()) &&
            associatedTable.getTableName().equalsIgnoreCase(table.getTableName())) {
          return true;
        }
      }
    }
   ...
   * For show mv command.
   */
  public List<MaterializedViewSchema> getSchemasOnTable(CarbonTable table)
      throws IOException {
    List<MaterializedViewSchema> schemasOnTable = new ArrayList<>();
    List<MaterializedViewSchema> schemas = getSchemas();
    for (MaterializedViewSchema schema : schemas) {
   ...
   * For show mv command.
   */
  public List<MaterializedViewSchema> getSchemasOnTable(String databaseName,
   ...
   */
  public List<MaterializedViewSchema> getSchemas() throws IOException {
   ...
   * It gives all mv schemas from store.
   */
  public List<MaterializedViewSchema> getSchemas(String databaseName) throws IOException {
    return schemaProvider.getSchemas(this, databaseName);
  }

  public MaterializedViewSchema getSchema(String databaseName, String viewName) throws IOException {
   ...
   */
  public void createSchema(String databaseName, MaterializedViewSchema viewSchema)
      throws IOException {
   ...
   * Get the mv catalog.
   */
  public MaterializedViewCatalog<?> getCatalog() {
    return catalog;
  }
   ...
   */
  public MaterializedViewCatalog<?> getCatalog(
      MaterializedViewCatalogFactory<?> catalogFactory,
      boolean reload) throws IOException {
    MaterializedViewCatalog<?> catalog = this.catalog;
  ...
  public static String getUpdatedSegmentMap(String mergedLoadName,
      MaterializedViewSchema viewSchema,
      LoadMetadataDetails[] viewLoadMetadataDetails) {
    Map<String, List<String>> segmentMapping = new HashMap<>();
    List<RelationIdentifier> relationIdentifiers = viewSchema.getAssociatedTables();
   ...
   */
  public List<MaterializedViewStatusDetail> getEnabledStatusDetails() throws IOException {
    List<MaterializedViewStatusDetail> statusDetails = new ArrayList<>();
    for (String database : this.getDatabases()) {
   ...
   * Get enabled mv status details
   */
  List<MaterializedViewStatusDetail> getEnabledStatusDetails(String databaseName)
      throws IOException {
    List<MaterializedViewStatusDetail> statusDetails =
        schemaProvider.getStatusDetails(databaseName);
    List<MaterializedViewStatusDetail> enabledStatusDetails = new ArrayList<>(statusDetails.size());
    for (MaterializedViewStatusDetail statusDetail : statusDetails) {
      if (statusDetail.getStatus() == MaterializedViewStatus.ENABLED) {
  ...
  public void setStatus(RelationIdentifier viewIdentifier, MaterializedViewStatus viewStatus)
      throws IOException {
    MaterializedViewSchema schema = getSchema(
  ...
  public void setStatus(List<MaterializedViewSchema> viewSchemas, MaterializedViewStatus viewStatus)
      throws IOException {
  ...
  public void onDrop(String databaseName, String viewName)
      throws IOException {
    MaterializedViewSchema viewSchema = getSchema(databaseName, viewName);
    if (viewSchema != null) {
      schemaProvider.updateStatus(
          Collections.singletonList(viewSchema), MaterializedViewStatus.DROPPED);
    }
  }
   ...
   */
  public void onTruncate(List<MaterializedViewSchema> schemas)
      throws IOException {
    for (MaterializedViewSchema schema : schemas) {
      if (!schema.isRefreshOnManual()) {
        setStatus(schema.identifier, MaterializedViewStatus.DISABLED);
      }
