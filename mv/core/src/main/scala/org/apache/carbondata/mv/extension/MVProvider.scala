...
class MVProvider(
  ...
  override def cleanMeta(): Unit = {
    ...
    // First, drop datamapschema and unregister MV from catalog, because if in
    ...
    try {
      ...
      DataMapStoreManager.getInstance.unRegisterMVCatalog(dataMapSchema)
    }
  }
