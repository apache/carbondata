...
trait AggregatePushDown { // self: ModularPlan =>
  ...
  private def transformAggregate(aggregate: AggregateExpression,
    ...
    aggregate match {
      case cnt@AggregateExpression(Count(exprs), _, false, None, _) if exprs.length == 1 &&
        exprs(0).isInstanceOf[Attribute] =>
        ...
        if (fact.outputSet.contains(tAttr)) {
          ...
          val tSum = AggregateExpression(Sum(alias.toAttribute), cnt.mode, false, None,
            cnt.resultId)
      ...
      case cnt@AggregateExpression(Count(exprs), _, false, None, _) if exprs.length == 1 &&
        ...
        val (name, id) = aliasInfo.getOrElse(("", NamedExpression.newExprId))
        map += (ith -> (Alias(tSum, name)(exprId = id), Seq(alias)))
      case sum@AggregateExpression(Sum(expr), _, false, None, _) if (expr
        ...
        if (fact.outputSet.contains(tAttr)) {
          ...
          val tSum = AggregateExpression(Sum(alias.toAttribute), sum.mode, false, None,
            sum.resultId)
          val (name, id) = aliasInfo.getOrElse(("", NamedExpression.newExprId))
          map += (ith -> (Alias(tSum, name)(exprId = id), Seq(alias)))
        } else {
      ...
      case sum@AggregateExpression(Sum(cast@MatchCast(expr, _)), _, false, None, _) =>
        val tAttr = selAliasMap.get(expr.asInstanceOf[Attribute]).getOrElse(expr)
          .asInstanceOf[Attribute]
        if (fact.outputSet.contains(tAttr)) {
      ...
      case sum@AggregateExpression(Sum(expr), _, false, None, _) if expr.isInstanceOf[Literal] =>
        ...
        val (name, id) = aliasInfo.getOrElse(("", NamedExpression.newExprId))
        map += (ith -> (Alias(tSum, name)(exprId = id), Seq(alias)))
      case max@AggregateExpression(Max(expr), _, false, None, _) if expr
        .isInstanceOf[Attribute] =>
        val tAttr = selAliasMap.get(expr.asInstanceOf[Attribute]).getOrElse(expr)
          .asInstanceOf[Attribute]
        if (fact.outputSet.contains(tAttr)) {
          ...
          val tMax = AggregateExpression(Max(alias.toAttribute), max.mode, false, None,
            max.resultId)
          val (name, id) = aliasInfo.getOrElse(("", NamedExpression.newExprId))
      ...
      case min@AggregateExpression(Min(expr), _, false, None, _) if expr
        .isInstanceOf[Attribute] =>
        val tAttr = selAliasMap.get(expr.asInstanceOf[Attribute]).getOrElse(expr)
          .asInstanceOf[Attribute]
        if (fact.outputSet.contains(tAttr)) {
          ...
          val tMin = AggregateExpression(Max(alias.toAttribute), min.mode, false, None,
            min.resultId)
          val (name, id) = aliasInfo.getOrElse(("", NamedExpression.newExprId))
      ...
      case avg@AggregateExpression(Average(expr), _, false, None, _) if expr
        .isInstanceOf[Attribute] =>
        val tAttr = selAliasMap.get(expr.asInstanceOf[Attribute]).getOrElse(expr)
          .asInstanceOf[Attribute]
        if (fact.outputSet.contains(tAttr)) {
