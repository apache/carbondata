          ...
          g.outputList.forall {
            case alias: Alias =>
              alias.child.isInstanceOf[AttributeReference] ||
                ...
                (alias.child match {
                  case AggregateExpression(First(_, _), _, _, _, _) => true
                  case AggregateExpression(Last(_, _), _, _, _, _) => true
                  case _ => false
                })
            case col =>
  ...
  protected override def computeStats(spark: SparkSession): Statistics = {
    ...
    val aliasMap = AttributeMap(
      source.asInstanceOf[GroupBy].outputList.collect {
        ...
        case a@Alias(AggregateExpression(First(ar: Attribute, _), _, _, _, _), _) =>
          (ar, a.toAttribute)
        case a@Alias(AggregateExpression(Last(ar: Attribute, _), _, _, _, _), _) =>
          (ar, a.toAttribute)
      })
